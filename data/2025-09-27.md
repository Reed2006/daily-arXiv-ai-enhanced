...
### [1] [Inverse Reinforcement Learning Using Just Classification and a Few Regressions](https://arxiv.org/abs/2509.21172)
...
Motivation: 传统最大熵IRL方法涉及复杂的内循环优化、重复动态规划或对抗训练，限制了现代高表达能力函数逼近器（如神经网络）的使用。

Method: 通过分析发现，群体最大似然解可由涉及行为策略的线性固定点方程表征，从而将IRL分解为概率分类和迭代回归两个监督学习任务。

Result: 该方法简单模块化，适用于各种函数逼近类，实验结果显示其性能与MaxEnt IRL相当或更优。

Conclusion: 该工作为IRL提供了一种更简单有效的实现框架，便于使用现代函数逼近器，具有广泛的应用前景。
...
TL;DR: 本文提出了多智能体生成流网络的理论框架，包含四种算法，实现了集中训练与分散执行的结合，并通过实验验证了其优于强化学习和MCMC方法。


<details>
  <summary>Details</summary>
Motivation: 现有的生成流网络缺乏多智能体协作的理论框架，无法支持多个智能体通过联合动作协作生成对象。

Method: 提出了MA-GFlowNets理论框架，包含集中式流网络、独立流网络、联合流网络及其条件版本四种算法，基于局部-全局原则实现集中训练与分散执行。

Result: 实验结果表明，所提出的框架在性能上优于强化学习和基于MCMC的方法。

Conclusion: MA-GFlowNets为多智能体协作生成提供了有效的理论框架和算法实现，具有重要的理论价值和实际应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有的草稿器（如EAGLE）需要N次顺序传递来提出N个标记，限制了推理速度。FastEagle旨在通过消除草稿生成中的顺序依赖关系，实现更高效的推理加速。

Method: FastEagle采用轻量级层级联结构替代时间步骤，通过层间监督训练来缓解错误累积，并结合约束草稿树保持无损失验证成本。
...
Conclusion: 消除草稿生成中的顺序依赖是实现无损失LLM推理加速的实用路径，FastEagle证明了这一方法的有效性。
...
TL;DR: 该论文提出了一种机器学习参数化方法(mloz)，用于在气候模型中交互式模拟臭氧变化，相比传统化学方案快31倍，且可在不同气候模型间迁移使用。
...
Method: 开发机器学习参数化方法，仅使用大气温度剖面信息作为输入，在UKESM和ICON两种气候模型中在线模拟臭氧的日变化和趋势，包括与准两年振荡的双向相互作用。

Result: mloz在十年时间尺度上表现出高保真度，比UKESM化学方案快31倍，仅占气候模型总运行时间的不到4%，且成功从UKESM迁移到ICON模型。

Conclusion: 该方法有潜力在缺乏交互式化学的CMIP级气候模型中广泛应用，特别适用于关注气候敏感性的未来气候变化评估。
...
TL;DR: 本文研究了对基于记忆化的影响估计器的攻击可行性，提出了一种实用的攻击方法，证明了即使是最先进的影响估计代理也容易受到针对性分数操纵。


<details>
  <summary>Details</summary>
Motivation: 随着影响估计工具在数据估值和负责任机器学习中的广泛应用，需要研究这些分数本身是否可能被对抗性操纵。

Method: 提出了一种实用的攻击方法，通过计算输入的伪逆来产生高度记忆化的样本，只需要对模型输出的黑盒访问，计算开销较小。

Result: 在广泛的图像分类任务中实证验证了攻击的有效性，表明最先进的影响估计代理也容易受到针对性分数操纵。

Conclusion: 研究揭示了基于影响的归因方法存在关键漏洞，表明需要开发鲁棒的防御机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有的目标条件强化学习（GCRL）方法主要使用对比表示或时序距离两种框架，但各自存在局限性：对比学习方法在拼接任务上表现不佳，而拟度量网络方法在噪声高维环境中表现较差。

Method: 提出了一种统一框架，结合拟度量表示空间的结构（三角不等式）和适当的约束条件，学习能够实现最优目标到达的后继表示。该方法能够利用拟度量距离参数化学习最优目标到达距离，即使使用次优数据和在随机环境中。
...
Method: 设计了能够显式捕获多尺度频率振荡的编码器；引入基于注意力的编码器学习EEG通道间和通道内局部补丁的交互；集成门控网络动态过滤噪声通道；使用结合监督学习和对比学习的新型损失函数。

Result: 该方法在多种CNS疾病治疗效果分类以及帕金森病和阿尔茨海默病诊断中验证有效，能够从不同物种的原始EEG信号中提取生物学有意义的模式。
...
TL;DR: DARTVAE是一种规则引导的多模态聚类框架，通过将领域特定约束嵌入到表示学习过程中，克服传统聚类仅依赖数据相似性的局限。
...
Method: 扩展VAE架构，嵌入显式规则、语义表示和数据驱动特征到统一潜在空间，通过规则一致性和违反惩罚在损失函数中强制约束合规。规则由LLM生成，构建为知识图谱。

Result: 在飞机和汽车数据集上的实验表明，规则引导聚类产生更具操作意义和可解释性的聚类（如分离无人机、统一隐形飞机、区分SUV和轿车），同时改进了传统聚类指标。

Conclusion: DARTVAE通过结合规则编码和学习表示，比纯数据驱动模型获得更有意义和一致的聚类结果，突显了约束引导多模态聚类在复杂知识密集型场景中的实用性，但面临LLM规则幻觉、规则冲突和可扩展性等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力层参数依赖输入元素的成对交互，导致内存和计算复杂度随序列长度呈二次方增长。现有方法（引入稀疏性或循环依赖）各有缺点，需要结合两者优势

Method: 基于树结构矩阵的高效求逆技术，开发新颖算法来结合稀疏注意力和循环依赖方法的优点

Result: 提出了一种能够有效缓解注意力机制二次方复杂度问题的新方法

Conclusion: 该算法通过树结构矩阵求逆技术，成功结合了稀疏性和循环依赖的优势，为解决注意力机制的计算复杂度问题提供了新思路
...
TL;DR: 本文提出了一种深度学习方法来预测混凝土中时间相关的全场损伤演化，通过自回归U-Net模型和CNN网络的双重架构，实现了高效的损伤预测和力学性能评估。


<details>
  <summary>Details</summary>
Motivation: 传统全场损伤评估计算成本高，需要开发更高效的方法来预测混凝土损伤演化，并理解骨料特性对力学性能的影响，以优化混凝土配合比设计。

Method: 采用自回归U-Net模型预测标量损伤场的演化，并使用CNN网络基于损伤估计预测关键力学性能（收缩和残余刚度），形成双网络架构。

Result: 该方法在合成数据集上表现出高计算效率和稳健的预测性能，显著降低了传统全场损伤评估的计算负担。

Conclusion: 该方法有助于深入理解骨料特性与有效收缩和刚度降低之间的关系，为优化混凝土配合比设计、提高耐久性和减少内部损伤提供了有效工具。
...
### [12] [Complexity-Driven Policy Optimization](https://arxiv.org/abs/2509.20509)
*Luca Serfilippi,Giorgio Franceschelli,Antonio Corradi,Mirco Musolesi*

Main category: cs.LG

TL;DR: 本文提出用复杂度奖励替代传统的熵奖励，通过结合香农熵和偏离均衡度来平衡随机性和结构性，从而引导智能体发现更有用的行为策略。


<details>
  <summary>Details</summary>
Motivation: 传统的熵最大化方法将策略推向均匀随机分布，这种探索策略缺乏结构且效率低下。需要一种更鲁棒的复杂度奖励来平衡探索和利用。

Method: 提出复杂度驱动策略优化（CDPO），用复杂度奖励替代PPO中的熵奖励。复杂度定义为香农熵和偏离均衡度的乘积，其中偏离均衡度量化了与均匀分布的距离。

Result: 在多个离散动作空间任务上的实验表明，CDPO对复杂度系数的选择比PPO对熵系数的选择更加鲁棒，特别是在需要更多探索的环境中表现更好。

Conclusion: 复杂度奖励能够有效平衡随机性和结构性，抑制极端情况，引导智能体发现结构化且适应性强的策略，在探索密集型环境中表现优异。
...
### [13] [A Recovery Theory for Diffusion Priors: Deterministic Analysis of the Implicit Prior Algorithm](https://arxiv.org/abs/2509.20511)
*Oscar Leong,Yann Traonmilin*

Main category: cs.LG

TL;DR: 本文提出了一个理论框架来分析基于确定性扩散模型的逆问题求解算法，将扩散先验算法解释为带有时变投影的广义投影梯度下降方法，并在满足限制等距性质的条件下推导出定量收敛率。


<details>
  <summary>Details</summary>
Motivation: 当前生成扩散模型在逆问题中表现出强大的数据驱动先验能力，但缺乏严格的理论恢复保证。本文旨在为基于扩散的逆问题算法建立理论分析框架。

Method: 研究确定性版本的Kadkhodaie & Simoncelli算法，将噪声卷积分数解释为对低维模型集的时变投影，将扩散先验算法建模为广义投影梯度下降。

Result: 当感知矩阵在模型集上满足限制等距性质时，可以推导出显式依赖于噪声调度方案的定量收敛率。在两个数据分布（低维紧凸集和低秩高斯混合模型）上验证了框架的有效性。

Conclusion: 该理论框架为基于扩散模型的逆问题求解提供了严格的理论基础，即使在非凸模型集上也能建立全局收敛保证。
...
### [14] [MDBench: Benchmarking Data-Driven Methods for Model Discovery](https://arxiv.org/abs/2509.20529)
...
<details>
  <summary>Details</summary>
Motivation: 当前模型发现方法的基准测试主要集中在识别单个方程（通常作为符号回归），缺乏对动态模型发现的全面基准测试，需要建立一个系统的评估框架。

Method: 开发MDBench基准测试框架，评估12种算法在14个PDE和63个ODE上的表现，考虑不同噪声水平，使用导数预测精度、模型复杂度和方程保真度等评估指标。

Result: 线性方法和遗传编程方法分别在PDE和ODE上获得最低预测误差，线性模型对噪声更具鲁棒性，同时发现了当前方法在流体动力学和热力学PDE系统上的关键局限性。

Conclusion: MDBench通过提供严谨、可扩展的基准测试框架和丰富的动态系统数据集，加速了模型发现方法的发展，支持系统评估、比较和改进方程的准确性和鲁棒性。
...
### [15] [Understanding and Improving Adversarial Robustness of Neural Probabilistic Circuits](https://arxiv.org/abs/2509.20549)
*Weixin Chen,Han Zhao*

Main category: cs.LG

TL;DR: 该论文分析了神经概率电路（NPC）的对抗鲁棒性，发现其仅依赖于属性识别模型的鲁棒性，并提出了RNPC方法通过类间集成来提高对抗攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: NPC模型虽然具有可解释性和高性能，但其基于神经网络的属性识别模型容易受到对抗攻击的影响，可能危及最终预测结果。

Method: 提出RNPC方法，采用新颖的类间集成推理机制，确保两个模块输出的鲁棒组合。

Result: 理论分析证明RNPC相比NPC具有可证明的对抗鲁棒性提升，在图像分类任务上的实验结果显示RNPC在保持良性输入高准确率的同时，实现了优于现有概念瓶颈模型的对抗鲁棒性。

Conclusion: RNPC是第一个针对识别模块对抗攻击的鲁棒神经概率电路，通过理论分析和实证验证了其有效性。
...
### [16] [Generalizable Diabetes Risk Stratification via Hybrid Machine Learning Models](https://arxiv.org/abs/2509.20565)
...
Method: 构建了两种混合分类器：XGBoost+随机森林和SVM+逻辑回归。使用泄漏安全的标准化流程（编码、插补、最小-最大缩放；仅在训练折叠上使用SMOTE；SVM概率校准），在主要数据集上拟合并冻结流程，然后在PIMA外部队列上进行验证。

Result: 在主要数据集上，XGB-RF（AUROC ~0.995，AUPRC ~0.998）显著优于SVM-LR（AUROC ~0.978，AUPRC ~0.947）。在外部验证集PIMA上，XGB-RF保持了强劲性能（AUROC ~0.990，AUPRC ~0.959），而SVM-LR性能较低（AUROC ~0.963，AUPRC ~0.875）。

Conclusion: XGB-RF在内部和外部队列中始终优于SVM-LR，在ROC/PR上表现出较小的外部衰减和可接受的校准。结果支持基于梯度提升的混合方法作为糖尿病风险分层的稳健、可转移方法。
...
### [17] [PIRF: Physics-Informed Reward Fine-Tuning for Diffusion Models](https://arxiv.org/abs/2509.20570)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在科学领域生成能力强，但经常违反物理定律。现有方法依赖DPS风格的价值函数近似，存在误差大、训练不稳定和推理效率低的问题。

Method: PIRF方法：1）层间截断反向传播利用物理奖励的时空局部性；2）基于权重的正则化方案比传统蒸馏方法更高效。

Result: 在五个PDE基准测试中，PIRF在高效采样机制下持续实现更优的物理约束执行效果。
...
### [18] [The Sensitivity of Variational Bayesian Neural Network Performance to Hyperparameters](https://arxiv.org/abs/2509.20574)
*Scott Koermer,Natalie Klein*

Main category: cs.LG

TL;DR: 本文对贝叶斯神经网络（BNNs）的超参数进行了全局敏感性分析，发现超参数之间存在复杂交互作用，影响预测准确性和不确定性量化（UQ）性能。建议使用全局敏感性分析或贝叶斯优化方法来改进BNNs在实际应用中的超参数选择。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯神经网络虽然能提供预测不确定性量化，但在实际应用中由于近似训练方法和超参数选择困难，难以获得准确的不确定性量化。超参数数量多且影响不透明，需要系统分析其影响。

Method: 采用全局敏感性分析方法，在不同超参数设置下评估BNNs的性能表现，分析超参数对预测准确性和不确定性量化的影响。

Result: 研究发现BNNs的多个超参数之间存在交互作用，共同影响模型的预测准确性和不确定性量化性能。

Conclusion: 为了在实际应用中更好地使用BNNs，建议采用全局敏感性分析或贝叶斯优化等方法来辅助超参数选择和维度缩减，以确保获得准确的不确定性量化。
...
### [19] [Learning Greens Operators through Hierarchical Neural Networks Inspired by the Fast Multipole Method](https://arxiv.org/abs/2509.20591)
...
<details>
  <summary>Details</summary>
Motivation: 尽管FMM在物理和工程中广泛应用，但其与现代机器学习架构的结合仍未被充分探索。该研究旨在将FMM的高效数值算法优势引入机器学习领域。

Method: 利用FMM方法的分层计算流，将局部和远场相互作用分离，并高效学习它们各自的表示。通过多极展开格林函数来构建神经网络架构。

Result: 开发了Neural FMM架构，能够有效处理N体问题中的长程力计算，特别适用于学习椭圆PDE的格林算子。

Conclusion: 该工作成功地将FMM的数值算法优势与神经网络相结合，为物理启发的机器学习架构提供了新的思路。
...
### [20] [TSKAN: Interpretable Machine Learning for QoE modeling over Time Series Data](https://arxiv.org/abs/2509.20595)
*Kamal Singh,Priyanka Rawat,Sami Marouani,Baptiste Jeudy*

Main category: cs.LG

TL;DR: 提出了一种使用可解释机器学习技术对视频流QoE建模的新方法，结合Kolmogorov-Arnold Networks在频域特征上实现透明可解释的预测


<details>
  <summary>Details</summary>
Motivation: 传统黑盒方法无法捕捉特征与用户体验的复杂关系，需要开发既能保持高精度又能提供透明解释的QoE建模方法

Method: 在原始时间序列数据上使用可解释ML技术，将KANs作为可解释读取层应用于紧凑频域特征，以捕捉时序信息同时保持模型透明度
...
### [21] [Explicit and Effectively Symmetric Schemes for Neural SDEs](https://arxiv.org/abs/2509.20599)
*Daniil Shmelev,Cristopher Salvi*

Main category: cs.LG

TL;DR: 提出了一种新型稳定、近似可逆的Runge-Kutta方法（EES方案），用于解决神经随机微分方程求解器在反向传播中的内存效率和梯度精度问题。


<details>
  <summary>Details</summary>
Motivation: 传统神经SDE求解器的反向传播方法存在两难选择：离散化后优化方法内存成本高，优化后离散化方法梯度近似误差大且计算慢。现有可逆求解器虽然承诺内存效率和梯度精度，但在复杂模型和大步长下不稳定。

Method: 开发了显式且有效对称（EES）的Runge-Kutta方案，这些方案保持了可逆求解器的优点，同时克服了其不稳定性，能够在不受步长或模型复杂度严重限制的情况下实现内存高效训练。

Result: 通过数值实验证明，EES方案在稳定性和可靠性方面表现优越，能够为神经SDE的可扩展和准确训练提供实用基础。

Conclusion: EES方案成功解决了神经SDE训练中的内存效率与梯度精度之间的权衡问题，为复杂模型和大步长条件下的稳定训练提供了有效解决方案。
...
### [22] [Function Spaces Without Kernels: Learning Compact Hilbert Space Representations](https://arxiv.org/abs/2509.20605)
*Su Ann Low,Quentin Rommel,Kevin S. Miller,Adam J. Thorpe,Ufuk Topcu*

Main category: cs.LG

TL;DR: 本文提出函数编码器技术，通过神经网络基函数学习希尔伯特空间的紧凑表示，建立了与特征学习和核方法的理论联系，并开发了两种训练算法来学习紧凑基函数。


<details>
  <summary>Details</summary>
Motivation: 函数编码器能够创建自适应、紧凑的函数表示，但缺乏与核方法的理论联系和可扩展的训练方法。本文旨在建立这种联系并提供有效的训练算法。

Method: 提出两种训练算法：渐进式训练方法逐步构建基函数，以及训练后剪枝方法提供计算效率高的替代方案。两种方法都利用PCA原理揭示学习空间的内在维度。

Result: 在多项式基准测试和非线性动态系统（Van der Pol振荡器和双体轨道模型）上的验证表明，使用显著更少的基函数可以达到相同的精度。

Conclusion: 这项工作为实现具有核级保证的神经预测器铺平了道路，使模型既高效又具有理论保证，能够适应大规模应用。
...
### [23] [MMG: Mutual Information Estimation via the MMSE Gap in Diffusion](https://arxiv.org/abs/2509.20609)
*Longxuan Yu,Xing Shi,Xianghao Kong,Tong Jia,Greg Ver Steeg*

Main category: cs.LG

TL;DR: 本文提出了一种利用去噪扩散模型估计互信息的新方法，通过信息论框架将互信息与条件和无条件扩散的最小均方误差差距联系起来
...
Method: 基于去噪扩散模型的信息论公式，将互信息表示为条件与无条件扩散的最小均方误差差距的一半，在噪声过程的所有信噪比上进行积分，并采用自适应重要性采样实现可扩展估计

Result: 该方法通过了自一致性测试，在性能上超越了传统和基于分数的扩散互信息估计器，即使在高互信息情况下仍保持强劲表现

Conclusion: 扩散模型为互信息估计提供了一种直接有效的方法，通过信息论框架实现了可扩展且高性能的互信息估计
...
### [24] [Policy Compatible Skill Incremental Learning via Lazy Learning Interface](https://arxiv.org/abs/2509.20612)
*Daehee Lee,Dongsu Lee,TaeYoon Kwack,Wonje Choi,Honguk Woo*

Main category: cs.LG

TL;DR: SIL-C是一个新颖的框架，通过双边懒惰学习映射技术确保技能与策略的兼容性，使得增量学习的技能改进能够直接提升下游策略性能而无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 随着技能库的演化，现有基于技能的策略可能会失去兼容性，限制了技能的重用性和泛化能力。

Method: 采用双边懒惰学习映射技术，动态对齐策略引用的子任务空间与技能解码为智能体行为空间，基于轨迹分布相似性为每个子任务选择合适的技能。

Result: 在多种SIL场景下评估表明，SIL-C能够在学习过程中保持演化技能与下游策略的兼容性，同时确保效率。

Conclusion: SIL-C框架有效解决了技能增量学习中技能-策略兼容性问题，为构建可重用和可泛化的分层策略提供了可行方案。
...
### [25] [Latent Twins](https://arxiv.org/abs/2509.20615)
*Matthias Chung,Deepanshu Verma,Max Collins,Amit N. Subrahmanya,Varuni Katti Sastry,Vishwas Rao*

Main category: cs.LG

TL;DR: Latent Twins提出了一种统一的数学框架，通过在潜在空间中创建隐藏代理来统一表示学习和算法求解方法，将经典建模、反演、模型约简和算子逼近统一为单一原理。


<details>
  <summary>Details</summary>
Motivation: 科学机器学习在复杂系统分析、建模和预测方面取得了进展，但表示学习和算法求解方法通常并行发展，缺乏统一框架。Latent Twins旨在弥合这一差距。

Method: 创建潜在空间中的隐藏代理，用学习到的算子来支配数学系统。该方法通过潜在空间中的算子来统一处理ODE和PDE问题。

Result: 在三个代表性场景中验证了框架：典型ODE系统、浅水方程PDE基准测试以及真实地理位势再分析数据集。结果显示Latent Twins能够提供紧凑、可解释的解决方案算子代理。

Conclusion: Latent Twins提供了一个可扩展的、理论基础的代理框架，弥合了数据驱动表示学习和经典科学建模之间的差距，与科学管道（如同化、控制和不确定性量化）兼容。
...
### [26] [Training Task Reasoning LLM Agents for Multi-turn Task Planning via Single-turn Reinforcement Learning](https://arxiv.org/abs/2509.20616)
*Hanjiang Hu,Changliu Liu,Na Li,Yebin Wang*

Main category: cs.LG

TL;DR: 该论文提出了一种将多轮任务规划转化为单轮任务推理问题的新方法，通过GRPO算法实现高效策略优化，在复杂任务规划基准上取得了优于更大基线模型的性能。


<details>
  <summary>Details</summary>
Motivation: 训练LLM代理进行复杂多轮任务规划面临稀疏奖励、长期信用分配和计算开销等挑战，需要更高效的优化方法。

Method: 使用Group Relative Policy Optimization (GRPO)算法，将多轮任务规划转化为单轮任务推理问题，利用专家轨迹提供密集可验证的奖励。

Result: 1.5B参数的模型在超过30步的长周期规划任务中达到70%的成功率，优于14B参数的基线模型。
...
### [27] [Personalized Federated Dictionary Learning for Modeling Heterogeneity in Multi-site fMRI Data](https://arxiv.org/abs/2509.20627)
*Yipu Zhang,Chengshuo Zhang,Ziyu Zhou,Gang Qu,Hao Zheng,Yuping Wang,Hui Shen,Hongwen Deng*

Main category: cs.LG

TL;DR: 提出PFedDL框架解决多站点fMRI数据隐私和异构性挑战，通过联邦字典学习实现跨站点协作建模，在ABIDE数据集上验证优于现有方法


<details>
  <summary>Details</summary>
Motivation: 多站点fMRI研究中数据隐私约束和站点特异性异质性导致非IID数据，阻碍通用模型的开发

Method: PFedDL框架在每个站点独立进行字典学习，将站点特定字典分解为共享全局组件和个性化本地组件，通过联邦聚合更新全局原子，独立优化本地原子

Result: 在ABIDE数据集上的实验表明，PFedDL在非IID数据集上的准确性和鲁棒性优于现有方法

Conclusion: PFedDL有效解决了多站点fMRI分析中的数据隐私和异构性问题，为跨站点协作建模提供了可行方案
...
### [28] [Investigating Modality Contribution in Audio LLMs for Music](https://arxiv.org/abs/2509.20641)
...
### [29] [Wonder Wins Ways: Curiosity-Driven Exploration through Multi-Agent Contextual Calibration](https://arxiv.org/abs/2509.20648)
...
Conclusion: CERMIC通过理论驱动的方式生成内在奖励，鼓励智能体探索具有高信息增益的状态转换，为多智能体探索提供了有效的解决方案
...
### [30] [Guiding Application Users via Estimation of Computational Resources for Massively Parallel Chemistry Computations](https://arxiv.org/abs/2509.20667)
*Tanzila Tabassum,Omer Subasi,Ajay Panyala,Epiya Ebiapia,Gerald Baumgartner,Erdal Mutlu,P.,Sadayappan,Karol Kowalski*

Main category: cs.LG

TL;DR: 本文开发了基于机器学习的策略来预测大规模并行化学计算所需的资源成本，以指导用户在超级计算机上运行昂贵实验之前做出决策。


<details>
  <summary>Details</summary>
Motivation: 为了解决用户在运行昂贵的化学计算（如耦合簇方法）前需要预测资源需求的问题，帮助用户确定最优的运行参数配置，包括节点数和瓦片大小。

Method: 使用梯度提升（GB）机器学习模型和主动学习策略，基于在DOE Frontier和Aurora超级计算机上运行的CCSD应用程序的运行时参数值集合进行训练和评估。

Result: 在预测CCSD迭代的总执行时间时，梯度提升模型在Aurora和Frontier上的平均绝对百分比误差（MAPE）分别为0.023和0.073。主动学习策略仅需约450个实验数据点即可达到约0.2的MAPE。

Conclusion: 机器学习方法能够有效预测化学计算的资源需求，帮助用户优化参数配置，在最短时间或最低成本之间做出权衡决策。
...
### [31] [Theoretical Bounds for Stable In-Context Learning](https://arxiv.org/abs/2509.20677)
...
<details>
  <summary>Details</summary>
Motivation: 上下文学习(ICL)虽然灵活，但其可靠性对提示长度高度敏感，需要理论指导来确保稳定性。

Method: 基于固定高维亚高斯表示下的协方差谱性质分析，建立ICL稳定性的最小演示数量下界，并提出两阶段可观测估计器进行单次校准。
...
### [32] [Bispectral OT: Dataset Comparison using Symmetry-Aware Optimal Transport](https://arxiv.org/abs/2509.20678)
*Annabel Ma,Kaiying Hou,David Alvarez-Melis,Melanie Weber*

Main category: cs.LG

TL;DR: 本文提出了Bispectral Optimal Transport（双谱最优传输），一种对称感知的离散OT扩展方法，通过使用双谱表示来比较元素，在保持信号结构的同时去除群作用带来的变化。


<details>
  <summary>Details</summary>
Motivation: 在对称性丰富的场景中，仅基于原始特征间几何距离的OT对齐可能忽略数据的内在相干结构。传统OT方法无法有效处理对称变换带来的冗余变化。

Method: 引入双谱最优传输，使用双谱（一种群傅里叶不变量）来表示元素，该表示能保留所有信号结构同时去除群作用引起的变异。

Result: 实验表明，在经视觉对称变换的基准数据集上，Bispectral OT计算的传输计划比原始特征OT具有更高的类别保持准确率。
...
### [33] [Can Federated Learning Safeguard Private Data in LLM Training? Vulnerabilities, Attacks, and Defense Evaluation](https://arxiv.org/abs/2509.20680)
...
<details>
  <summary>Details</summary>
Motivation: 虽然联邦学习被认为是隐私保护的协作训练框架，但现有研究假设全局模型只包含泛化知识从而保护隐私。本文旨在验证在LLM微调场景下，FL是否真的能有效保护客户端数据隐私。

Method: 通过大量实验验证数据泄露风险，使用简单的生成方法从全局模型中提取训练数据，并设计了针对FL的增强攻击策略（跟踪训练过程中的全局模型更新）。同时评估了差分隐私、正则化约束更新和安全对齐LLM等隐私保护技术。

Result: 实验表明攻击者仍能从FL训练的全局模型中提取训练数据，数据泄露程度随模型规模增大而增加。增强攻击策略能进一步加剧隐私泄露。隐私保护技术能在一定程度上缓解风险但效果有限。

Conclusion: 联邦学习在LLM微调场景下并不能完全保证数据隐私安全，需要结合多种隐私保护技术来降低风险。研究结果为FL训练LLM时的隐私保护提供了实践指导。
...
### [34] [Learning to Align Molecules and Proteins: A Geometry-Aware Approach to Binding Affinity](https://arxiv.org/abs/2509.20693)
*Mohammadsaleh Refahi,Bahrad A. Sokhansanj,James R. Brown,Gail Rosen*

Main category: cs.LG

TL;DR: FIRM-DTI是一个轻量级药物-靶点结合亲和力预测框架，通过FiLM层实现分子嵌入的条件化，并使用三元组损失强化度量结构，在DTI-DG基准测试中达到最先进性能
...
Method: 使用特征级线性调制(FiLM)层将分子嵌入条件化于蛋白质嵌入，通过三元组损失强化度量结构，采用RBF回归头基于嵌入距离生成平滑可解释的亲和力预测
...
### [35] [CE-GPPO: Controlling Entropy via Gradient-Preserving Clipping Policy Optimization in Reinforcement Learning](https://arxiv.org/abs/2509.20712)
*Zhenpeng Su,Leiyu Pan,Minxuan Lv,Yuntao Li,Wenping Hu,Fuzheng Zhang,Kun Gai,Guorui Zhou*

Main category: cs.LG

TL;DR: 提出CE-GPPO算法，通过保留被PPO裁剪的低概率token的梯度信号来更好地控制策略熵，在数学推理任务上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有PPO及其变体在训练过程中会丢弃低概率token的梯度信号，但这些信号对调节熵动态具有关键作用，需要一种能够温和保留这些梯度的方法

Method: CE-GPPO算法在标准PPO基础上，以有界方式重新引入被裁剪token的梯度，通过控制裁剪区间外token的梯度大小来实现探索-利用平衡

Result: 理论分析和实验证明CE-GPPO能有效缓解熵不稳定性，在数学推理基准测试中在不同模型规模下均优于强基线方法

Conclusion: CE-GPPO通过梯度保留机制改进了PPO的熵控制能力，为强化学习优化大语言模型提供了更有效的方法
...
### [36] [A Genetic Algorithm for Navigating Synthesizable Molecular Spaces](https://arxiv.org/abs/2509.20719)
*Alston Lo,Connor W. Coley,Wojciech Matusik*

Main category: cs.LG

TL;DR: SynGA是一种基于遗传算法的分子设计方法，直接在合成路径上操作，通过定制交叉和变异算子确保分子的可合成性，并可与机器学习过滤器结合提升性能。


<details>
  <summary>Details</summary>
Motivation: 受遗传算法有效性和分子设计中可合成性重要性的启发，开发一种能够直接在可合成分子空间中进行操作的方法。

Method: 使用遗传算法，定制交叉和变异算子约束在可合成分子空间，通过修改适应度函数适应不同设计任务，并与机器学习过滤器结合形成SynGBO变体。
...
### [37] [Scaling Laws are Redundancy Laws](https://arxiv.org/abs/2509.20721)
*Yuda Bi,Vince D Calhoun*

Main category: cs.LG

TL;DR: 本文通过将缩放定律解释为冗余定律，首次为深度学习中的缩放现象提供了严格的数学解释，揭示了缩放指数取决于数据冗余而非普遍性。


<details>
  <summary>Details</summary>
Motivation: 深度学习中的缩放定律虽然普遍存在，但其数学起源特别是缩放指数一直缺乏理论解释。本文旨在从数学上解释缩放定律的本质。

Method: 使用核回归方法，分析数据协方差谱的多项式尾部，推导出超额风险的幂律表达式，并验证该定律在多种场景下的普适性。

Result: 发现缩放指数α=2s/(2s+1/β)，其中β控制谱尾，1/β衡量冗余度。学习曲线的斜率取决于数据冗余，而非普遍常数。

Conclusion: 缩放定律本质上是有限样本冗余定律，这一发现统一了经验观察与理论基础，为理解深度学习的缩放行为提供了新的数学框架。
...
### [38] [The Impact of Audio Watermarking on Audio Anti-Spoofing Countermeasures](https://arxiv.org/abs/2509.20736)
...
Conclusion: 音频水印是一个被忽视的域偏移因素，本研究建立了首个水印弹性反欺骗系统的基准，相关协议已公开。
...
### [39] [Measuring LLM Sensitivity in Transformer-based Tabular Data Synthesis](https://arxiv.org/abs/2509.20768)
...
### [40] [Sig2Model: A Boosting-Driven Model for Updatable Learned Indexes](https://arxiv.org/abs/2509.20781)
...
<details>
  <summary>Details</summary>
Motivation: 传统学习索引在静态数据集上效率很高，但在动态更新时性能下降严重，因为维护CDF不变性需要全局模型重训练，这会阻塞查询并限制QPS。现有方法未能有效解决重训练成本问题。
...
Conclusion: Sig2Model通过创新的技术组合有效解决了学习索引在动态更新环境中的性能瓶颈，为实际工作负载提供了可行的解决方案。
...
TL;DR: 提出了一种结合MLP和CNN的时间序列预测模型，通过MLP捕捉长期趋势依赖，CNN处理局部季节性模式和残差，并引入IConv架构独立处理通道依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据存在非平稳性，现有MLP模型在线性特性下难以处理不同通道的分布多样性，会忽略局部变化模式如季节性模式和残差成分。

Method: 将MLP与CNN结合：MLP建模整体趋势以考虑长期依赖；CNN使用多样化核函数建模细粒度局部模式；提出IConv架构独立处理时间依赖通道，通过不同层考虑通道间关系。

Result: 在时间序列数据集上的广泛实验表明，该方法在多元时间序列预测方面具有优越性。

Conclusion: 结合MLP和CNN的方法能有效处理时间序列的非平稳性，IConv架构通过独立通道处理和大核尺寸实现了对局部变化的有效建模。
...
<details>
  <summary>Details</summary>
Motivation: 解决在噪声标签和数据异质性存在的情况下训练深度神经网络的挑战，提高模型在噪声环境下的泛化能力和鲁棒性。

Method: 使用三个可学习参数动态调整每个训练样本的损失权重，基于样本难度进行分类，在每个训练小批量后使用验证集进行单步梯度下降更新权重。

Result: 在多个通用和医学影像数据集、不同噪声水平和类型、损失函数及架构上的实验表明，LiLAW能持续提升性能，即使在高噪声环境下也有效。

Conclusion: LiLAW提供了一种计算高效的解决方案，可在任何神经网络训练设置中提升模型的泛化能力和鲁棒性，不依赖数据增强或高级正则化技术。
...
TL;DR: 本文提出了任务依赖初始化（TDI）框架，通过功率谱匹配方法使状态空间模型（SSMs）的归纳偏置与任务的光谱特性对齐，从而显著提高模型的泛化能力和样本效率。


<details>
  <summary>Details</summary>
Motivation: 大规模模型的成功依赖于缩放定律，但高质量数据的有限性带来了挑战。状态空间模型使用固定的归纳偏置，当任务结构与模型偏置不匹配时会导致样本效率低下。

Method: 首先形式化线性时不变SSMs的归纳偏置，提出TDI方法：通过功率谱匹配在训练前将模型的光谱响应与任务的光谱特征对齐。
...
TL;DR: 本文提出了FERD框架，通过调整对抗样本的比例和分布来解决数据自由鲁棒性蒸馏中的鲁棒公平性问题，提升最差类别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有数据自由鲁棒性蒸馏方法忽视了鲁棒公平性问题，导致不同类别间的鲁棒性存在严重差异。
...
Conclusion: FERD在鲁棒性和公平性方面都表现出优越性能，是首个增强公平性的数据自由鲁棒性蒸馏框架。
...
TL;DR: T2I-Diff是一个基于时间频率表示和去噪扩散的fMRI生成框架，通过将BOLD信号转换为时频谱图并利用无分类器扩散模型生成高质量fMRI数据，解决了传统生成模型在捕捉BOLD信号非平稳性和非线性动态方面的不足。


<details>
  <summary>Details</summary>
Motivation: fMRI数据采集资源密集，高质量样本稀缺限制了数据驱动脑分析模型的发展。现有生成模型因忽略BOLD信号的复杂非平稳性和非线性动态而性能不佳。

Method: 1. 通过时间相关傅里叶变换将BOLD信号转换为窗口化时频谱图；2. 训练无分类器扩散模型生成类条件频率谱图；3. 通过逆傅里叶变换将谱图还原为BOLD信号。

Result: 该方法在下游fMRI脑网络分类任务中表现出更高的准确性和更好的泛化能力。

Conclusion: T2I-Diff框架有效解决了fMRI数据生成中的挑战，为数据驱动的脑分析提供了更可靠的合成数据源。
...
TL;DR: CaTS-Bench是第一个大规模、真实世界的情境感知时间序列字幕基准，包含465k训练和105k测试时间戳，结合数值序列、元数据、线图图像和字幕，并提供新的评估指标和基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列字幕基准通常依赖合成数据或过于简化的字幕，且忽略元数据和视觉表示，需要填补这一空白。

Method: 从11个多样化数据集构建字幕和问答任务，使用可扩展管道生成参考字幕（主要由oracle LLM生成并经过事实检查、人类不可区分性研究和多样性分析验证），并提供人类修订的579个测试字幕。

Result: 建立了CaTS-Bench基准，包含大规模真实世界数据，并提出新的定制评估指标，对领先的VLMs进行基准测试，揭示了其优势和局限。

Conclusion: CaTS-Bench及其字幕管道为时间序列分析与基础模型交叉领域的未来研究提供了可靠且可扩展的基础。
...
TL;DR: 本文通过神经坍缩的视角，为深度神经网络训练中的后期现象（如grokking和信息瓶颈）提供了统一解释，揭示了类内方差收缩是这些现象的关键机制。
...
Method: 通过分析神经坍缩的动态过程，研究类内方差收缩与训练集拟合的时间尺度差异，并在多个数据集和架构上进行验证。

Result: 发现类内方差收缩是grokking和信息瓶颈现象的共同关键因素，不同时间尺度解释了这些后期现象的行为。

Conclusion: 神经坍缩为理解深度神经网络训练中的后期现象提供了统一的理论框架，揭示了类内方差收缩的核心作用。
...
Method: 提出两阶段训练框架：1）单模态训练阶段塑造初始状态；2）引入ECS量化模态竞争强度；3）开发FastPID算法进行部分信息分解，测量模态特异性、冗余性和协同性；4）异步控制器动态平衡模态并定位理想初始状态。
...
<details>
  <summary>Details</summary>
Motivation: 多组学数据能捕捉复杂的生物分子相互作用，但缺失模态会阻碍异构组学数据的整合分析，需要开发能够处理不完整数据的方法。
...
Conclusion: MOIRA方法具有生物学相关性，能够有效整合不完整的多组学数据，为疾病研究提供有力工具。
...
TL;DR: 本文提出了因果时间序列生成(CaTSG)这一新任务家族，通过因果视角扩展传统条件时间序列生成，支持干预和反事实生成。
...
Method: 开发了基于扩散模型的统一框架CaTSG，采用后门调整引导，通过因果得分函数和后门调整-行动-预测过程实现因果采样。

Result: 在合成和真实数据集上的实验表明，CaTSG在保真度方面表现优异，并能处理现有基线无法支持的干预和反事实生成。

Conclusion: 提出了因果时间序列生成任务家族，并通过CaTSG提供了概念验证，为更可靠的干预下模拟和反事实生成开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 约10%的新生儿需要呼吸辅助，5%需要通气支持。可穿戴FHR监测设备在孕妇运动时易出现信号丢失，传统插值方法无法保持信号频谱特性，限制了AI分析的准确性

Method: 使用掩码变换器自编码器方法，通过捕捉数据的空间和频率成分来重建缺失的FHR信号，支持信号修复和预测

Result: 该方法在不同缺失时长下表现出鲁棒性，可用于信号修复和预测，可应用于回顾性研究数据集

Conclusion: 该方法可集成到可穿戴FHR监测设备中，实现更早、更稳健的风险检测，支持AI风险评估算法开发
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在电子健康记录应用中面临数据缺失挑战，特别是当不同医疗机构收集时间序列数据的时态粒度不一致时
...
TL;DR: StyleBench是一个系统性评估LLM推理风格的基准测试，分析了5种推理风格在15个开源模型上的表现，发现没有单一最优策略，效果取决于模型规模和任务类型。
...
Method: 构建StyleBench基准，评估5种推理风格（CoT、ToT、AoT、SoT、CoD）在5个推理任务上的表现，使用15个开源模型（270M-120B参数）。

Result: 搜索类方法（AoT、ToT）在开放性问题中表现优异但需要大规模模型，简洁风格（SoT、CoD）在定义明确任务上效率更高；小模型常无法遵循输出指令而猜测，推理鲁棒性随规模提升。
...
TL;DR: 该论文提出了一种处理随机传感器延迟的模型强化学习框架，通过基于模型的滤波过程来更新信念状态，使智能体能够有效应对观察延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现实环境中经常出现延迟，但标准强化学习算法通常假设环境感知是瞬时的。论文研究了POMDP中的随机传感器延迟问题，这是RL中尚未解决的新设置。

Method: 提出了一个基于模型的滤波过程，顺序更新基于观察流的信念状态，并引入了一个简单的延迟感知框架，将其整合到基于模型的RL中。将该框架应用于Dreamer算法。

Result: 该方法在延迟感知基准测试中表现优于现有方法，并在部署过程中对延迟分布变化表现出鲁棒性。在模拟机器人任务上的实验也验证了其有效性。

Conclusion: 明确建模观察延迟对于处理现实世界延迟问题至关重要，提出的框架为随机传感器延迟提供了一种有效的解决方案。
...
TL;DR: 本文提出了一种联邦学习中的客户端选择方法，通过控制标签分布来缓解数据不平衡问题，实验证明该方法在局部和全局不平衡场景下均能有效提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据不平衡会降低共享模型的性能，现有方法主要通过客户端选择来缓解这一问题，但缺乏对标签分布的有效控制。

Method: 提出分布控制的客户端选择方法，选择能够使当前标签分布与目标分布（平衡分布或联邦组合标签分布）最佳对齐的活跃客户端。

Result: 在三种常见FL策略和两个数据集上的实验表明，面对局部不平衡时，与平衡分布对齐效果最佳；面对全局不平衡时，与联邦组合标签分布对齐更优。

Conclusion: 分布控制的客户端选择方法能有效提升联邦学习在数据不平衡场景下的性能，不同目标分布适用于不同类型的不平衡问题。
...
TL;DR: 提出了一种基于联邦学习的注意力增强LSTM模型，用于脓毒症发病预测，支持可变预测窗口，在保护患者隐私的同时提高了早期检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决ICU中脓毒症早期准确预测的挑战，同时克服单个医院数据量有限的问题，通过联邦学习实现多中心协作训练而不共享数据。

Method: 使用联邦学习框架下的注意力增强LSTM模型，支持可变预测窗口，能够同时进行短期和长期预测。

Result: 联邦学习不仅提高了整体预测性能（接近集中式模型），而且特别有利于脓毒症的早期预测，可变预测窗口显著降低了计算和通信开销。

Conclusion: 该方法在保护隐私的前提下有效提升了脓毒症预测性能，特别是早期检测能力，同时通过可变预测窗口优化了资源利用。
...
TL;DR: 提出基于马尔可夫链的离散状态扩散模型确定性去噪算法，通过引入弱混沌动力学的herding算法变体实现确定性离散状态转换，无需重新训练或连续状态嵌入


<details>
  <summary>Details</summary>
Motivation: 为离散扩散模型开发确定性反向过程，替代随机去噪过程，提高生成效率和质量

Method: 使用带有弱混沌动力学的herding算法变体来去随机化生成反向过程，实现确定性离散状态转换

Result: 在文本和图像生成任务中实现了效率和样本质量的一致提升

Conclusion: 确定性反向过程在离散状态空间中同样有效，这一简单去随机化方法有望增强离散扩散在生成建模中的重要性
...
<details>
  <summary>Details</summary>
Motivation: 探索在精细时空分辨率下，结合微观移动特征与历史犯罪、社会人口数据是否能提升犯罪预测的准确性

Method: 使用ConvLSTM网络，结合14天和2天的输入序列，预测12小时后的犯罪发生，并与逻辑回归、随机森林和标准LSTM基线模型比较

Result: 结合移动特征和社会人口特征时预测性能最佳，深度学习模型在四个城市均取得最高召回率、精确率和F1分数，长序列对暴力犯罪预测更有效，短序列对财产犯罪更有效

Conclusion: 整合多样化数据源对时空犯罪预测很重要，深度学习在精细时空尺度上具有优势但也有局限性
...
### [59] [Energy saving in off-road vehicles using leakage compensation technique](https://arxiv.org/abs/2509.20926)
*Gyan Wrat,J. Das*

Main category: cs.LG

TL;DR: 本文比较了两种液压回路在重型土方设备线性执行器中的能效表现，发现采用比例流量控制阀（PFCV）的创新型方案比传统比例方向控制阀（PDCV）能效提高8.5%。


<details>
  <summary>Details</summary>
Motivation: 提高重型土方设备（特别是挖掘设备吊臂）中线性执行器的能效，减少能量损失和环境影响。

Method: 使用MATLAB/Simulink进行液压回路仿真，比较传统PDCV方案与创新型PFCV方案。PFCV通过在执行器两端之间设置人工泄漏来减少热量形式的能量损失。位置控制采用模糊控制器调谐的PID控制器。

Result: PFCV液压回路比传统PDCV回路能效提高8.5%，通过旁路泵的多余流量减少能量损失。
...
### [60] [GenFacts-Generative Counterfactual Explanations for Multi-Variate Time Series](https://arxiv.org/abs/2509.20936)
*Sarah Seifi,Anass Ibrahimi,Tobias Sukianto,Cecilia Carbonelli,Lorenzo Servadei,Robert Wille*

Main category: cs.LG

TL;DR: GenFacts是一个基于类别判别变分自编码器的生成框架，用于为多元时间序列生成反事实解释，通过整合对比学习和分类一致性目标、基于原型的初始化以及真实性约束优化，显著提升了反事实的合理性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法为多元时间序列生成的反事实解释往往无效、不合理或不直观，需要一种能够产生更合理、更直观反事实的新方法。

Method: 基于类别判别变分自编码器的生成框架，整合了对比学习和分类一致性目标、原型初始化以及真实性约束优化。

Result: 在雷达手势数据和手写字母轨迹数据集上，GenFacts在合理性方面优于现有基线方法18.7%，并在人类研究中获得最高可解释性评分。
...
### [61] [Why Attention Fails: The Degeneration of Transformers into MLPs in Time Series Forecasting](https://arxiv.org/abs/2509.20942)
*Zida Liang,Jiayi Zhu,Weiqiang Sun*

Main category: cs.LG

TL;DR: 本文研究发现，在时间序列预测中，Transformer架构表现不佳甚至不如简单线性模型，主要原因是注意力机制失效，Transformer块退化为简单MLP。通过实验和理论分析发现，当前嵌入方法无法让Transformer在良好结构的潜在空间中工作。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer在时间序列预测中表现不佳的根本原因，现有研究未能深入分析Transformer失效的具体机制。

Method: 设计了一系列渐进式实验，将Transformer逐步修改为MLP来研究注意力机制的影响；构建可解释数据集分析注意力机制失效原因；进行理论分析探讨嵌入方法的根本问题。

Result: 发现现有时间序列Transformer中的Transformer块经常退化为简单MLP；注意力机制没有按预期方式工作；当前嵌入方法无法为Transformer提供良好结构的潜在空间。

Conclusion: Transformer在时间序列预测中的失败主要源于注意力机制失效和嵌入方法不当，需要重新设计适合时间序列特性的嵌入方法才能发挥Transformer的优势。
...
### [62] [Decoupled-Value Attention for Prior-Data Fitted Networks: GP Inference for Physical Equations](https://arxiv.org/abs/2509.20950)
*Kaustubh Sharma,Simardeep Singh,Parikshit Pareek*

Main category: cs.LG

TL;DR: 本文提出了解耦值注意力（DVA）机制，用于改进先验数据拟合网络（PFNs）在高维回归任务中的性能，使其能够更有效地替代高斯过程进行快速物理系统建模。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer注意力在PFNs中处理高维回归任务效果有限，而高斯过程的特性表明函数空间完全由输入核决定，预测均值是训练目标的加权和。这启发了将相似性计算与标签传播解耦的设计思路。

Method: 提出DVA机制，仅从输入计算相似性，通过值传播标签，镜像高斯过程更新但保持无核特性。研究表明注意力规则而非架构本身是扩展PFNs的关键因素。

Result: 局部化注意力在不同维度设置下显著降低验证损失（5维和10维情况下减少50%以上）。CNN-based PFNs性能与Transformer-based相当。64维功率流方程近似平均绝对误差达1E-3量级，比精确GP推理快80倍以上。

Conclusion: DVA机制成功提升了PFNs在高维回归任务中的性能，证明了注意力规则设计的重要性，为快速物理系统建模提供了高效解决方案。
...
### [63] [Flow Matching in the Low-Noise Regime: Pathologies and a Contrastive Remedy](https://arxiv.org/abs/2509.20952)
*Weili Zeng,Yichao Yan*

Main category: cs.LG

TL;DR: 本文揭示了流匹配在低噪声区域存在不稳定性问题，并提出LCF混合训练方法来解决该问题


<details>
  <summary>Details</summary>
Motivation: 流匹配框架在低噪声区域存在基本不稳定性，当噪声水平接近零时，输入中的微小扰动会导致速度目标的大幅变化，造成学习问题的条件数发散

Method: 提出Local Contrastive Flow (LCF)混合训练协议，在小噪声水平下用对比特征对齐替代直接速度回归，在中等和高噪声水平下保留标准流匹配
...
### [64] [Alignment Unlocks Complementarity: A Framework for Multiview Circuit Representation Learning](https://arxiv.org/abs/2509.20968)
*Zhengyuan Shi,Jingxin Wang,Wentao Jiang,Chengyu Ma,Ziyang Zheng,Zhufei Chu,Weikang Qian,Qiang Xu*

Main category: cs.LG

TL;DR: MixGate是一个多视图布尔电路学习框架，通过功能对齐预训练解决不同图表示之间的结构异质性，使掩码建模能够有效利用互补信息


<details>
  <summary>Details</summary>
Motivation: 多视图布尔电路学习具有巨大潜力，但不同图表示（如AIG与XMG）之间的结构异质性阻碍了有效融合，特别是对自监督技术如掩码建模而言，跨视图上下文被视为噪声
...
Conclusion: 功能对齐是多视图自监督学习发挥威力的必要前提条件，MixGate框架成功解决了多视图布尔电路学习中的结构异质性问题
...
### [65] [Knowledgeable Language Models as Black-Box Optimizers for Personalized Medicine](https://arxiv.org/abs/2509.20975)
...
Method: LEON采用'基于提示的优化'方法，将LLM作为随机引擎提出治疗设计，无需任务特定微调，利用其上下文理解非结构化领域知识的能力。

Result: 在真实世界优化任务上的实验表明，LEON在提出个体化治疗方案方面优于传统方法和基于LLM的方法。

Conclusion: LEON提供了一种数学上严谨的方法来利用LLM作为黑盒优化器，结合领域知识为个性化医疗提供有效的治疗方案优化。
...
### [66] [CLUE: Conflict-guided Localization for LLM Unlearning Framework](https://arxiv.org/abs/2509.20977)
*Hang Chen,Jiaying Zhu,Xinyu Yang,Wenya Wang*

Main category: cs.LG

TL;DR: 本文提出了一种名为CLUE的新框架，通过电路发现技术识别LLM中负责遗忘和保留的神经元，并使用CNF形式化方法进行精确干预，解决了现有方法中神经元纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法将遗忘和保留神经元视为单一纠缠组，导致过度遗忘或目标知识擦除不完整的问题。需要更精确的神经元定位方法来区分不同功能。

Method: 使用电路发现技术识别遗忘和保留电路，将其转换为合取范式（CNF），通过可满足性解确定每个神经元应遗忘还是保留，并针对不同类别神经元提供定向微调策略。

Result: 大量实验表明，与现有定位方法相比，CLUE通过精确的神经元定位实现了更好的遗忘效果和保留效用。

Conclusion: CLUE框架通过冲突引导的定位方法有效解决了LLM遗忘中的神经元纠缠问题，为精确控制模型知识提供了新思路。
...
### [67] [FracAug: Fractional Augmentation boost Graph-level Anomaly Detection under Limited Supervision](https://arxiv.org/abs/2509.20978)
...
<details>
  <summary>Details</summary>
Motivation: 图级异常检测在药物发现等领域至关重要，但高标注成本和数据集不平衡限制了GNN的性能表现。

Method: FracAug通过学习图内语义并合成分数变体，采用加权距离感知边界损失来生成多样化的语义保持图，然后利用原始图和增强图的预测结果对未标注数据进行伪标注，迭代扩展训练集。

Result: 在12个真实数据集上对14种GNN进行的实验显示，FracAug将平均AUROC、AUPRC和F1分数分别提升了最高5.72%、7.23%和4.18%。
...
### [68] [Intercept Cancer: Cancer Pre-Screening with Large Scale Healthcare Foundation Models](https://arxiv.org/abs/2506.00209)
*Liwen Sun,Hao-Ren Yao,Gary Gao,Ophir Frieder,Chenyan Xiong*

Main category: cs.LG

TL;DR: CATCH-FM是一种基于医疗记录进行癌症早期筛查的方法，使用24亿参数的医疗基础模型，在3万患者回顾性评估中达到60%敏感性和99%特异性。


<details>
  <summary>Details</summary>
Motivation: 现有癌症筛查技术昂贵且侵入性强，全球普及度低，导致许多本可挽救的生命流失。需要开发基于历史医疗记录的非侵入性预筛查方法。

Method: 基于数百万电子健康记录建立医疗代码序列的扩展规律，预训练计算最优的基础模型（最高24亿参数），然后在临床医生策划的癌症风险预测队列上进行微调。

Result: 在3万患者评估中，CATCH-FM以60%敏感性和99%特异性显著优于基于特征的树模型以及通用和医疗大语言模型，在EHRSHOT胰腺癌风险预测排行榜上达到最先进水平。
...
TL;DR: LCR是一个实用的基于学习的GPU缓存框架，通过LARU算法增强LRU缓存策略，结合机器学习预测和在线误差估计，在推荐模型和大型语言模型中显著提升性能并保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代GPU推理中缓存效率是主要瓶颈，传统启发式策略如LRU在结构化访问模式下表现不佳，而现有学习方法存在预测不准确时性能急剧下降或设计保守收益有限的问题。

Method: 提出LCR框架，核心算法LARU在LRU基础上集成机器学习预测，通过在线误差估计动态适应预测准确性，准确预测时接近最优性能，不准确时优雅退化到接近LRU性能。

Result: 实验显示在DLRM和LLM场景中，LCR将吞吐量提升高达24.2%，P99 TTFT降低高达28.3%，即使在预测不佳时性能仍保持稳定。
...
TL;DR: 本文提出了一种在截断伊辛模型中基于单一样本估计逆温度参数β的高效方法，该方法适用于由k-SAT公式定义的截断集，并在几乎线性时间内实现一致性估计。


<details>
  <summary>Details</summary>
Motivation: 截断伊辛模型在统计物理和机器学习中有广泛应用，但现有方法大多需要多个样本或无法处理复杂截断约束。本文旨在解决单一样本下对逆温度参数的高效估计问题。

Method: 基于伪似然最大化方法，扩展了Daskalakis等人和Galanis等人的技术，针对截断伊辛模型的特殊结构设计估计器。算法运行时间接近O(n)，适用于有界度图和k-SAT定义的截断集。

Result: 提出的估计器在k ≳ log(d²k)Δ³条件下，能够实现O(Δ³/√n)的一致性估计精度，即估计误差随样本维度增加而衰减。

Conclusion: 该方法为截断伊辛模型的参数估计提供了有效的单样本解决方案，扩展了伪似然方法在复杂约束模型中的应用范围，具有重要的理论和实践意义。
...
TL;DR: 提出了一种新的二进制自编码器（BAE），通过在隐藏激活上施加最小熵约束来提升LLM特征解缠的稀疏性和独立性，解决了现有方法因缺乏全局稀疏性保证而产生密集特征的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单实例正则化（如L1归一化、top-k函数）来解缠LLM隐藏状态中的数值特征，但缺乏跨实例的全局稀疏性保证，导致大量密集特征，损害了特征的稀疏性和原子化程度。

Method: 提出二进制自编码器（BAE），通过步进函数将隐藏激活离散化为1位二进制值，并应用梯度估计实现反向传播，从而在批次级别上强制最小熵约束，促进特征独立性和稀疏性。

Result: BAE在特征集熵计算和特征解缠两个主要应用上表现优异：能够可靠估计二进制隐藏激活的熵来表征LLM推理动态；在特征提取方面避免密集特征，产生最多可解释特征，优于基线方法。

Conclusion: BAE作为一种特征提取器，通过最小熵约束有效解决了特征稀疏性问题，在LLM特征解缠和推理动态分析方面具有显著优势。
...
TL;DR: 本文提出了一种基于局部唯一标识符（Local-UID）的图神经网络方法，通过d跳邻域内的唯一着色方案，在保持表达力的同时提高整数线性规划问题的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 标准匿名图神经网络在整数线性规划问题上表达能力有限，而全局唯一标识符方法会引入虚假相关性，严重损害泛化性能。需要一种平衡表达力和泛化能力的新方法。

Method: 提出了基于d跳唯一着色的Local-UID方案，确保标识符仅在节点d跳邻域内唯一。基于此设计了ColorGNN（通过颜色条件嵌入整合颜色信息）和ColorUID（轻量级特征级变体）。

Result: 实验表明该方法在三个ILP基准测试上取得显著提升，在线性规划数据集上表现出强大的OOD泛化能力，与最先进方法结合时还能进一步提升图级任务的性能。

Conclusion: Local-UID方案在d层网络中实现了与Global-UID相同的表达能力，同时提供更强的泛化能力，为解决ILP优化问题提供了有效的学习优化框架。
...
TL;DR: 本文提出将无损压缩作为时间序列模型评估的新范式，基于香农信源编码定理，将最优压缩长度与负对数似然等价，为建模能力提供严格统一的信息论标准。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列模型评估主要关注四个典型任务（预测、插补、异常检测、分类），但这些任务主要评估特定任务性能，无法严格衡量模型是否捕捉数据的完整生成分布。

Method: 引入无损压缩作为评估范式，定义标准化评估协议和指标，开发开源评估框架TSCom-Bench，使时间序列模型能够快速适配为无损压缩的骨干网络。

Result: 在多个数据集上对TimeXer、iTransformer、PatchTST等先进模型的实验表明，压缩能够揭示传统基准测试忽略的分布弱点。
...
TL;DR: 提出MAIFormer多智能体倒置变换器架构，用于预测多飞机飞行轨迹，通过掩码多变量注意力捕捉个体时空模式，通过智能体注意力建模复杂空域中的交互模式，在真实数据集上表现最佳且具有可解释性。


<details>
  <summary>Details</summary>
Motivation: 多智能体飞行轨迹预测对理解当前空中交通流至关重要，但面临两大挑战：建模个体飞机行为随时间变化和复杂飞行间交互，以及生成可解释的预测结果。

Method: MAIFormer框架包含两个关键注意力模块：掩码多变量注意力（捕捉个体飞机的时空模式）和智能体注意力（建模复杂空域场景中多智能体的社交模式）。
...
TL;DR: ExMoIRL是一个新颖的生成框架，通过整合表型和靶点特异性线索，用于从头分子生成，克服了当前表型基和靶点基策略的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于表型和基于靶点的策略各有局限性，要么实验成本高，要么忽视系统级细胞反应。为了弥补这一差距，提出了ExMoIRL框架。

Method: 表型引导的生成器首先在广泛的药物诱导转录谱上进行预训练，然后通过多目标强化学习进行微调。奖励函数融合了对接亲和力和药物相似性评分，并增加了排序损失、先验似然正则化和熵最大化。

Result: 广泛的实验表明，ExMoIRL在多个明确表征的靶点上优于最先进的表型基和靶点基模型。生成的分子表现出良好的药物样性质、高靶点亲和力以及对癌细胞的抑制效力。
...
TL;DR: 本文通过信息移除的新视角研究上下文学习(ICL)的内在机制，发现零样本情况下语言模型生成非选择性表示，而少样本ICL通过选择性移除冗余信息来引导模型专注于目标任务。


<details>
  <summary>Details</summary>
Motivation: ICL作为一种新兴的少样本学习范式，其内部工作机制尚不清楚，作者希望通过信息移除的视角来揭示ICL的核心机制。

Method: 通过设计度量指标测量隐藏状态，发现ICL模拟任务导向的信息移除过程；识别出诱导移除操作的关键注意力头（去噪头），并通过阻断信息移除操作的消融实验验证机制。

Result: 零样本时LM生成非选择性表示导致准确率接近零；少样本ICL通过选择性信息移除显著提升性能；阻断去噪头的移除操作会显著降低ICL准确率。

Conclusion: 信息移除机制是ICL的关键机制，去噪头在选择性移除冗余信息中发挥关键作用，这为理解ICL的工作机制提供了新视角。
...
### [77] [DELTA-Code: How Does RL Unlock and Transfer New Programming Algorithms in LLMs?](https://arxiv.org/abs/2509.21016)
*Yiyou Sun,Yuhan Cao,Pohao Huang,Haoyue Bai,Hannaneh Hajishirzi,Nouha Dziri,Dawn Song*

Main category: cs.LG

TL;DR: DELTA-Code是一个用于评估LLMs学习新推理策略能力的基准测试，通过合成编程问题家族来探究模型的可学习性和可迁移性，发现了模型在强化学习训练中出现的顿悟现象。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs是否能够真正学习和泛化新的推理策略这一开放性问题，超越预训练或后训练中已编码的技能。

Method: 设计了DELTA-Code基准测试，包含模板化问题生成器，通过强化学习训练模型，探索了分阶段预热、经验回放、课程训练和验证循环等关键训练要素。

Result: 实验发现模型在强化学习训练中会出现顿悟相变现象，在家族内部和技能重组方面有显著提升，但在转化性案例中仍存在持续弱点。
...
### [78] [Predicting LLM Reasoning Performance with Small Proxy Model](https://arxiv.org/abs/2509.21013)
*Woosung Koh,Juyoung Suk,Sungjun Han,Se-Young Yun,Jay Shin*

Main category: cs.LG

TL;DR: rBridge方法通过让小模型（≤1B参数）更接近预训练目标和目标任务，有效预测大模型的推理能力，将数据集排名成本降低100倍以上。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型成本过高，需要利用小代理模型优化数据集，但推理能力只在较大模型规模（超过7B参数）才可靠出现，小模型难以预测大模型的推理表现。

Method: rBridge通过加权负对数似然与任务对齐，使用前沿模型的推理轨迹作为黄金标签，让小代理模型更接近预训练目标和目标任务。

Result: rBridge在1B到32B规模上实现了最强的相关性，在1B到7B规模上实现了预测关系的零样本迁移，数据集排名成本比最佳基线降低100倍以上。
...
TL;DR: E-CIT是一个基于集成学习的条件独立性测试框架，通过数据分区和p值聚合策略，显著降低了因果发现中条件独立性测试的计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统的基于约束的因果发现方法需要进行大量条件独立性测试，这些测试在样本量较大时计算成本极高，限制了实际应用。

Method: 提出E-CIT框架：将数据划分为多个子集，在每个子集上独立应用基础条件独立性测试，然后使用基于稳定分布特性的新方法聚合p值。

Result: 实验结果表明，E-CIT不仅显著降低了计算负担，而且在复杂测试场景下（特别是真实世界数据集）表现出竞争性甚至更好的性能。

Conclusion: E-CIT为条件独立性测试提供了一个通用、即插即用的解决方案，在保持理论一致性的同时实现了计算效率的线性提升。
...
Result: 在ScaleDiff-Math数据集上微调的模型相比原始数据集性能提升11.3%，在AIME'24等5个困难基准测试上平均准确率达到65.9%，超越了OpenThinker3等强模型。
...
TL;DR: 提出Actor-Critic without Actor (ACA)框架，消除显式演员网络，直接从噪声级评论家的梯度场生成动作，简化架构并保持性能


<details>
  <summary>Details</summary>
Motivation: 传统actor-critic方法依赖分离的演员和评论家网络，训练复杂且对架构决策敏感；扩散模型虽能表达多模态行为但带来额外设计负担

Method: ACA框架通过噪声级评论家的梯度场直接生成动作，无需显式演员网络，保持策略改进与评论家价值估计的紧密对齐

Result: 在标准在线RL基准测试中，ACA相比标准actor-critic和扩散方法获得更优的学习曲线和竞争性性能

Conclusion: ACA为在线RL提供了简单而强大的解决方案，结合了简洁性和表达性
...
TL;DR: 本文分析了视觉越狱攻击在多模态大语言模型中的局限性，发现现有攻击方法存在高锐度区域问题，导致跨模型迁移性差。作者提出了FORCE方法，通过纠正特征过度依赖问题来提升攻击的跨模型迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉越狱攻击虽然能有效攻击开源MLLMs，但在跨模型迁移到闭源MLLMs时效果不佳。作者旨在理解这一局限性背后的原因，并开发能够有效评估闭源MLLMs安全性的方法。

Method: 作者分析了越狱攻击的损失景观和特征表示，发现攻击倾向于位于高锐度区域。基于此提出了FORCE方法，通过引导攻击探索更广泛的可行区域，并根据语义内容重新调整频率特征的影响。

Result: 实验表明FORCE方法能够有效提升视觉越狱攻击的跨模型迁移性，为闭源MLLMs的红队评估提供了有效工具。

Conclusion: 通过消除对层特征和频谱特征的非泛化依赖，FORCE方法能够发现更平坦的可行区域，显著改善视觉越狱攻击的跨模型迁移能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管RL微调被证明能提升LLMs能力，但其对不同模型内部机制的具体影响机制尚不明确，需要系统性的分析。

Method: 使用边缘归因修补(EAP)技术对比分析多个模型家族在RL微调前后的内部激活差异，重点关注PPO、GRPO和DPO等不同方法。

Result: 在线RL训练导致两个稳健变化：(1)激活强度整体增加，更多内部通路被激活；(2)激活模式多样性提高，熵值增加且边缘分布更分散。DPO方法则显示出较弱或不一致的内部变化。

Conclusion: RL微调通过重塑信息流使其更具冗余性和灵活性来提升泛化能力，在线RL方法与基于偏好的方法在内部机制改变上存在显著差异。
...
TL;DR: 本文基于物理学中的最小作用量原理，推导出经典学习算法、强化学习中的贝尔曼最优方程以及生成模型中的Adam优化器，提出学习是在拉格朗日量中寻找平稳路径的过程。
...
Method: 基于最小作用量原理，从第一性原理（学习拉格朗日量）推导学习算法，认为学习是寻找拉格朗日量中的平稳路径。
...
TL;DR: 本文提出了几何问题中的指称表达理解任务，并构建了GeoRef基准数据集，通过监督微调和GRPO方法提升模型在几何图理解上的表现，验证了该任务对下游几何推理的价值。
...
Method: 构建GeoRef基准数据集，使用结构化几何形式语言生成合成训练数据，采用SFT和GRPO两种微调方法，并提出验证-再生机制纠正错误预测。

Result: GRPO方法显著优于SFT，验证-再生机制进一步提升准确性。即使最先进的MLLM模型在该任务上表现不佳，表明需要专门训练几何理解能力。

Conclusion: 几何指称表达理解是几何问题求解的基础能力，GeoRef数据集和训练方法有效提升了模型性能，并证明了对下游几何推理任务的迁移价值。
...
TL;DR: SPREAD是一个基于去噪扩散概率模型(DDPMs)的生成式多目标优化框架，能够高效计算Pareto最优解集，特别适用于大规模和计算昂贵的问题


<details>
  <summary>Details</summary>
Motivation: 开发高效的多目标优化方法来计算冲突目标之间的Pareto最优折衷解集仍然是一个关键挑战，特别是在大规模和计算昂贵的问题上

Method: SPREAD首先学习决策空间采样点的条件扩散过程，然后在每个反向扩散步骤中，通过自适应多梯度下降启发的更新进行快速收敛，并结合高斯RBF排斥项保持多样性来精炼候选解

Result: 在多目标优化基准测试（包括离线和基于贝叶斯代理的设置）上的实证结果表明，SPREAD在效率、可扩展性和Pareto前沿覆盖度方面匹配或超越了领先的基线方法
...
TL;DR: SATMC是一个新颖的图域自适应框架，通过图结构和属性转换来顺序对齐网络间的分布，解决了传统方法在处理图结构异构性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图域自适应方法主要关注在原始图结构上转换节点属性并对齐特征分布，但往往难以处理不同图域之间的结构异构性，导致分布对齐效果不佳。

Method: SATMC框架通过图结构和属性转换顺序对齐网络分布，引入私有域信息减少机制和经验Wasserstein距离来减轻私有信息负面影响并增强模型泛化能力。

Result: 在9对公开跨域数据集上的实验表明，SATMC在跨网络节点分类任务中优于现有最先进方法。理论证明SATMC能获得比现有方法更紧的误差界。

Conclusion: SATMC通过结构和属性双重转换有效解决了图域自适应中的结构异构性问题，在跨网络节点分类任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MLA注意力机制中，naive核在训练阶段计算效率高，而absorb核在解码阶段带宽利用率好，但两者各有局限。absorb实现的计算绑定特性阻碍了注意力计算中数据重用的性能优势，如共享前缀。

Method: 提出TyphoonMLA混合方法，对注意力计算的compute-bound部分应用naive公式以利用共享前缀，对非共享部分使用absorb公式以减少带宽需求。

Result: 在NPU和GPU上分别实现了高达3倍和3.24倍的注意力计算吞吐量提升，HBM大小仅增加3%的开销。

Conclusion: TyphoonMLA成功结合了两种核实现的优势，为MLA架构提供了高效的注意力计算解决方案。
...
Method: 生成具有持久语义社区的图家族，确保概念一致性同时精细控制结构属性（如同质性和度分布），支持受控分布偏移测试。

Result: 基准测试显示传导性能不能预测归纳泛化能力，对分布偏移的鲁棒性高度依赖于模型架构选择和初始图机制。
...
Result: VARL在不同环境和代理设置下的评估显示，它显著提高了样本效率，且没有引入显著的计算开销。

Conclusion: VARL使强化学习在现实世界环境中从零开始直接应用变得可行，是一个通用的在线强化学习框架。
...
TL;DR: EvoMail是一个自进化的认知代理框架，通过构建异质邮件图、认知图神经网络和对抗性自进化循环，实现强大的垃圾邮件和钓鱼攻击检测。


<details>
  <summary>Details</summary>
Motivation: 现代垃圾邮件和钓鱼攻击已超越传统关键词黑名单和简单启发式方法，采用多模态策略快速适应绕过过滤器，传统检测系统难以整合异质信号和持续适应。

Method: 构建统一异质邮件图融合文本内容、元数据和嵌入资源；使用LLM增强的认知图神经网络进行上下文感知推理；采用红队-蓝队对抗性自进化循环持续改进检测能力。

Result: 在真实数据集和合成对抗变体上的实验表明，EvoMail在检测准确性、适应性和可解释性方面均优于最先进的基线方法。
...
### [92] [LAVA: Explainability for Unsupervised Latent Embeddings](https://arxiv.org/abs/2509.21149)
*Ivan Stresec,Joana P. Gonçalves*

Main category: cs.LG

TL;DR: LAVA是一种后处理模型无关方法，用于通过输入特征与嵌入空间的关系来解释局部嵌入组织，特别适用于无监督黑盒模型的解释。


<details>
  <summary>Details</summary>
Motivation: 无监督黑盒模型难以解释，尤其是在发现科学知识时。现有解释方法要么过于细粒度，要么过于简化，无法有效关联相似样本。

Method: LAVA将潜在空间表示为一系列局部区域，通过原始特征之间的相关性来描述这些区域，然后揭示整个潜在空间中重复出现的相关性模式。

Result: 基于MNIST和单细胞肾脏数据集的UMAP嵌入，LAVA能够捕获相关的特征关联，在看似遥远的潜在空间区域之间发现视觉和生物学相关的局部模式。

Conclusion: LAVA提供了一种有效的方法来解释无监督学习中的潜在空间结构，有助于科学发现和理解模型输出。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在图像分类任务中表现出色但容易受到精心设计的对抗性扰动攻击。本研究旨在探索线性降维方法作为简单、数据自适应的防御机制。

Method: 使用稀疏主成分分析（SPCA）作为前端特征提取器，结合理论分析推导鲁棒性证书，并通过实验比较SPCA与标准PCA在对抗性攻击下的性能表现。

Result: 实验表明，SPCA在强白盒和黑盒攻击下比PCA表现更稳定，同时保持竞争力的干净准确率。理论分析证实稀疏投影降低了对抗性杠杆效应。

Conclusion: 稀疏投影通过减少对抗性杠杆效应提高了模型的鲁棒性，这一优势在非线性设置中依然存在，为对抗性防御提供了新的思路。
...
### [94] [GRPO is Secretly a Process Reward Model](https://arxiv.org/abs/2509.21154)
*Michael Sullivan*

Main category: cs.LG

TL;DR: 本文证明了GRPO强化学习算法在特定假设下会诱导出非平凡的过程奖励模型，并提出了改进算法λ-GRPO来缓解原始目标函数中的缺陷，从而提升模型性能


<details>
  <summary>Details</summary>
Motivation: 研究GRPO算法中隐含的过程奖励模型特性，并解决其目标函数中由于过程步骤分布不均匀导致的探索和利用问题

Method: 首先理论证明GRPO会诱导出过程奖励模型，然后提出λ-GRPO算法作为改进，通过实验验证新算法的有效性

Result: λ-GRPO相比标准GRPO在验证准确率和下游推理任务上表现更好，达到峰值性能更快，且训练时间和成本影响可忽略

Conclusion: 研究表明可以利用GRPO算法内置的隐含过程奖励模型结构来提升性能，而无需依赖成本高昂的显式定义过程奖励模型
...
### [95] [Towards Foundation Models for Zero-Shot Time Series Anomaly Detection: Leveraging Synthetic Data and Relative Context Discrepancy](https://arxiv.org/abs/2509.21190)
...
### [96] [CAD-Tokenizer: Towards Text-based CAD Prototyping via Modality-Specific Tokenization](https://arxiv.org/abs/2509.21150)
*Ruiyu Wang,Shizhao Sun,Weijian Ma,Jiang Bian*

Main category: cs.LG

TL;DR: CAD-Tokenizer是一个多模态分词框架，用于改进文本引导的CAD原型设计，通过原始级池化和约束解码生成紧凑、原始感知的表示，显著提升了指令跟随和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM分词器将CAD序列分解为自然语言词片段，无法捕捉CAD的原始级语义，阻碍了几何结构建模。需要一种与CAD原始和结构性质对齐的多模态分词策略。

Method: 提出CAD-Tokenizer框架，使用基于序列的VQ-VAE，结合原始级池化和约束解码，生成模态特定的分词表示。

Result: 在统一的文本引导CAD原型设计中，CAD-Tokenizer显著改善了指令跟随和生成质量，在定量和定性评估上都优于通用LLM和任务特定基线。

Conclusion: 多模态分词策略能够提供更有效的CAD表示，为文本引导的CAD原型设计提供了更好的解决方案。
...
### [97] [Tree Search for LLM Agent Reinforcement Learning](https://arxiv.org/abs/2509.21240)
*Yuxiang Ji,Ziyu Ma,Yong Wang,Guanhua Chen,Xiangxiang Chu,Liaoni Wu*

Main category: cs.LG

TL;DR: 提出了基于树搜索的分组智能体强化学习方法Tree-GRPO，通过树结构轨迹构建过程监督信号，解决长期多轮任务中稀疏奖励问题


<details>
  <summary>Details</summary>
Motivation: 现有仅依赖结果奖励的强化学习方法在长期多轮智能体任务中存在监督信号稀疏的问题

Method: Tree-GRPO方法：基于树搜索的分组智能体RL，每个树节点代表完整智能体交互步骤，通过共享公共前缀增加采样效率，利用树结构轨迹构建过程监督信号

Result: 在11个数据集和3种QA任务上的实验表明，基于树的RL方法优于基于链的RL方法

Conclusion: Tree-GRPO通过树结构有效解决了稀疏奖励问题，在复杂智能体任务中表现出优越性能
...
### [98] [DATS: Distance-Aware Temperature Scaling for Calibrated Class-Incremental Learning](https://arxiv.org/abs/2509.21161)
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法主要从数据角度进行校准，使用单一温度参数，忽略了任务特异性差异，导致不同任务间的校准误差波动较大。在安全关键应用中，模型不仅需要良好的预测性能，还需要可靠的校准不确定性。

Method: 提出距离感知温度缩放(DATS)方法，结合基于原型的距离估计和距离感知校准，无需任务信息即可推断任务接近度并分配自适应温度参数。

Result: 在标准基准和真实世界不平衡生物医学数据集上的广泛实验表明，DATS方法相比最先进方法在降低跨任务校准误差方面表现稳定、可靠且一致。

Conclusion: DATS方法为持续学习中的校准问题提供了一个更原则性的解决方案，能够有效处理任务特异性差异，提高模型在安全关键应用中的可靠性。
...
### [99] [Explaining Fine Tuned LLMs via Counterfactuals A Knowledge Graph Driven Framework](https://arxiv.org/abs/2509.21241)
*Yucheng Wang,Ziyang Chen,Md Faisal Kabir*

Main category: cs.LG

TL;DR: 该论文提出了一个基于知识图谱反事实解释的框架，用于理解LoRA微调后大语言模型的结构推理和语义行为变化。


<details>
  <summary>Details</summary>
Motivation: 当前虽然LoRA能够高效地让大语言模型获取领域知识，但人们仍不清楚这种微调机制如何改变模型的结构推理和语义行为。
...
### [100] [Mixture of Thoughts: Learning to Aggregate What Experts Think, Not Just What They Say](https://arxiv.org/abs/2509.21164)
*Jacob Fein-Ashley,Dhruv Parikh,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.LG

TL;DR: MoT是一种在异构专家模型间进行潜在层协作的方法，通过轻量级路由器和交互层实现单次推理的多模型合作，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 开源大语言模型日益专业化，需要系统能够利用不同模型的互补优势。现有方法要么只路由到少数专家独立生成，要么通过多轮交互聚合输出，要么需要架构同质性进行权重融合。

Method: MoT使用轻量级路由器选择top-K专家并指定主专家，通过统一放置的交互层将隐藏状态投影到共享潜在空间，主专家对活跃同行进行交叉注意力。预训练专家保持冻结，只训练路由器和交互层。
...
### [101] [A Causality-Aware Spatiotemporal Model for Multi-Region and Multi-Pollutant Air Quality Forecasting](https://arxiv.org/abs/2509.21260)
*Junxin Lu,Shiliang Sun*

Main category: cs.LG

TL;DR: AirPCM是一个新颖的深度时空预测模型，用于解决空气污染多污染物、多区域的复杂预测问题，通过整合多区域多污染物动态与显式气象-污染物因果关系建模，实现精确可扩展的空气质量预测。
...
Method: AirPCM采用统一架构，联合捕捉跨站点空间相关性、时间自相关性以及气象-污染物动态因果关系，实现细粒度、可解释的多污染物预测，包括突发污染事件。

Result: 在多尺度真实数据集上的广泛评估表明，AirPCM在预测准确性和泛化能力方面持续超越现有最先进基准方法。
...
TL;DR: 本文提出了一个基于f-散度的统一框架，用于扩散模型的机器遗忘，替代传统的MSE方法，允许根据不同应用需求选择最优散度来平衡遗忘效果和概念保留。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型遗忘方法主要依赖均方误差(MSE)，作者发现这只是f-散度框架的一个特例，希望通过统一框架提供更灵活的遗忘策略。

Method: 构建基于f-散度的统一遗忘框架，分析不同f-散度对算法收敛性和遗忘质量的影响，允许根据具体应用选择最优散度。

Result: 证明了MSE方法是f-散度框架的特殊情况，不同f-散度在遗忘攻击性和概念保留之间存在不同权衡关系。

Conclusion: 提出的统一框架为机器遗忘提供了灵活范式，能够根据特定应用需求选择最优散度，实现更好的遗忘效果与概念保留平衡。
...
### [103] [It's Not You, It's Clipping: A Soft Trust-Region via Probability Smoothing for LLM RL](https://arxiv.org/abs/2509.21282)
*Madeleine Dwyer,Adam Sobey,Adriane Chapman*

Main category: cs.LG

TL;DR: 本文提出概率平滑策略优化（PSPO）方法，通过平滑当前策略概率来替代传统的比率裁剪，在保持梯度信号的同时创建软信任区域，从而稳定大语言模型的强化学习训练。
...
Method: PSPO方法在计算重要性比率之前，将当前策略的概率向旧策略平滑，类似于标签平滑技术。该方法在GRPO框架中实现为GR-PSPO。

Result: 在Qwen2.5-0.5B和Qwen2.5-1.5B模型上的实验显示，相比裁剪GRPO，GR-PSPO在GSM8K测试集上性能提升超过20%（0.5B模型：39.7% vs 17.6%；1.5B模型：59.4% vs 37.8%），并且产生更清晰、简洁和逻辑性更强的推理过程。

Conclusion: PSPO方法通过概率平滑有效解决了传统比率裁剪的信息丢失问题，在保持训练稳定性的同时显著提升了模型性能，为大语言模型的强化学习训练提供了更优的优化策略。
...
TL;DR: 本文研究了过参数化线性回归中最小ℓ_p插值器的参数范数族随样本量的缩放规律，并通过双射线分析揭示了信号尖峰与零坐标之间的竞争关系，建立了数据依赖的过渡点和通用阈值。同时将理论结果扩展到对角线性网络，揭示了显式和隐式偏置之间的预测桥梁。


<details>
  <summary>Details</summary>
Motivation: 研究过参数化线性回归中最小ℓ_p插值器的参数范数缩放规律，解决这一基本但尚未解决的问题，并为理解不同ℓ_r范数在泛化中的预测能力提供理论基础。

Method: 采用双射线分析方法，分析X^⊤Y中信号尖峰与零坐标之间的竞争关系，得到数据依赖的过渡点n_⋆和通用阈值r_⋆=2(p-1)。通过校准对角线性网络的初始化尺度α到有效p_eff(α)，将理论结果扩展到神经网络。

Result: 建立了参数范数族{∥ŵ_p∥_r}_{r∈[1,p]}的统一高概率缩放特征，发现当r<r_⋆时范数饱和，当r>r_⋆时范数以显式指数增长。对角线性网络继承了相同的肘部/阈值规律。

Conclusion: 该统一解决方案揭示了ℓ_p偏置插值下所有ℓ_r范数的缩放规律，解释了哪些范数饱和、哪些随n增长而增加。由于许多泛化代理依赖于∥ŵ_p∥_r，结果表明它们的预测能力将敏感地依赖于所使用的ℓ_r范数。
...
### [105] [No Prior, No Leakage: Revisiting Reconstruction Attacks in Trained Neural Networks](https://arxiv.org/abs/2509.21296)
*Yehonatan Refael,Guy Smorodinsky,Ofir Lindenbaum,Itay Safran*

Main category: cs.LG

TL;DR: 本文分析了神经网络训练数据重建攻击的固有局限性，证明了在没有先验知识的情况下存在无限多远离真实训练集的替代解，使得重建本质上不可靠。


<details>
  <summary>Details</summary>
Motivation: 神经网络对训练数据的记忆引发了隐私和安全担忧，现有重建攻击方法虽然在某些条件下有效，但其可靠性和理论基础仍不明确。本文旨在分析现有重建方法的弱点，识别其失败的条件。

Method: 采用理论分析和实证验证相结合的方法：理论上严格证明无先验知识时重建的不可靠性；实证上展示精确复制训练样本仅偶然发生。

Result: 研究发现，更充分训练的网络（满足隐式偏差条件更强）实际上更不容易受到重建攻击，这在该设置下调和了隐私与强泛化需求之间的矛盾。

Conclusion: 研究结果完善了对训练集泄露可能性的理论理解，并为缓解重建攻击提供了新见解，表明隐私保护与模型泛化能力可以共存。
...
### [106] [Differential-Integral Neural Operator for Long-Term Turbulence Forecasting](https://arxiv.org/abs/2509.21196)
*Hao Wu,Yuan Gao,Fan Xu,Fan Zhang,Qingsong Wen,Kun Wang,Xiaomeng Huang,Xian Wu*

Main category: cs.LG

TL;DR: 本文提出了一种名为DINO的新型神经网络算子框架，通过分解局部微分算子和全局积分算子来改进湍流的长期预测，解决了现有方法在长期自回归预测中误差累积和物理保真度丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法（特别是神经算子）在湍流的长期自回归预测中经常失败，主要问题是灾难性误差累积和物理保真度丢失。这是因为它们无法同时捕捉湍流动力学的两种不同数学结构：局部耗散效应和全局非局部相互作用。

Method: DINO框架采用算子分解的第一性原理方法，通过并行分支学习不同的物理算子：局部微分算子（由约束卷积网络实现，可证明收敛到导数）和全局积分算子（由Transformer架构学习数据驱动的全局核）。这种基于物理的分解赋予了DINO卓越的稳定性和鲁棒性。

Result: 在具有挑战性的2D Kolmogorov流基准测试中，DINO在长期预测方面显著优于最先进的模型。它成功抑制了数百个时间步长的误差累积，在涡度场和能量谱方面保持了高保真度。

Conclusion: DINO为物理一致的长期湍流预测建立了新的基准，证明了通过物理驱动的算子分解方法可以有效解决湍流长期预测的挑战。
...
### [107] [From Physics to Machine Learning and Back: Part II - Learning and Observational Bias in PHM](https://arxiv.org/abs/2509.21207)
...
TL;DR: 这篇综述论文探讨了物理信息机器学习在预测与健康管理中的应用，通过嵌入物理知识来解决数据噪声、标签有限和系统复杂性等挑战，实现从被动预测到主动决策的转变。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的预测与健康管理面临传感器数据噪声、标签有限、退化行为复杂等挑战，需要结合物理知识来提高模型的可靠性和一致性。

Method: 通过物理信息建模和数据策略引入学习和观测偏差，包括物理约束损失函数、虚拟传感、物理仿真数据增强、多传感器融合，并结合强化学习实现主动决策。

Result: 物理信息机器学习能够提高PHM模型的预测准确性和物理一致性，支持从个体设备到整个机队的规模化部署。

Conclusion: 物理信息机器学习为PHM提供了有前景的解决方案，通过结合物理知识和数据驱动方法，实现了更可靠的预测和自适应决策能力。
...
### [108] [Go With The Flow: Churn-Tolerant Decentralized Training of Large Language Models](https://arxiv.org/abs/2509.21221)
*Nikolay Blagoev,Bart Cox,Jérémie Decouchant,Lydia Y. Chen*

Main category: cs.LG

TL;DR: GWTF是第一个支持容错的实用去中心化大语言模型训练框架，能够在异构客户端上实现高效协作训练，处理节点流失和网络不稳定性问题。
...
### [109] [AbideGym: Turning Static RL Worlds into Adaptive Challenges](https://arxiv.org/abs/2509.21234)
*Abi Aryan,Zac Liu,Aaron Childress*

Main category: cs.LG

TL;DR: AbideGym是一个动态的MiniGrid包装器，通过引入智能体感知扰动和可扩展复杂性来强制智能体在单个回合内进行适应，旨在解决强化学习智能体在动态变化环境中策略脆弱的问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习智能体通常训练出脆弱的策略，在环境动态变化时会失效，而静态基准测试加剧了这一问题。需要动态评估框架来促进策略的鲁棒性和适应性。

Method: 开发AbideGym作为MiniGrid的动态包装器，包含智能体感知扰动机制和可扩展的复杂性设置，能够强制智能体在单个训练回合中进行实时适应。

Result: AbideGym能够暴露静态策略的弱点，促进策略的韧性发展，为课程学习、持续学习和鲁棒泛化研究提供模块化、可复现的评估框架。

Conclusion: AbideGym为解决强化学习智能体在动态环境中的脆弱性问题提供了有效的测试平台，推动了适应性策略和鲁棒泛化能力的研究进展。
...
### [110] [Federated Flow Matching](https://arxiv.org/abs/2509.21250)
*Zifan Wang,Anqi Dong,Mahmoud Selim,Michael M. Zavlanos,Karl H. Johansson*

Main category: cs.LG

TL;DR: 本文提出了联邦流匹配（FFM）框架，用于在隐私约束下训练流匹配模型，解决了分散数据无法集中训练生成模型的问题。
...
Method: 提出了三种方法：FFM-vanilla（本地独立训练）、FFM-LOT（使用局部最优传输）和FFM-GOT（基于半对偶最优传输的全局协调策略）。

Result: 在合成和图像数据集上的实验表明，FFM能够在保护隐私的同时提高流直度和样本质量，性能接近集中式基线。

Conclusion: FFM框架成功实现了隐私保护的联邦训练，在保持数据隐私的同时达到了与集中训练相当的性能。
...
### [111] [humancompatible.train: Implementing Optimization Algorithms for Stochastically-Constrained Stochastic Optimization Problems](https://arxiv.org/abs/2509.21254)
*Andrii Kliachkin,Jana Lepšová,Gilles Bareilles,Jakub Mareček*

Main category: cs.LG

TL;DR: 本文介绍了humancompatible.train，一个基于PyTorch的Python工具包，用于训练带有随机约束的深度神经网络，实现了多种随机约束随机优化算法，并在公平性约束的深度学习任务上进行了算法比较。


<details>
  <summary>Details</summary>
Motivation: 目前深度神经网络约束训练在公平性和安全性等应用中受到广泛关注，但尚无行业标准工具包，需要开发易于扩展的解决方案。

Method: 开发了基于PyTorch的Python工具包humancompatible.train，实现了多种先前未实现的随机约束随机优化算法。

Result: 成功实现了工具包，并在带有公平性约束的深度学习任务上比较了两种算法的性能。

Conclusion: humancompatible.train是一个有效的、易于扩展的工具包，为深度神经网络的约束训练提供了标准化解决方案。
...
### [112] [SuperOffload: Unleashing the Power of Large-Scale LLM Training on Superchips](https://arxiv.org/abs/2509.21271)
*Xinyu Lian,Masahiro Tanaka,Olatunji Ruwase,Minjia Zhang*

Main category: cs.LG

TL;DR: 本文提出了SuperOffload系统，专门针对Superchips（GPU-CPU紧密耦合架构）优化LLM训练，通过自适应权重卸载、桶化重分区等技术，在NVIDIA GH200上实现2.5倍吞吐量提升，支持单芯片训练250亿参数模型。


<details>
  <summary>Details</summary>
Motivation: Superchips采用GPU-CPU紧密耦合架构提供前所未有的计算能力，但现有研究很少探索LLM训练如何受益于这种新架构，特别是卸载策略需要重新审视传统假设。

Method: 提出SuperOffload系统，结合自适应权重卸载、桶化重分区、Superchip感知类型转换、推测执行等技术，并为Grace CPU优化Adam优化器。

Result: 在NVIDIA GH200上评估显示，相比最先进的卸载系统，吞吐量提升达2.5倍，单Superchip可训练250亿参数模型。扩展后支持8个GH200训练130亿参数模型，序列长度达100万token，实现55%模型FLOPs利用率。

Conclusion: SuperOffload证明了Superchip架构在LLM训练中的巨大潜力，通过专门优化的卸载策略可显著提升训练效率和模型规模。
...
### [113] [Optimal Robust Recourse with $L^p$-Bounded Model Change](https://arxiv.org/abs/2509.21293)
*Phone Kyaw,Kshitij Kayastha,Shahin Jabbari*

Main category: cs.LG

TL;DR: 本文提出了一种新的算法，用于计算广义线性模型的最优鲁棒补救措施，使用L^p范数（p≥1但p≠∞）来约束模型变化，相比之前使用L^∞范数的方法，显著降低了补救成本。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒补救方法使用L^∞范数衡量模型变化，可能导致补救成本过高。本文旨在通过使用更约束的L^p范数来降低补救价格。

Method: 开发了一种新算法，能够可证明地计算广义线性模型的最优鲁棒补救，使用L^p范数（p≥1但p≠∞）来定义模型变化约束。

Result: 实验表明，该算法在线性和非线性模型上都显著降低了补救成本（可达几个数量级），在补救实施成本与有效性之间表现出更好的权衡，且提供更稀疏的补救方案。

Conclusion: 所提出的方法相比现有工作具有更低的补救成本、更好的权衡性能和稀疏性，同时对保证可行性的后处理方法保持鲁棒性。
...
### [114] [Recidivism and Peer Influence with LLM Text Embeddings in Low Security Correctional Facilities](https://arxiv.org/abs/2509.20634)
*Shanjukta Nath,Jiwon Hong,Jae Ho Chang,Keith Warren,Subhadeep Paul*

Main category: econ.EM

TL;DR: 使用预训练LLM对监狱居民书面交流进行嵌入分析，能比传统协变量更准确预测再犯罪率，并通过零样本分类和新的同伴效应模型揭示语言使用的社会动态。


<details>
  <summary>Details</summary>
Motivation: 研究监狱环境中语言交流模式与再犯罪率的关系，探索AI嵌入技术在刑事司法预测中的应用价值。

Method: 采用预训练Transformer LLM对8-12万条书面交流生成嵌入向量，进行零样本分类降维，并开发新的多元同伴效应模型分析网络内生性。

Result: 嵌入向量预测准确率比传统协变量高30%，发现语言使用中存在显著的同伴效应。

Conclusion: AI语言嵌入技术能有效预测再犯罪率，新开发的同伴效应模型为理解监狱社会动态提供了有力工具。
...
### [115] [Overidentification testing with weak instruments and heteroskedasticity](https://arxiv.org/abs/2509.21096)
*Stuart Lane,Frank Windmeijer*

Main category: econ.EM

TL;DR: 本文比较了Kleibergen-Paap（KP）秩检验和传统J检验作为异方差稳健的过度识别检验方法，发现KP检验通常表现更好，J检验容易产生严重的尺寸扭曲。建议在弱工具变量情况下使用KP检验而非J检验。


<details>
  <summary>Details</summary>
Motivation: 工具变量的外生性是IV估计的关键，可以通过过度识别检验来评估。传统J检验在存在弱工具变量和异方差时可能表现不佳，需要寻找更稳健的检验方法。

Method: 推导了J检验和KP检验在异方差弱工具变量情况下的极限分布，作为通过2SLS和LIML估计的稳健得分检验的特例。通过蒙特卡洛模拟比较两种检验的性能。

Result: 蒙特卡洛模拟显示KP检验通常优于J检验，J检验容易出现严重的尺寸扭曲。检验尺寸依赖于模型参数，在弱工具变量情况下这些参数无法一致估计，因此建议采用保守方法。

Conclusion: 建议在异方差情况下使用基于LIML的KP检验而非J检验。在生命周期消费模型中估计跨期替代弹性时，KP检验不拒绝工具变量有效性，而J检验经常过度拒绝，表明文献中EIS估计值的差异不太可能是由工具变量无效性引起的。
...
### [116] [Integrated analysis of informality, minimum wage, and monopsony power: A synthesis of meta-analyses with unified theoretical underpinnings](https://arxiv.org/abs/2509.20465)
*Ricardo Alonzo Fernandez Salguero*

Main category: econ.TH

TL;DR: 本文通过整合多个系统综述和元分析，对劳动力市场的非正规性、最低工资影响和买方垄断权力三个相互关联领域进行了综合分析，发现传统观点往往高估了这些效应的程度，并揭示了普遍存在的发表偏倚问题。


<details>
  <summary>Details</summary>
Motivation: 旨在综合现有关于劳动力市场三个关键领域（非正规性、最低工资效应、买方垄断权力）的经济文献，通过元分析方法揭示真实效应大小，并解决传统观点可能存在的偏差问题。

Method: 采用文献综合和元分析方法，整合多个现有系统综述和元分析的结果，并引入一个整合了企业异质性、买方垄断权力和内生正规化决策的综合理论模型来微观基础化实证发现。

Result: 研究发现：基于降低正规化成本的非正规性减少政策基本无效，而加强执法更有效；最低工资对就业的影响（OWE）普遍较小；所有研究领域都存在显著的发表偏倚，校正后的效应估计值通常接近零。

Conclusion: 元回归分析是识别异质性和真实基础效应的关键工具，需要重新校准理论和政策建议，采用更细致和整合的方法。发表偏倚导致已发表文献中报告的效应大小被夸大。
...
### [117] [Börgers's Open Question Resolved](https://arxiv.org/abs/2509.20790)
*Siyang Xiong*

Main category: econ.TH

TL;DR: 本文研究了随机有限行动机制下的占优策略和迭代占优策略实现问题，解决了Börgers(1995)中的开放性问题，并在不要求可分离性、拟线性或无限行动集的条件下提供了首个积极结果。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为纳什实现需要可分离性、拟线性或无限行动集等强假设，本文旨在突破这些限制，探索在更一般条件下的实现可能性。

Method: 研究随机有限行动机制，分析占优策略和迭代占优策略的实现条件。

Result: 建立了可能性和不可能性结果，解决了Börgers(1995)中的开放性问题，并提供了首个不依赖传统强假设的积极实现结果。

Conclusion: 证明了在随机有限行动机制框架下，即使不要求可分离性、拟线性或无限行动集，也能实现占优策略和迭代占优策略的实现。
