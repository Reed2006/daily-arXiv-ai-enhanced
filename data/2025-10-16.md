...
Motivation: 尽管大语言模型在多种语言中表现出色，但在波斯语等低资源语言中的有效性需要深入研究，以填补相关研究空白。

Method: 使用ParsiNLU和ArmanEmo等波斯语数据集，在情感分析、命名实体识别、阅读理解等任务上评估多个开源LLM，采用零样本和少样本学习范式，使用准确率、F1分数、BLEU和ROUGE等指标。

Result: Gemma 2模型在几乎所有任务和两种学习范式中表现最佳，尤其在复杂推理任务中表现突出，但大多数模型在命名实体识别等token级理解任务上表现不佳。

Conclusion: 该研究为多语言大语言模型研究提供了波斯语性能的宝贵见解，并为未来模型开发提供了基准参考，同时揭示了波斯语处理中的特定挑战。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录包含不一致结构或自由文本数据，需要高效预处理以支持预测性医疗模型。虽然AI驱动的自然语言处理工具在自动化诊断分类方面有潜力，但其比较性能和临床可靠性需要系统评估。

Method: 分析了762个独特诊断(326个ICD代码描述，436个自由文本条目)，来自3456名癌症患者的记录。测试模型将诊断分类到14个预定义类别的能力，由两名肿瘤学专家验证分类结果。

Result: BioBERT在ICD代码上获得最高加权宏F1分数(84.2)，在ICD代码准确率上与GPT-4o持平(90.8)。对于自由文本诊断，GPT-4o在加权宏F1分数(71.8 vs 61.5)和准确率(81.9 vs 81.6)上优于BioBERT。GPT-3.5、Gemini和Llama在两种格式上表现较差。常见误分类包括转移瘤与中枢神经系统肿瘤混淆，以及涉及模糊或重叠临床术语的错误。

Conclusion: 虽然当前性能水平对于行政和研究用途似乎足够，但可靠的临床应用将需要标准化文档实践以及强大的监督机制，以支持高风险决策。
...
<details>
  <summary>Details</summary>
Motivation: 当前偏好学习数据集将多个标注聚合成单一标签，抹杀了人类价值观的多元性，而这正是AI对齐旨在保留的核心内容。

Method: 提出将HLV作为Selbstzweck(自为目的)纳入AI系统设计，并概述了将HLV整合到偏好数据集中的可行步骤。

Result: 论证了HLV不应被视为噪声而应作为信号，特别是在大语言模型时代，HLV对于模型对齐具有日益重要的意义。

Conclusion: HLV体现了人类视角的合法多样性，必须作为AI系统设计的核心目标来保留，而非被消除的噪声。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究记录了LLM在不同患者群体输出中的差异，但缺乏对受控人口统计变化下内部推理如何转变的理解。

Method: 构建MEDEQUALQA基准，仅扰动患者代词（他/她/他们）而保持关键症状和条件不变，生成三个平行数据集（共69,000项），使用GPT-4.1模型并计算语义文本相似性来评估推理稳定性。

Result: 结果显示总体相似性高（平均STS>0.80），但在引用的风险因素、指南依据和鉴别诊断排序方面存在一致的局部差异，即使最终诊断保持不变。

Conclusion: MEDEQUALQA提供了一个受控诊断环境，用于审计医学AI中的推理稳定性，揭示了可能转化为不公平护理的临床相关偏见位点。
...
Method: 采用分类器增强生成(CAG)方法，结合话语分解、分类器和特定阶段的小样本提示来准确预测阶段，然后通过边缘预测连接非线性工作流，并从子话语上下文推断阶段属性

Result: 与强基线方法相比，CAG在准确性和效率方面表现更优，同时显著减少token使用量

Conclusion: 该系统是首个在自然语言驱动ETL创作中，对阶段预测、边缘布局和属性生成进行全面评估的系统，具有模块化、可解释性和端到端工作流生成能力
...
Result: 有提示时大多数模型（特别是Gemini-2.5-pro和Claude-3.7-Sonnet）达到接近完美的表现；无提示时所有模型在同行评估中都选择欺骗而非坦白（100%），在廉价谈话中选择欺骗的模型成功率高达95-100%。
...
Method: 使用GPT-5模型的不同证明者和验证者实例协作协议，最终通过Lean证明助手进行正式验证，并由人工确认前提和结论的一致性。

Result: 成功解决了2025年IMO的5/6问题，并证明了Cohen数论猜想集中66个猜想中的1/3（约22个）。

Conclusion: 该方法证明了大型语言模型在复杂定理证明任务中的有效性，通过协作协议和正式验证确保了证明的可靠性。
...
Method: 将任务建模为马尔可夫决策过程，代理与数据库交互获取执行反馈，与持久对话记忆进行连贯性验证，执行"提出→执行→验证→优化"的迭代循环。

Result: 在COSQL和SPARC数据集上的实验表明，MTSQL-R1持续优于强基线，突显了环境驱动验证和记忆引导优化在对话语义解析中的重要性。

Conclusion: 环境驱动的验证和记忆引导的优化对于对话语义解析至关重要，MTSQL-R1框架为多轮Text-to-SQL任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统标注指南是为人类标注者设计的，而大语言模型需要明确、结构化的指令，因此需要研究如何将现有指南重新用于指导LLM标注。

Method: 提出了一种基于LLM调节的指南重构方法，通过LLM调节过程将传统指南转化为适合LLM的清晰指令。

Result: 实验表明重构后的指南能有效指导LLM标注者，同时揭示了若干实际挑战。

Conclusion: 该方法具有支持可扩展、经济高效的标注指南优化和自动标注的潜力。
...
Result: 在32B规模上，A²FM在BrowseComp达到13.4%，AIME25达到70.4%，HLE达到16.7%，创下可比模型的新SOTA；自适应执行每个正确答案成本仅0.00487美元，相比推理模式降低成本45.2%，相比代理模式降低成本33.5%

Conclusion: A²FM统一框架成功解决了推理型和代理型LLM的能力分裂问题，在保持高准确性的同时显著提升了成本效率，为构建更高效的多功能语言模型提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 现有评估LLM长文本生成事实准确性的方法存在效率低下（复杂流水线）和效果不佳（声明集不准确、证据收集不足）的问题。

Method: 采用分块级声明提取与基于置信度的预验证，收集文档级证据并在验证时选择性检索，减少网络搜索和推理调用成本。

Result: 在聚合的人工标注基准测试中，FastFact在效率和效果上都优于现有基线方法，与人工评估具有最高一致性。

Conclusion: FastFact是一个快速有效的评估框架，能够可靠地评估LLM长文本生成的事实准确性，代码和基准数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型的评估主要局限于英语为中心的基准，缺乏对多语言和长文本设置的评估，特别是对低资源语言的覆盖不足。

Method: 从目标语言的网络资源中策划数据集，涵盖十个不同的图像类别和丰富的文本上下文，通过自动评估和母语者评估来测试模型的响应和推理能力。
...
Result: 在浅层六层模型中组合这些机制可能会增加延迟，但在SST-2任务上比优化的DistilBERT基线获得了稍高的准确率。

Conclusion: EAT展示了动态计算在延迟敏感NLP中的潜力，主要贡献是提供了一个开放的、端到端的可复现框架作为社区研究工具。
...
<details>
  <summary>Details</summary>
Motivation: 古兰经诵读教学面临现代化挑战，现有数字工具未能有效解决教学需求，需要重新思考自动评估方法。

Method: 对过去20年的学术研究、网络平台和商业应用进行全面文献分析，识别现有方法的局限性。

Result: 发现基于ASR的方法存在数据依赖、人口偏见和诊断反馈不足等问题，无法满足教学需求。

Conclusion: 未来应发展结合深度语言知识和高级音频分析的混合系统，构建稳健、公平且教学有效的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在教育领域的应用日益重要，需要专门的师生对话基准来评估其教学能力，现有的多轮对话基准不足以满足智能教育的需求。

Method: 构建EduDial数据集，基于Bloom教育目标分类法和十种提问策略（包括情境提问、最近发展区提问、元认知提问等），为不同认知水平学生设计差异化教学策略，并训练EduDial-LLM 32B模型。

Result: 在17个主流LLM上的实验表明，大多数模型在学生中心教学场景中表现不佳，而EduDial-LLM在所有指标上均显著优于基线模型。

Conclusion: EduDial为评估LLMs教学能力提供了全面基准，EduDial-LLM在师生对话场景中表现出色，为智能教育应用提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: LLMs应基于客观知识真实回答事实性问题，不受用户个人背景信息影响。现有研究主要关注对抗性输入，而忽略了真实交互中用户披露的个人属性对模型鲁棒性的影响。

Method: 系统评估LLMs对询问角色（传达身份、专业知识或信念等属性的用户档案）的鲁棒性，关注真实交互中用户可能披露的合理、以人为中心的询问角色线索。

Result: 发现此类角色线索能显著改变问答准确性，并触发拒绝回答、虚构限制和角色混淆等失败模式。

Conclusion: 模型对用户框架的敏感性会损害事实可靠性，询问角色测试是评估模型鲁棒性的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在人类互动中作用日益重要，但好奇心这一核心驱动因素在不同文化背景下的表现尚未得到充分研究。

Method: 使用Yahoo! Answers多国数据集，提出CUEST评估框架，通过语言风格、话题偏好分析和社会科学构建来测量人机好奇心对齐度，并探索微调策略。

Result: 发现LLMs会削弱跨文化多样性，更接近西方国家的表达方式；通过微调可将人机对齐差距缩小50%；好奇心对LLM跨文化适应性具有实际价值。

Conclusion: 好奇心是未来NLP研究的重要方向，对提升LLMs的跨文化适应能力至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在生成类似人类内容方面表现出色，人们担心其可能被滥用于大规模生成具有说服力的个性化虚假信息。此前研究虽证实LLM能生成虚假信息，但在说服力和个性化方面的具体问题尚未充分研究。

Method: 采用红队测试方法，系统评估LLM安全机制对个性化提示的鲁棒性。创建AI-TRAITS数据集，包含约160万条由8个先进LLM生成的文本，基于324个虚假信息叙事和150个不同人物画像，覆盖4种主要语言和关键人口维度。

Result: 使用简单的个性化策略会显著提高所有研究LLM的越狱可能性。个性化提示导致语言和修辞模式改变，并增强了LLM生成虚假叙事的说服力。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习评估缺乏对人类学习模式的模拟，需要更系统、细粒度的基准来评估模型在技能获取过程中的保留和迁移能力。

Method: 构建技能图谱将广泛技能分解为小能力、具体目标和可测量指标，生成包含段落、理解问答、技能测试问答和指令-响应的合成数据集，使用1.35亿参数transformer在不同设置下训练。
...
<details>
  <summary>Details</summary>
Motivation: 收集人类偏好数据成本高昂且效率低下，需要减少所需标注的方法。偏好方差衡量模型在比较响应对时的偏好差异程度，可能影响DPO训练效果。

Method: 通过理论分析建立DPO梯度范数上界与偏好方差的关系，然后使用奖励模型生成偏好数据，在AlpacaEval 2.0和Arena-Hard基准上微调LLMs进行验证。

Result: 实验表明高偏好方差的提示优于随机选择或低方差提示。使用小奖励模型进行选择也表现稳健。在UltraFeedback数据集上，仅使用前10%高方差提示的训练效果优于使用完整数据集。

Conclusion: 偏好方差是识别信息丰富示例的重要指标，有助于实现高效的LLM对齐，减少所需标注数据量。
...
<details>
  <summary>Details</summary>
Motivation: 现有MoE架构面临功能相似专家同时被选中的问题，导致计算冗余和有效模型容量受限。现有的平衡损失方法仅改善token分布，未能解决专家多样性问题。

Method: GatePro识别最相似的专家对，引入局部竞争机制，防止冗余专家同时激活，同时保持专家自然专业化。该方法无需额外可学习参数，可在任何训练阶段热插拔部署。

Result: 综合评估表明GatePro在不同模型规模和基准测试中均有效。分析显示GatePro能实现增强的专家多样性，专家发展出更独特和互补的能力，避免功能冗余。

Conclusion: GatePro为提升MoE效率提供了实用解决方案，通过直接促进专家选择多样性，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验对医学进步至关重要，需要包含适当和多样化的参与者。自动化临床文本资格分类和摘要能提高研究效率。

Method: 结合词嵌入(Word2Vec)、命名实体识别、计数向量化和TF-IDF等特征提取方法，使用随机森林和SVM进行多标签分类，评估TextRank、Luhn和GPT-3等摘要技术。

Result: 通过ROUGE分数评估显示所提方法有效，系统在自动化临床试验资格评估方面显示出潜力。

Conclusion: 该系统展示了使用数据驱动方法自动化临床试验资格评估的潜力，能提高研究效率。
...
Method: 比较两种单样本策略：基于质心的代表性示例（提出方法）和随机采样示例（基线），同时调整采样温度，评估其对LLM集成性能的影响。

Result: 提出的方法在较高温度设置下显著优于随机选择：宏F1提高+7.6%，RMSE降低-10.5%；同时超过五样本提示：宏F1提高+21.1%，RMSE降低-24.0%。
...
<details>
  <summary>Details</summary>
Motivation: 中东和北非地区在当前AI评估中代表性不足，需要专门基准来评估LLMs与该地区文化价值观的对齐程度。

Method: 基于大规模人类调查构建结构化数据集，在三种视角框架（中立、个性化、第三方文化观察者）和两种语言模式（英语和本地语言）下评估多种模型。

Result: 发现跨语言价值转移、推理导致文化对齐恶化、隐藏偏好等关键现象，模型在本地语言中会将多样化国家简化为单一实体。
...
<details>
  <summary>Details</summary>
Motivation: 传统推测解码方法在增加草稿大小时会提高接受率但引入额外延迟，限制了速度-准确性的权衡。现有方法部分减少了草稿成本，但要么降低接受率，要么引入限制扩展的开销。

Method: Mirror-SD从早期退出信号启动分支完整展开，与目标模型后缀并行执行，在异构加速器（GPU和NPU）上映射计算以利用跨设备并行性。草稿推测目标模型的向前延续，同时目标模型推测草稿的修正路径，形成两个互补的执行流水线。还添加推测流式处理，使草稿每步发射多个令牌。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数LLMs擅长生成原始的序列化代码，但在基于图的抽象代码生成方面研究较少，这种能力对于可视化编程语言和无法访问源代码的场景很重要。

Method: 提出并评估了图的JSON表示方法，在基于自定义Python实现的Scratch的ScratchTest基准上进行测试，检验LLMs在图结构代码空间的生成能力。

Result: 研究发现LLMs确实能够在单次推理中完成基于图的抽象代码生成任务，无需复杂流水线，且不同表示方法会导致显著不同的准确率。
...
Method: 构建多样化推理轨迹池，通过自动检索领域知识进行增强，使用新颖性驱动选择、反思性重组和变异进行迭代优化，通过评估答案正确性、连贯性和知识利用效率的适应度函数指导优化过程。

Result: 生成高质量的科学推理思维链数据集，基于该数据集微调的紧凑模型在科学推理基准上实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有CoT微调研究多关注技术层面，缺乏从人类认知机制角度的系统分析。鉴于CoT的最终目标是让LLM具备类人推理能力，从人类推理理论视角进行研究至关重要。

Method: 采用六顶思考帽框架对CoT微调方法进行分类和系统分析，收集整理了现有数据集和模型性能，并维护实时更新的GitHub仓库跟踪最新进展。

Result: 提出了基于人类推理理论的CoT微调系统分类框架，识别了CoT微调在数学推理和代码生成等任务中的显著改进效果。

Conclusion: 该综述为CoT微调研究提供了有价值的资源，有望激发这一快速演进领域的创新和进步，强调了从人类认知角度理解CoT技术的重要性。
...
Method: SHIELD结合细粒度安全分类与类别特定指导，使用明确的行动（阻止、重构、转发），通过组合定制化的安全提示来强制执行细微的拒绝或安全重定向。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG研究主要关注检索质量和提示策略，而检索文档的上下文格式如何影响模型性能尚未得到充分探索。作者发现看似表面的格式选择会导致准确性和稳定性的显著变化。

Method: 设计了控制实验来研究上下文密度、分隔符样式和位置放置的影响，并基于这些发现提出了上下文标准化策略，该策略在生成前自适应地标准化上下文表示。

Result: 在受控和真实世界RAG基准测试上的广泛实验表明，所提出的策略持续提高了对顺序变化的鲁棒性，并增强了长上下文的利用能力。

Conclusion: 可靠的RAG不仅依赖于检索正确的内容，还依赖于内容的呈现方式。本研究提供了新的实证证据和实用技术，以改善长上下文推理。
...
<details>
  <summary>Details</summary>
Motivation: 传统语音翻译系统忽略了重音等韵律特征，导致说话者意图和情感表达不完整，需要开发能够保留副语言线索的翻译方法。

Method: 利用LLM进行跨语言重音转换，将源语言重音转换为目标语言标签，指导可控TTS模型生成。开发自动生成对齐训练数据的流程，并引入LLM-as-Judge进行评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI系统中因数据代表性不足导致的群体间性能差异问题，传统公平性方法依赖受保护属性标签且存在准确性与公平性权衡

Method: 使用大语言模型生成性别交换的反事实示例，通过结构化提示创建性别转换的改写句子，并进行语义相似度检查、属性验证、毒性筛查等质量控制

Result: 在控制数据集上，LGSA将性别偏见差距从7.2%降至1.9%，同时将准确率提升至99.1%，改善了女性标签样本的性能
...
Conclusion: 该方法能够有效解决低资源语言的数据稀缺问题，为机器翻译等NLP任务提供高质量的双语数据资源。
...
<details>
  <summary>Details</summary>
Motivation: 探究LLMs展现的人类水平语言能力背后的计算机制是否与人类大脑相似，以及识别负责句法处理的具体组件。

Method: 使用分层频率标记探针(HFTP)进行频域分析，识别LLMs中单个MLP神经元和人类大脑皮层区域编码句法结构的组件，并进行表征相似性分析。

Result: GPT-2、Gemma、Gemma 2、Llama 2、Llama 3.1和GLM-4在相似层级处理句法，而人类大脑依赖不同皮层区域；LLM表征与大脑左半球对齐更强；模型升级趋势不同：Gemma 2比Gemma更接近大脑，Llama 3.1比Llama 2更偏离大脑。

Conclusion: HFTP是连接计算语言学和认知神经科学的有价值工具，为LLMs行为改进的可解释性提供新见解，并引发这些改进是否由类人或非类人机制驱动的思考。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在抽象推理任务上仍有根本性弱点，特别是当任务涉及网格、符号或视觉模式等不同于自然语言训练数据的表示时。需要开发更接近LLM预训练数据的自然语言基准来评估溯因推理能力。

Method: 引入Concept单词猜测棋盘游戏作为评估基准，该游戏使用自然语言表示。测试了LLMs在游戏中的表现，并扩展到多种语言（英语、荷兰语、法语、西班牙语）进行跨语言评估。

Result: 人类成功率超过90%，而最先进的LLMs成功率不超过40%。LLMs在理解玩家战略意图和根据序列信息修正初始假设方面表现不佳。在低资源语言中LLM性能进一步下降。

Conclusion: Concept游戏是一个有效的自然语言溯因推理基准，揭示了LLMs在抽象推理方面的显著局限性，特别是在多语言环境中表现更差，表明当前LLMs在理解战略意图和动态修正假设方面仍有重大挑战。
...
Method: 引入VERITAS框架，在强化学习过程中集成三种忠实度指标的信息-思维忠实度、思维-答案忠实度和思维-搜索忠实度的细粒度奖励。

Result: 使用VERITAS训练的模型在七个问答基准测试中显著提高了推理忠实度，同时保持了可比较的任务性能。

Conclusion: VERITAS框架有效解决了RL搜索代理的忠实度问题，在保持性能的同时显著提升了推理过程的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有激活引导方法使用固定干预强度，导致要么控制效果不足，要么过度干预破坏文本的合理性和连贯性，需要一种能根据输入自适应调整的方法。

Method: IDS方法在表示空间中基于输入数据分布动态调整干预强度，根据输入在分布中的位置自适应地施加干预，确保文本生成的稳定性。

Result: 实验表明IDS在分类任务上达到高准确率，同时生成连贯且不崩溃的文本，特别适合实际应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 随着服务需求的规模和复杂性增长，提升客户体验对企业成功至关重要。现有智能交互系统面临冷启动数据构建困难、多轮对话性能不佳、业务规则频繁演化、单一大模型能力不足以及开放域对话难以量化评估等挑战。

Method: WOWService整合LLMs和多智能体架构，实现自主任务管理和协作问题解决。系统核心模块包括数据构建、通用能力增强、业务场景适配、多智能体协调和自动化评估。

Result: WOWService已在美团App部署，关键指标显著提升：用户满意度指标1(USM 1)降低27.53%，用户满意度指标2(USM 2)提升25.51%，证明其在捕捉用户需求和推进个性化服务方面的有效性。

Conclusion: WOWService通过多智能体架构和LLMs集成，成功解决了工业级智能交互系统的关键挑战，显著提升了用户满意度和个性化服务水平。
...
Method: 提出自适应CFG方案，使用大型语言模型或自然语言推理模型检测风格-内容不匹配程度，并据此调整CFG参数。

Result: 提出的自适应CFG方案在保持音频质量和可懂度的同时，显著提升了自回归TTS模型的情感表达能力。
...
<details>
  <summary>Details</summary>
Motivation: 计算风格学通过文本中的量化模式分析写作风格，但监督和对比方法依赖带有虚假相关性的数据，经常混淆风格与主题。现代LLM的CLM预训练在通用作者身份问题中很少被利用。

Method: 基于LLM的广泛预训练和上下文学习能力，使用LLM的对数概率来测量从一个文本到另一个文本的风格可转移性。

Result: 该方法显著优于同等规模的LLM提示方法，在控制主题相关性的情况下比对比训练基线获得更高准确率。性能随基础模型大小一致扩展，在作者身份验证中通过增加测试时计算实现计算成本与准确性的灵活权衡。

Conclusion: LLM预训练和上下文学习能力为无监督风格分析提供了有效方法，在控制主题混淆的同时实现了优越的性能和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 对话问答中用户意图会随对话轮次演变，且表达往往不完整，需要上下文解释、查询重构以及检索与生成的动态协调，静态的'重写、检索、生成'流程无法满足需求。

Method: 基于强化学习的推理框架，交错进行搜索和推理，提出意图感知奖励机制提供轮次级反馈，使检索和推理与用户目标对齐。

Result: 在3B和7B模型骨干上均表现优异，在五个CQA数据集上超越竞争模型，在F1、BERTScore和LLM-as-judge等指标上均有优势。

Conclusion: 基于强化学习的推理比静态CQA流程更具灵活性和上下文敏感性，能够有效处理多领域对话问答任务。
...
TL;DR: 提出了EBCAR，一种轻量级的重排序框架，通过增强跨段落理解来解决RAG系统中需要跨段落推理的信息检索问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统将长文档分割为短段落进行检索，但忽略了需要跨段落推理的情况，如指代消解、实体消歧和证据聚合等挑战。

Method: EBCAR直接在检索段落的嵌入上操作，通过段落结构信息和混合注意力机制，捕获文档间的高层交互和文档内的低层关系。

Result: 在ConTEB基准测试中，EBCAR在需要跨段落推理的信息检索任务上表现出色，在准确性和效率方面均优于现有SOTA重排序方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法基于稳定性假设（固定的缓存条目子集在生成过程中始终重要），但该假设本质脆弱，均值聚合在极端情况下容易失效。
...
TL;DR: 利用LLM分析希腊谚语情感分布，发现大多数地区负面情感更普遍，LLM在非常规情感极性任务中表现准确


<details>
  <summary>Details</summary>
Motivation: 探索全球谚语景观，特别是希腊谚语的情感分析，因为许多文化通过口头传统保存智慧，现有研究不足

Method: 利用LLM进行谚语情感分类，扩展带注释的希腊谚语数据集以包含方言，结合地理位置、方言和主题进行组合分析

Result: LLM能准确描绘谚语情感，希腊大多数地区负面情感更普遍

Conclusion: LLM在谚语情感分析中表现良好，特别是在非常规情感极性任务中，希腊谚语整体偏向负面情感
...
Method: 采用基于低秩适应（LoRA）的微调特定类别适配器，在广泛的多模态数据集上训练，通过教师辅助标注管道生成高质量、上下文感知的标签。
...
TL;DR: 本文分析了联邦学习环境中ASR模型对属性推断攻击的脆弱性，通过白盒攻击方法仅使用权重差异就能推断出说话者的敏感属性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护隐私，但ASR模型在联邦设置下可能存在属性推断攻击的风险，需要评估这种安全威胁。

Method: 使用非参数白盒攻击方法，在被动威胁模型下测试Wav2Vec2、HuBERT和Whisper三个ASR模型，仅通过权重差异进行攻击。

Result: 攻击成功推断出性别、年龄、口音、情绪和构音障碍等敏感属性，其中口音信息在所有模型中都可靠地被推断出来。

Conclusion: 联邦ASR模型存在未记录的漏洞，预训练数据中代表性不足或缺失的属性更容易受到推断攻击，这为改进安全性提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在多轮对话中存在事实不一致和逻辑衰减问题，RAG和工作记忆等方法仍依赖静态知识源，无法适应动态对话上下文。

Method: 包含两个组件：动态结构化记忆(DSM)增量构建对话知识图谱，推理树(RT)在图上执行可追溯的多步推理搜索。

Result: 在MT-Bench-101基准测试中，D-SMART显著优于现有方法，对话一致性得分提升超过48%，开源模型质量得分提升达10.1%。

Conclusion: D-SMART通过动态结构化表示和显式推理机制，有效解决了多轮对话中的一致性问题，为对话系统提供了新的解决方案。
...
TL;DR: 本文综述了大型语言模型如何变革文档AI领域，从传统的编码器-解码器架构转向仅解码器架构，带来了理解和生成能力的显著提升。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的出现，文档AI领域经历了重大变革，需要全面梳理这一演变过程，总结当前研究进展并展望未来发展方向。

Method: 采用综述研究方法，系统分析文档AI的演进历程，重点关注多模态、多语言和检索增强的文档AI中的关键进展与挑战。

Result: 提供了文档AI领域的最新发展状况的结构化分析，涵盖了LLMs在该领域的应用现状和未来潜力。

Conclusion: 文档AI在LLMs的推动下取得了革命性进展，未来研究方向包括基于代理的方法和文档特定基础模型，对学术和实际应用都具有重要意义。
...
TL;DR: 该论文提出了一种基于贝叶斯说服框架的自然语言说服方法，通过承诺-沟通机制增强大语言模型的战略说服能力，在单轮对话中显著提高了说服成功率。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在说服能力方面存在挑战，现有研究往往忽视信息不对称的战略运用或依赖强假设。本文旨在探索贝叶斯说服在自然语言对话中的应用，提升LLMs的战略说服能力。

Method: 提出承诺-通信机制，说服者通过叙述自身潜在类型（如诚实或不诚实）来明确信息架构，引导被说服者进行贝叶斯信念更新。评估了半正式自然语言和完全自然语言两种变体。

Result: 实验表明：(1) 基于BP策略的LLMs说服成功率显著高于非BP基线；(2) SFNL具有更高可信度和逻辑一致性，FNL在自然对话中展现更强情感共鸣和鲁棒性；(3) 通过监督微调，小模型可达到与大模型相当的BP性能。

Conclusion: 贝叶斯说服框架能有效增强LLMs的战略说服能力，不同变体在不同场景下各有优势，且模型规模可通过适当训练得到补偿。
...
TL;DR: 该研究评估了生成代理模型Concordia和GPT-4在模拟真实世界环境中建模心智理论的能力，发现GPT-4经常无法基于信念归因选择行动，其表面上的心智理论能力可能源于浅层统计关联而非真正推理。


<details>
  <summary>Details</summary>
Motivation: 探索生成代理模型能否有效建模心智理论，并评估GPT-4是否能够从社会语境进行真正推理而非依赖语言记忆。

Method: 使用生成代理模型Concordia在模拟真实世界环境中评估GPT-4的心智理论能力，特别关注基于信念归因的行动选择和复杂社会互动的因果效应生成。

Result: GPT-4经常无法基于信念归因选择行动，在生成代理行动的连贯因果效应方面存在困难，表明其表面心智理论能力可能源于浅层统计关联。

Conclusion: 研究结果挑战了当前关于LLMs中涌现心智理论能力的说法，强调需要更严格的基于行动的评价框架。
...
TL;DR: 该研究应用系统发育比较模型分析三个语系中概念对的共词化现象，发现概念关联性越强，共词化范围越广且变化越慢；而使用频率高和易借用的概念对变化更快且较少共词化。
...
Method: 使用系统发育比较模型分析南岛语系、印欧语系和乌拉尔语系的词典数据，评估概念关联性、借用性和使用频率三个预测因素的影响。

Result: 概念关联性强的概念对在语系中更广泛共词化且变化更慢；高频和易借用的概念对变化更快且较少共词化；不同语系间存在显著差异。

Conclusion: 概念关联性促进共词化稳定性，而使用频率和借用性加速变化；区域和文化因素在意义演变中起重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语NLP研究者提供全面的基准测试参考，分析现有基准的多样性和不足，促进阿拉伯语LLM评估的发展。

Method: 提出四类分类法：知识、NLP任务、文化与方言、目标特定评估；分析三种主要方法：原生收集、翻译和合成生成。

Result: 发现基准多样性显著进步，但存在关键差距：时间评估有限、多轮对话评估不足、翻译数据集文化不对齐。

Conclusion: 为阿拉伯语NLP研究提供基准测试方法论、可重复性标准和评估指标的见解，并提出未来发展建议。
...
TL;DR: 提出了M^2PO框架，通过多视角奖励引擎和多对构造策略解决DPO在机器翻译中的两个关键问题：质量评估模型的缺陷奖励信号和低效数据利用。


<details>
  <summary>Details</summary>
Motivation: 当前基于DPO的机器翻译方法存在两个主要限制：(1) 质量评估模型提供的奖励信号存在缺陷，忽略关键错误如翻译幻觉；(2) 数据利用效率低下，仅选择单一胜负对丢弃了有价值的学习信号。

Method: M^2PO框架包含：1) 多视角奖励引擎，结合幻觉惩罚和动态质量评分；2) 多对构造策略，从所有翻译候选中系统性地创建全面的偏好对集合。

Result: 在WMT21-22基准测试中，M^2PO显著优于现有的偏好优化方法，并与领先的专有大语言模型表现出高度竞争力。

Conclusion: M^2PO通过多视角奖励和多对构造的协同方法，使模型能够从更丰富的质量权衡谱中学习，产生更鲁棒和忠实的翻译。
...
TL;DR: 提出了LiteraryQA，一个基于NarrativeQA的高质量文学问答子集，通过人工和LLM验证流程清理数据，并发现基于n-gram的自动评估指标与人类判断相关性低，而LLM-as-a-Judge方法能更好地匹配人类评估。
...
Method: 使用人工和LLM验证的流程识别和修正低质量问答样本，同时从源文档中移除无关文本，然后进行自动指标的元评估。

Result: 所有基于n-gram的指标在系统级别与人类判断的相关性较低，而LLM-as-a-Judge评估（即使是小型开源模型）能与人类确定的排名高度一致。
...
TL;DR: 该论文提出了第一个动态、实时的意图理解评估基准\bench，专门针对消费领域的真实公共讨论场景，解决LLM在理解复杂人类意图方面的评估空白。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏大规模基准来评估LLM在真实世界人类意图理解方面的能力，特别是在涉及多方观点、冲突视角和动态演变的公共讨论场景中。

Method: 构建了\bench基准，支持实时更新，通过自动化管理流程防止数据污染，是同类基准中规模最大、多样性最丰富的。

Result: 成功创建了首个专门用于意图理解的动态实时评估基准，填补了该领域评估工具的空白。

Conclusion: \bench基准为评估LLM在复杂真实世界场景中的意图理解能力提供了重要工具，有助于推动相关研究发展。
...
TL;DR: 本文提出了MedREK框架，通过检索式编辑方法解决医疗LLMs中知识过时和错误的问题，支持单样本和批量编辑，在医疗基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 医疗LLMs面临知识快速更新和训练数据错误的问题，导致生成过时或不准确信息，限制了在临床实践中的应用。模型编辑是无需完全重新训练的有效解决方案。

Method: 构建MedVersa基准测试集，提出MedREK框架，集成共享查询-键模块进行精确匹配，结合基于注意力的提示编码器提供信息指导。

Result: 在多个医疗基准测试中，MedREK在不同核心指标上均取得优异性能，并首次为医疗LLMs提供了经过验证的批量编辑解决方案。

Conclusion: MedREK框架有效解决了医疗LLMs的编辑问题，特别是在批量编辑方面填补了研究空白，为医疗领域的高风险应用提供了可靠支持。
...
TL;DR: 该论文通过分析注意力机制揭示了LLM的推理模式，提出了基于注意力模式的强化学习策略，在推理任务中实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: LLM的推理模式不透明，强化学习通常对整个生成过程均匀分配奖励，无法区分关键步骤和常规步骤。

Method: 区分局部和全局注意力头，提出两个度量指标：窗口平均注意力距离和未来注意力影响，并基于这些发现开发了三种针对关键节点的强化学习策略。

Result: 揭示了重复出现的预规划-锚定机制，提出的强化学习策略在各种推理任务中实现了持续的性能提升。
...
TL;DR: 提出一种通过识别语言特定子网络进行针对性微调的方法，在仅更新最多1%参数的情况下，提升低资源语言的单语能力，同时保持模型的通用性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在不同语言间表现不均，高资源和低资源语言之间存在显著性能差距，需要一种高效的方法来提升低资源语言能力。

Method: 使用语言激活概率熵识别语言特定神经元，仅微调与这些神经元相关的权重（专用子网络）在目标语言数据上。

Result: 在12种中低资源语言上的实验表明，该方法在性能上持续优于全微调、仅FFN微调、LoRA适应和随机子集微调基线。

Conclusion: 该方法为将最先进模型适配到低资源语言提供了一条成本效益高的路径，同时发布了100多种语言的神经元识别结果和适配流程。
...
TL;DR: 该论文介绍了在CPDC 2025 Round 2竞赛中，结合轻量级提示技术和微调大模型的方法，在任务导向对话和上下文感知对话任务中取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: 利用大语言模型创建游戏中的动态非玩家角色，实现功能性任务执行和角色一致性对话生成，参与CPDC竞赛评估智能体在任务导向对话、上下文感知对话及其整合方面的能力。

Method: 采用两种互补策略：API赛道使用轻量级提示技术（包括Deflanderization提示方法抑制过度角色扮演），GPU赛道使用微调的Qwen3-14B模型（结合监督微调和LoRA适配）。

Result: 最佳提交在Task 1排名第2，Task 3（API赛道）排名第2，Task 3（GPU赛道）排名第4。

Conclusion: 结合提示工程和模型微调的混合方法在角色一致性对话生成任务中表现优异，证明了该方法在创建动态NPC方面的有效性。
...
Method: 从维基百科实时收集表格数据，支持英语、德语、俄语和法语等多语言，实现领域平衡的基准测试生成。

Result: LLM从新收集表格生成的见解在自动指标上表现较差，但在LLM和人工评估中差异不明显；领域效应在所有评估中都很明显。

Conclusion: 领域平衡的基准测试更具挑战性，FreshTab能有效解决数据污染和领域不平衡问题。
...
TL;DR: NOSA是一种可训练的稀疏注意力框架，通过引入显式局部性约束来减少KV缓存传输，在保持训练时相同注意力计算的同时提升解码吞吐量。
...
Result: 预训练的1B参数模型在保持接近无损性能的同时，相比基线InfLLM-V2实现了高达2.3倍的解码吞吐量提升。

Conclusion: NOSA通过显式局部性约束有效解决了KV缓存传输瓶颈，在保持性能的同时显著提升了解码效率。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在时间理解方面存在困难，特别是在涉及多实体、复合运算符和演化事件序列的问题中。现有基于时间知识图谱的LLM推理方法面临四个主要挑战：多跳推理中的时间忠实性、多实体时间同步、适应不同时间运算符的检索，以及重用先前推理经验以提高稳定性和效率。

Method: MemoTime将复杂时间问题分解为时间树层次结构，实现运算符感知推理，强制执行单调时间戳并在统一时间边界下共同约束多个实体。动态证据检索层自适应选择运算符特定的检索策略，而自演化经验记忆存储已验证的推理轨迹、工具包决策和子问题嵌入以供跨类型重用。

Result: 在多个时间QA基准测试上的综合实验表明，MemoTime实现了总体最先进的结果，比强基线高出24.0%。此外，MemoTime使较小模型（如Qwen3-4B）能够达到与GPT-4-Turbo相当的推理性能。

Conclusion: MemoTime通过结构化基础、递归推理和持续经验学习有效解决了时间推理中的关键挑战，显著提升了LLM的时间理解能力，并使较小模型能够达到与大型模型相当的推理性能。
...
TL;DR: 该研究通过基于公开目录的指令微调，显著提升了开源大语言模型在德语肿瘤诊断文本ICD编码任务中的准确性，ICD-10-GM精确准确率从1.4-24%提升至41-58%，部分准确率从31-74%提升至73-83%。


<details>
  <summary>Details</summary>
Motivation: 德国肿瘤诊断的ICD-10-GM和ICD-O-3准确编码对结构化癌症文档至关重要，但小型开源LLM在德语语境下的编码准确性较差，需要探索通过指令微调提升其性能。

Method: 基于ICD-10-GM、ICD-O-3和OPS目录创建了超过50万个问答对作为训练数据，对Qwen、Llama和Mistral家族的8个开源模型（7-70B参数）进行指令微调，并使用本地肿瘤文档系统的编码诊断作为测试数据。

Result: 微调后ICD-10-GM精确准确率提升至41-58%，部分准确率提升至73-83%；ICD-O-3地形编码精确准确率为22-40%，部分准确率为56-67%；畸形代码输出降至0%，肿瘤诊断识别率达99%；模型大小与准确性正相关但差距缩小；推理模式性能较低且速度慢100倍。

Conclusion: 利用公开目录构建指令数据集可以有效提升LLM在医疗文档任务中的性能，证明了开源模型在隐私保护自动化中的潜力。
...
TL;DR: SALAD方法通过跨模态蒸馏和有针对性的合成数据，在减少语音数据使用量的情况下，有效缩小文本-语音理解差距


<details>
  <summary>Details</summary>
Motivation: 语音适配的LLMs在语言理解任务上表现不如文本模型，存在文本-语音理解差距，现有方法依赖大规模合成数据或专有数据集，需要更数据高效的替代方案

Method: 提出SALAD方法，结合跨模态蒸馏和有针对性的合成数据，通过主动选择和学习来改善对齐同时减轻遗忘

Result: 在3B和7B LLMs上，SALAD在知识、语言理解和推理等广泛领域基准测试中，使用比公开语料库少一个数量级的语音数据，实现了与强开放权重模型竞争的性能

Conclusion: SALAD提供了一种数据高效的方法来缩小文本-语音理解差距，通过解决能力遗忘和跨模态不对齐问题，在减少数据依赖的同时保持竞争力
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM生成文本日益普遍且难以与人类写作区分，自动文本检测研究备受关注。虽然现有检测器报告接近完美的准确率，但这些结果通常基于固定的生成设置，缺乏对解码策略变化的鲁棒性评估。

Method: 系统性地研究了采样解码对可检测性的影响，重点关注模型在（子）词级别分布的细微变化如何影响检测性能。分析了温度、top-p、核采样等解码参数的变化。

Result: 发现即使对解码参数进行微小调整（如温度、top-p或核采样）也会严重损害检测器准确率，AUROC从接近完美水平降至1%。创建了包含37种解码配置的大规模数据集。
...
TL;DR: 提出了GAPS框架，一个多维度的AI临床医生系统评估范式，包含认知深度(G)、答案完整性(A)、鲁棒性(P)和安全性(S)四个维度，并通过自动化流程构建基准测试。
...
Method: 开发了全自动、基于指南的流程来构建GAPS对齐的基准测试，包括构建证据邻域、创建图和树表示、自动生成问题，并使用DeepResearch代理合成评分标准，通过LLM集成进行评分。

Result: 验证确认自动生成的问题质量高且与临床医生判断一致。评估发现最先进模型存在关键失败模式：随着推理深度增加性能急剧下降、模型难以保证答案完整性、对对抗性扰动高度脆弱且存在安全隐患。
...
TL;DR: 本文提出了一种评估聊天助手网络搜索行为的新方法，重点关注来源可信度和回答与引用来源的关联性。通过对GPT-4o、GPT-5、Perplexity和Qwen Chat在5个易产生错误信息的主题上的测试，发现不同助手在来源可信度方面存在差异。
...
Method: 使用100个涉及5个易产生错误信息主题的声明，评估GPT-4o、GPT-5、Perplexity和Qwen Chat等聊天助手，重点关注来源可信度和回答与引用来源的关联性。

Result: 不同助手表现存在差异，Perplexity获得最高的来源可信度，而GPT-4o在敏感主题上引用了更多非可信来源。
...
TL;DR: 提出了一种基于前馈网络激活的检索增强生成系统置信度估计方法，在金融客服场景中优于基线方法，并能在低延迟下保持高精度


<details>
  <summary>Details</summary>
Motivation: 在金融、医疗等高风险领域，错误答案的代价远高于不回答问题，因此需要准确估计LLM输出的置信度

Method: 利用原始前馈网络激活作为自回归信号，避免投影和softmax归一化造成的信息损失；将置信度预测建模为序列分类任务，使用Huber损失进行正则化训练

Result: 在真实金融行业客服场景中表现优于强基线方法，在严格延迟约束下保持高精度；在Llama 3.1 8B模型上，仅使用第16层激活即可保持精度同时降低响应延迟
...
TL;DR: 该论文通过机制性和因果分析框架，发现符号接地现象在语言模型的中间层计算中集中出现，通过注意力头聚合环境基础来支持语言形式预测，这一现象在多模态对话和不同架构中可复现。


<details>
  <summary>Details</summary>
Motivation: 探索大规模训练的语言模型中符号接地现象的具体出现位置和驱动机制，目前相关研究仍较少。

Method: 引入受控评估框架，通过机制性和因果分析系统追踪符号接地在内部计算中的产生过程。

Result: 发现接地集中在中间层计算，通过注意力头聚合环境基础实现，在Transformer和状态空间模型中可复现，但在单向LSTM中不行。
...
TL;DR: 提出一种通过学习和压缩KV缓存来减少长上下文推理中内存和计算成本的方法


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长上下文推理中的可扩展性受到Transformer键值缓存线性增长的严重限制，导致显著的内存和计算成本

Method: 定期使用学习的专用令牌压缩生成KV缓存并驱逐压缩条目，通过改进的联合蒸馏和强化学习框架训练模型

Result: 在内存-准确率Pareto边界上优于无缓存压缩模型和免训练压缩技术

Conclusion: 该方法有效解决了长上下文推理中的缓存扩展问题，实现了更好的内存效率
...
TL;DR: BRIEF-Pro是一个轻量级通用压缩器，可将检索到的文档压缩为简洁摘要，用于增强RAG系统处理复杂多跳问题的能力，显著提高性能并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 随着RAG处理复杂任务时上下文不断扩展，虽然提供了更丰富的信息，但也带来了更高的延迟和模型认知负担。特别是对于复杂的多跳问题，需要解决这一瓶颈问题。

Method: 使用相对较短的上下文（少于1000词）作为种子数据，训练BRIEF-Pro对超过10000词的扩展上下文进行抽象压缩，并允许用户控制摘要句子数量。

Result: 在四个开放域多跳问答数据集上的实验表明，BRIEF-Pro能生成更简洁相关的摘要，提升各种规模语言模型的性能。使用70B阅读器模型时，32倍压缩比LongLLMLingua的9倍压缩性能提升4.67%，计算开销仅需23%。
...
TL;DR: 提出基于高频K线数据的现货协方差估计新框架，利用开盘、最高、最低、收盘价格信息，在统计精度与定位能力间取得更好平衡，并应用于比特币ETF市场中性检验。


<details>
  <summary>Details</summary>
Motivation: 传统基于高频开盘-收盘收益数据的现货协方差估计方法面临统计精度与定位能力之间的权衡问题，需要更有效的解决方案。

Method: 利用高频K线数据（包含开盘、最高、最低、收盘价格）构建新的估计框架，开发基于K线的现货协方差推断检验方法。

Result: 新方法相比基准方法提高了估计精度并保持局部结构，检验方法在样本量较小时具有合理的尺寸控制和显著提升的检验功效。实证分析显示IBIT比特币ETF存在系统性偏离市场中性，尤其在市场压力时期。

Conclusion: 基于K线数据的估计框架有效解决了现货协方差估计中的权衡问题，能够检测到温和信息事件引发的细微变化，在金融实证分析中具有实用价值。
...
TL;DR: 开发了一种非参数框架来识别处理效应的空间边界，无需参数函数形式限制。该方法使用局部线性回归和数据驱动带宽选择来灵活估计空间衰减模式并检测处理效应边界。


<details>
  <summary>Details</summary>
Motivation: 传统参数方法可能强加虚假的空间模式，需要一种灵活的非参数方法来准确识别复杂的空间模式，避免参数模型的限制。

Method: 采用局部线性回归与数据驱动带宽选择的非参数方法，通过蒙特卡洛模拟验证方法性能，并应用于2015-2020年银行分行开业数据和2010-2023年分行生存数据。

Result: 银行分行邻近度显著影响贷款申请量（每10英里下降8.5%），但不影响批准率；分行生存与地区收入呈非单调关系，高收入地区反而有更多关闭，主要受分行密度而非收入直接影响。

Conclusion: 非参数方法对于检测参数模型会遗漏的复杂空间模式是必要的，挑战了关于银行荒漠的简单叙述，揭示了空间整合决策背后的组织复杂性。
...
TL;DR: 基于Moravec悖论构建理论驱动的自动化暴露指数，评估19,000个职业任务的自动化风险，发现管理、STEM和科学类职业暴露度最高，而维护、农业和建筑类最低。


<details>
  <summary>Details</summary>
Motivation: 挑战技能偏向型技术变革的传统观点，探索AI对劳动力市场的潜在影响，特别是当AI可能替代而非补充人力时。

Method: 基于Moravec悖论，从性能方差、隐性知识、数据丰度和算法差距四个维度对19,000个O*NET任务进行评分。

Result: 工资与自动化暴露度呈正相关，隐性知识与工资正相关，验证了Eloundou等人的AI标注方法（相关性0.72），与LLM前指数无正相关。

Conclusion: 该指数识别了根本的自动化潜力而非当前能力，表明自动化模式发生了范式转变，支持资历偏向型技术变革而非技能偏向型变革。
...
TL;DR: 提出了一个统一的市场基础框架，用于描述单一证券、市场所有证券以及市场投资组合交易的收益和方差，强调随机交易量变化的影响。


<details>
  <summary>Details</summary>
Motivation: 建立统一的市场交易描述框架，揭示Markowitz投资组合方差忽略了连续交易随机交易量的影响，提供更准确的风险评估方法。

Method: 基于投资者观察市场交易时间序列，将其转换为单一证券和市场投资组合的交易模型，建立统一的收益和方差描述方法。

Result: 市场基础方差在假设连续交易量恒定情况下等同于Markowitz投资组合方差，揭示了后者忽略随机交易量影响的局限性。

Conclusion: 该方法为任何投资组合及其证券交易提供了统一的收益和方差描述框架，但预测精度受高斯分布限制和经济障碍影响。
...
TL;DR: 研究分析如何优化州级补充补贴来缓解联邦保费补贴到期导致的医保覆盖损失，发现对低收入人群的补贴效果最佳，但州级补贴只能缓解而非完全阻止覆盖损失。


<details>
  <summary>Details</summary>
Motivation: 2026年联邦增强保费补贴到期预计将使700多万美国人失去医保覆盖，而10个州已创建补充补贴计划，但如何优化这些补贴结构以最大化覆盖范围尚未得到充分研究。

Method: 使用马里兰州医保市场的行政注册数据估计医保需求，然后基于估计参数和不同预算约束，模拟如何最优分配州级补充保费补贴。

Result: 收入低于联邦贫困线200%的参保人对保费最敏感，每月额外10美元补贴可使覆盖概率提高约6.5个百分点；每1000万美元年度州补贴可保留约5000名参保人，但一旦所有低收入参保人获得全额补贴，成本效益显著下降。

Conclusion: 州级补贴能够缓解但不能完全阻止联邦保费补贴到期导致的覆盖损失，需要有针对性的补贴策略。
...
### [83] [Local Timescale Gates for Timescale-Robust Continual Spiking Neural Networks](https://arxiv.org/abs/2510.12843)
...
<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络在神经形态硬件上具有能效优势，但在需要快速适应和长期记忆的持续学习任务中存在困难，特别是稳定性与可塑性之间的平衡问题。

Method: LT-Gate模型让每个脉冲神经元并行跟踪快慢两个时间尺度的信息，通过学习的门控机制局部调整其影响，并引入方差跟踪正则化来稳定放电活动。

Result: 在时序分类基准测试中达到约51%的最终准确率，优于Hebbian持续学习基线的约46%和之前的SNN方法，且无需外部重放或昂贵正交化。
...
### [84] [Lifting Manifolds to Mitigate Pseudo-Alignment in LLM4TS](https://arxiv.org/abs/2510.12847)
*Liangwei Nathan Zheng,Wenhao Liang,Wei Emma Zhang,Miao Xu,Olaf Maennel,Weitong Chen*

Main category: cs.LG

TL;DR: 论文研究了LLM4TS模型中的伪对齐问题，发现其根源是预训练LLM的锥效应与时间序列数据低维流形的相互作用，并提出了TimeSUP方法来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 伪对齐问题是导致许多时间序列大语言模型表现不佳的关键因素，但社区对其成因讨论有限，需要深入探究其根源。

Method: 提出了TimeSUP方法，通过增加时间序列流形维度以匹配语言嵌入的内在维度，使模型能清晰区分时间信号，同时捕捉跨模态共享结构。

Result: TimeSUP在长期预测性能上持续优于最先进的LLM4TS方法和其他轻量级基线，并能无缝集成到四种现有LLM4TS流程中显著提升预测性能。

Conclusion: 伪对齐源于LLM锥效应与时间序列低维流形的相互作用，TimeSUP通过调整流形维度有效缓解了该问题，在保持各模态独特特征的同时学习其共性。
...
### [85] [FedGTEA: Federated Class-Incremental Learning with Gaussian Task Embedding and Alignment](https://arxiv.org/abs/2510.12927)
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中任务增量学习面临的统计异质性、遗忘问题和隐私约束挑战，同时确保方法的可扩展性。
...
Conclusion: FedGTEA通过概率化任务嵌入和对齐机制，在联邦类增量学习中有效平衡了性能、可扩展性和隐私保护。
...
### [86] [Learning at the Speed of Physics: Equilibrium Propagation on Oscillator Ising Machines](https://arxiv.org/abs/2510.12934)
*Alex Gower*

Main category: cs.LG

TL;DR: 该论文展示了在振荡器伊辛机(OIMs)上实现平衡传播(EP)的方法，将机器学习优化与物理系统的能量下降过程统一起来，在MNIST和Fashion-MNIST数据集上取得了竞争性的准确率，同时保持了在硬件约束下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 利用物理系统自然执行能量下降的特性来加速机器学习，特别是将振荡器伊辛机的GHz频率动力学与能量基模型的优化和梯度下降联系起来，同时利用固有噪声实现朗之万动力学，支持采样和优化。

Method: 在振荡器伊辛机上实现平衡传播(EP)，通过单一总能量景观上的下降过程，实现无需全局反向传播的局部学习规则。

Result: 在MNIST数据集上达到97.2±0.1%的准确率，在Fashion-MNIST数据集上达到88.0±0.1%的准确率，并在参数量化和相位噪声等实际硬件约束下保持鲁棒性。

Conclusion: 振荡器伊辛机为神经形态学习提供了快速、节能的硬件基底，表明能量基模型可以在直接执行其优化的物理硬件上实现实际应用。
...
### [87] [Pruning Cannot Hurt Robustness: Certified Trade-offs in Reinforcement Learning](https://arxiv.org/abs/2510.12939)
*James Pedley,Benjamin Etheridge,Stephen J. Roberts,Francesco Quinzan*

Main category: cs.LG

TL;DR: 该论文证明了在状态对抗马尔可夫决策过程中，剪枝可以收紧认证鲁棒性边界，而不会降低策略的鲁棒性，并在中等稀疏度下发现性能-鲁棒性的平衡点。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的强化学习策略需要在对抗扰动下保持可靠性，而现代深度强化学习代理过度参数化导致成本高和脆弱性。剪枝在监督学习中已被证明能提高鲁棒性，但在对抗强化学习中的作用尚不清楚。

Method: 开发了首个在状态对抗MDP中剪枝认证鲁棒性的理论框架，分析了高斯和分类策略的Lipschitz网络，证明逐元素剪枝只能收紧鲁棒性边界。提出了三术语遗憾分解方法。

Result: 在连续控制基准测试中，剪枝在中等稀疏度下发现了可复现的"最佳点"，鲁棒性显著提高且不影响甚至有时提升干净性能。
...
### [88] [An Investigation of Memorization Risk in Healthcare Foundation Models](https://arxiv.org/abs/2510.12950)
*Sana Tonekaboni,Lena Stempfle,Adibvafa Fallahpour,Walter Gerych,Marzyeh Ghassemi*

Main category: cs.LG

TL;DR: 提出了一个评估电子健康记录基础模型中隐私记忆风险的黑盒测试框架，包括嵌入层和生成层的记忆探测方法，旨在区分模型泛化和有害记忆。


<details>
  <summary>Details</summary>
Motivation: 基于大规模电子健康记录训练的基础模型在临床应用中存在隐私风险，特别是可能记忆患者信息，这引发了重要的隐私担忧。

Method: 开发了一套黑盒评估测试，包括在嵌入层和生成层探测记忆的方法，并发布了开源工具包用于医疗AI的隐私评估。

Result: 在公开可用的电子健康记录基础模型上验证了该方法，能够有效评估隐私相关的记忆风险。

Conclusion: 该框架有助于识别和减轻基础模型在医疗应用中可能存在的隐私风险，特别是对弱势群体的保护。
...
### [89] [A Multimodal XAI Framework for Trustworthy CNNs and Bias Detection in Deep Representation Learning](https://arxiv.org/abs/2510.12957)
...
Result: 在MNIST多模态扩展数据集上达到93.2%分类准确率、91.6% F1分数和78.1%解释保真度（IoU-XAI），优于单模态和非可解释基线。消融研究表明，将可解释性与偏见感知学习相结合能增强鲁棒性和人类对齐。

Conclusion: 我们的工作弥合了性能、透明度和公平性之间的差距，为敏感领域中的可信AI提供了一条实用路径。
...
### [90] [Balancing Performance and Reject Inclusion: A Novel Confident Inlier Extrapolation Framework for Credit Scoring](https://arxiv.org/abs/2510.12967)
*Athyrson Machado Ribeiro,Marcos Medeiros Raimundo*

Main category: cs.LG

TL;DR: 提出了一种新的自信内点外推框架(CI-EX)来处理信用评分中的拒绝推断问题，通过迭代识别被拒绝客户的分布并基于概率分配标签，在保持AUC竞争力的同时显著提升拒绝推断特定指标。


<details>
  <summary>Details</summary>
Motivation: 传统拒绝推断方法假设被拒绝客户的行为可以从已接受客户中推断，但忽略了两个群体之间可能存在的分布差异，导致盲目外推问题。

Method: CI-EX框架迭代使用异常检测模型识别被拒绝客户样本的分布，然后基于监督分类模型的概率为最接近已接受群体分布的个体分配标签。

Result: 在两个大型真实信用数据集上的实验表明，CI-EX在拒绝推断特定指标上持续优于现有方法，同时在大多数实验中保持AUC的竞争力。

Conclusion: 拒绝推断方法通常在AUC和拒绝推断特定指标之间存在权衡，但提出的CI-EX框架在保持AUC竞争力的同时显著提升了拒绝推断特定指标的性能。
...
### [91] [A Connection Between Score Matching and Local Intrinsic Dimension](https://arxiv.org/abs/2510.12975)
...
<details>
  <summary>Details</summary>
Motivation: 现有LID估计方法需要多次前向传播或梯度计算，在计算和内存受限场景下应用受限。

Method: 通过理论证明LID是去噪分数匹配损失的下界，从而将其作为LID估计器，并与隐式分数匹配损失和FLIPD方法建立联系。

Result: 在流形基准和Stable Diffusion 3.5上的实验表明，该方法在准确性和内存占用方面优于现有方法。
...
### [92] [Reference-Specific Unlearning Metrics Can Hide the Truth: A Reality Check](https://arxiv.org/abs/2510.12981)
*Sungjun Cho,Dasol Hwang,Frederic Sala,Sangheum Hwang,Kyunghyun Cho,Sungmin Cha*

Main category: cs.LG

TL;DR: 提出了FADE指标，通过比较未学习模型与参考模型在生成样本上的双向似然分配来评估分布相似性，解决了现有遗忘指标依赖特定参考的局限性


<details>
  <summary>Details</summary>
Motivation: 当前生成模型的遗忘指标基于参考响应或分类器输出评估，而非评估核心目标：遗忘模型是否与从未见过不需要数据的模型行为不可区分。这种参考特定方法存在系统性盲点

Method: 提出功能性对齐分布等价(FADE)指标，通过比较未学习模型和参考模型在生成样本上的双向似然分配来测量分布相似性

Result: 在TOFU和UnlearnCanvas基准测试中，传统指标接近最优的方法未能实现分布等价，许多方法在遗忘后与黄金标准距离更远
...
### [93] [CSI-4CAST: A Hybrid Deep Learning Model for CSI Prediction with Comprehensive Robustness and Generalization Testing](https://arxiv.org/abs/2510.12996)
...
TL;DR: CSI-4CAST是一种混合深度学习架构，用于大规模MIMO系统中的CSI预测，通过集成CNN残差、自适应校正层、ShuffleNet块和Transformer来有效捕获局部和长程依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在应对实际非高斯噪声、跨不同信道条件的泛化能力和计算效率方面存在局限性，需要更鲁棒和高效的CSI预测方案。

Method: 提出混合深度学习架构CSI-4CAST，整合了卷积神经网络残差、自适应校正层、ShuffleNet块和Transformer四个关键组件，并构建了包含30多万样本的CSI-RRG基准数据集。

Result: CSI-4CAST在88.9%的TDD场景和43.8%的FDD场景中优于基线模型，计算量比最强基线LLM4CP减少5倍和3倍，同时实现了更高的预测精度。

Conclusion: 该工作不仅提出了高效的CSI预测模型，还建立了标准化基准CSI-RRG，为研究鲁棒高效的CSI预测提供了重要工具和洞察。
...
### [94] [Max It or Miss It: Benchmarking LLM On Solving Extremal Problems](https://arxiv.org/abs/2510.12997)
*Binxin Gao,Jingjun Han*

Main category: cs.LG

TL;DR: 论文提出ExtremBench基准测试，评估LLMs在极值问题求解方面的推理能力，发现现有数学基准测试可能无法全面评估数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs推理能力的具体来源和机制，特别是优化推理（在约束条件下寻找极值）这一基础抽象能力，该能力在规划、控制、资源分配等应用中至关重要。

Method: 引入ExtremBench基准数据集，包含93个标准化的极值求解问题，源自中国数学奥林匹克不等式练习。对Qwen3、GPT-OSS、DeepSeek等开源模型进行广泛评估。

Result: 发现LLMs的极值求解推理能力与当前数学基准测试（如AIME25和MATH-500）的能力并不完全一致，有些模型在一般数学推理上表现良好但在极值求解上较差，反之亦然。

Conclusion: 现有基准测试实践存在关键差距，可能无法全面捕捉数学推理能力的全谱系，强调了开发更全面评估方法的必要性。
...
### [95] [AMORE: Adaptive Multi-Output Operator Network for Stiff Chemical Kinetics](https://arxiv.org/abs/2510.12999)
*Kamaljyoti Nath,Additi Pandey,Bryan T. Susi,Hessam Babaee,George Em Karniadakis*

Main category: cs.LG

TL;DR: AMORE框架通过自适应多输出算子网络和自适应损失函数，有效解决了刚性系统时间积分中的计算成本问题，能够预测所有热化学状态并满足质量分数约束。


<details>
  <summary>Details</summary>
Motivation: 刚性系统的时间积分是燃烧、高超声速等反应输运系统中的主要计算成本来源。刚性会引入比其他物理过程小得多的时间尺度，需要极小的显式时间步长或计算密集的隐式方法。因此，需要策略来缓解刚性带来的挑战。

Method: 开发AMORE框架，包含能够预测多个输出的算子网络和确保可靠算子学习的自适应损失函数。提出两种自适应损失函数，考虑每个状态变量和样本的误差来惩罚损失函数。设计自动满足单位分解的trunk网络，并提出可逆解析映射来精确执行质量分数约束。采用两步训练方法，并扩展自适应损失函数用于trunk和branch训练。

Result: 通过两个示例验证了模型的有效性和适用性：合成气（12个状态）和GRI-Mech 3.0（54个状态中的24个活跃状态）。提出的DeepONet将成为未来CFD研究加速湍流燃烧模拟的骨干。

Conclusion: AMORE是一个通用框架，除了DeepONet外，还展示了其在FNO上的应用。该框架能够有效处理刚性系统，为加速湍流燃烧模拟提供了有力工具。
...
### [96] [Escaping Local Optima in the Waddington Landscape: A Multi-Stage TRPO-PPO Approach for Single-Cell Perturbation Analysis](https://arxiv.org/abs/2510.13018)
...
Method: 采用多阶段强化学习：第一阶段使用Fisher向量积和共轭梯度求解器计算显式自然梯度更新，通过KL信任域约束提供安全的曲率感知初始策略；第二阶段应用带裁剪代理的PPO，利用小批量效率优化策略

Result: 该初始化方法显著提高了在单细胞RNA测序和单细胞ATAC测序扰动分析中的泛化能力

Conclusion: 多阶段强化学习算法通过精心设计的初始化策略，能够有效避免局部最优，收敛到正确的细胞谱系，为单细胞扰动建模提供了新的解决方案
...
### [97] [Machine Learning-Based Ultrasonic Weld Characterization Using Hierarchical Wave Modeling and Diffusion-Driven Distribution Alignment](https://arxiv.org/abs/2510.13023)
*Joshua R. Tempelman,Adam J. Wachtor,Eric B. Flynn*

Main category: cs.LG

TL;DR: 提出了一个端到端的机器学习工作流程，用于在工业环境中进行自动化超声焊缝检测，解决了数据稀缺和信号损坏的挑战。


<details>
  <summary>Details</summary>
Motivation: 自动化超声焊缝检测面临训练数据有限和环境干扰导致信号损坏的挑战，在工业环境中实现端到端机器学习工作流程一直难以实现。

Method: 采用降阶建模方案、基于扩散的分布对齐和U-Net分割反演。使用基于Lamb波理论的降阶Helmholtz模型生成数据集，通过迁移学习使用有限3D弹性动力学模拟进行精炼，使用引导扩散处理OOD实验数据。

Result: 开发了一个集成框架，为真实数据上的自动化焊缝检测提供端到端解决方案。

Conclusion: 该工作流程成功解决了数据获取和信号处理的关键挑战，实现了工业环境下可靠的焊缝检测。
...
### [98] [Information Shapes Koopman Representation](https://arxiv.org/abs/2510.13025)
*Xiaoyuan Cheng,Wenxuan Yuan,Yiming Yang,Yuanzhao Zhang,Sibo Cheng,Yi He,Zhuo Sun*

Main category: cs.LG

TL;DR: 本文通过信息论视角重新审视Koopman学习，提出了一种平衡表示简洁性和表达性的信息论拉格朗日公式，解决了传统方法中潜在空间崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: Koopman算子的无限维特性使得寻找合适的有限维子空间具有挑战性，特别是在深度架构中。现有方法在表示学习中存在次优问题，难以平衡简洁性和表达性之间的张力。

Method: 提出基于信息瓶颈理论的信息论拉格朗日公式，利用潜在互信息促进简洁性，使用冯·诺依曼熵维持表达性以防止模式崩溃。开发了相应的算法来平衡这一权衡。

Result: 在多种动力系统上的验证表明，该方法相比现有Koopman学习方法性能有所提升，学习到的表示更加稳定和可解释，可视化结果与理论预测一致。

Conclusion: 通过信息论框架平衡简洁性和表达性，可以获得更稳定和可解释的Koopman表示，为动力系统建模提供了新的有效方法。
...
### [99] [Bridging Idealized and Operational Models: An Explainable AI Framework for Earth System Emulators](https://arxiv.org/abs/2510.13030)
...
<details>
  <summary>Details</summary>
Motivation: 现有高分辨率业务模型存在持续偏差，特别是在极端事件和统计分布模拟方面，而理想化模型虽然能精确描述特定动力学特征，但不同模型之间存在学科壁垒。
...
Conclusion: 该框架提供了物理洞察理解，超越了黑盒修正，在计算高效的框架中实现了有效的物理辅助数字孪生和不确定性量化，强调了推进理想化模型发展和建模社区间交流的重要性。
...
### [100] [Randomness and Interpolation Improve Gradient Descent](https://arxiv.org/abs/2510.13040)
...
Method: IAGD使用二阶牛顿插值加速梯度下降收敛过程，假设迭代间梯度相关；NRSGD在优化过程中引入受控噪声进行正则化以防止过拟合。

Result: 在CIFAR-10和CIFAR-100数据集上的比较实验显示，IAGD和NRSGD在多种CNN模型上表现优于Keras包中的经典优化器。

Conclusion: IAGD和NRSGD是SGD的有效改进方法，证明了在加速收敛和正则化方面的技术进步的有效性。
...
### [101] [An Operational Deep Learning System for Satellite-Based High-Resolution Global Nowcasting](https://arxiv.org/abs/2510.13050)
...
TL;DR: Global MetNet是一个全球机器学习降水临近预报模型，利用卫星和NWP数据为全球南方等雷达稀疏地区提供高分辨率降水预报，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 解决全球南方地区因雷达覆盖稀疏而无法获得准确降水临近预报的问题，传统NWP方法延迟高、分辨率低，现有机器学习方法无法适用于雷达稀疏地区。

Method: 利用全球降水任务CORRA数据集、地球静止卫星数据和全球NWP数据，构建机器学习模型预测未来12小时降水，空间分辨率约5公里，时间分辨率15分钟。

Result: 模型在数据稀疏区域表现优于美国的高分辨率NWP模型，在所有降水率和预报时效的关键指标上均有显著提升，预报生成时间不到1分钟。

Conclusion: 该模型已在Google搜索中部署，为全球数百万用户提供服务，是减少全球预报质量差异和将稀疏高分辨率卫星观测整合到天气预报中的重要一步。
...
### [102] [Time-Varying Optimization for Streaming Data Via Temporal Weighting](https://arxiv.org/abs/2510.13052)
...
Result: 均匀权重下跟踪误差以O(1/t)速率渐近消失，折扣权重会产生由折扣因子和每时间步梯度更新次数控制的非零误差下限。

Conclusion: 理论分析揭示了不同权重策略对跟踪误差的影响，数值模拟验证了理论发现，为流数据学习提供了时变优化视角的见解。
...
### [103] [Achieving Logarithmic Regret in KL-Regularized Zero-Sum Markov Games](https://arxiv.org/abs/2510.13060)
*Anupam Nayak,Tong Yang,Osman Yagan,Gauri Joshi,Yuejie Chi*

Main category: cs.LG

TL;DR: 该论文研究了KL正则化在博弈论环境中的理论优势，提出了OMG和SOMG算法，在矩阵博弈和马尔可夫博弈中实现了与KL正则化强度β成反比的对数级遗憾。
...
Method: 提出了OMG算法（基于最佳响应采样和乐观奖励）用于矩阵博弈，以及SOMG算法（基于最佳响应采样和超乐观奖励）用于马尔可夫博弈。

Result: 两种算法都实现了与KL正则化强度β成反比的对数级遗憾，同时保持了标准的不依赖β的√T遗憾。
...
### [104] [Absolute indices for determining compactness, separability and number of clusters](https://arxiv.org/abs/2510.13065)
...
Method: 定义每个聚类的紧密度函数和聚类对之间的邻域点集，用于计算紧密度和分离度指数

Result: 在合成和真实数据集上验证了新指数的性能，并与广泛使用的聚类有效性指标进行比较

Conclusion: 提出的绝对聚类指数能有效评估聚类质量并确定最优聚类数量
...
### [105] [NeuroRVQ: Multi-Scale EEG Tokenization for Generative Large Brainwave Models](https://arxiv.org/abs/2510.13068)
...
TL;DR: NeuroRVQ是一种基于码本的脑电信号大型模型，通过多尺度特征提取和分层残差向量量化技术，解决了现有脑电tokenizer无法保留高频动态信息的问题，实现了高保真度的脑电信号重建。


<details>
  <summary>Details</summary>
Motivation: 现有脑电基础模型的性能受限于其信号tokenization模块，这些神经tokenizer无法保留高频动态信息，限制了脑电信号的重建保真度。

Method: 提出NeuroRVQ模型，包含：(i)多尺度特征提取模块捕获全频段神经频谱；(ii)分层残差向量量化码本进行高分辨率编码；(iii)基于脑电信号相位和幅度的损失函数进行高效训练。

Result: NeuroRVQ实现了更低的重建误差，并在多种下游任务中优于现有大型脑电模型。
...
### [106] [Transformer-based Scalable Beamforming Optimization via Deep Residual Learning](https://arxiv.org/abs/2510.13077)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大规模多用户MISO系统中波束成形的计算复杂度问题，特别是在动态环境中需要快速响应的场景，传统优化方法如WMMSE计算成本高，难以实时应用。

Method: 采用学习优化范式，使用多层Transformer通过残差连接迭代优化信道和波束成形器特征，并引入课程学习、半摊销学习和滑动窗口训练三种策略来增强训练效果。

Result: 仿真结果表明，该方案在低到中等信噪比下优于现有基线方法，在高信噪比下接近WMMSE性能，同时推理速度显著快于迭代和在线学习方法。

Conclusion: 该无监督深度学习框架为大规模MU-MISO系统的波束成形提供了一种高效解决方案，在性能和计算效率之间取得了良好平衡。
...
### [107] [DeepCausalMMM: A Deep Learning Framework for Marketing Mix Modeling with Causal Inference](https://arxiv.org/abs/2510.13087)
...
Method: 使用门控循环单元(GRU)自动学习时间模式(如广告滞留效应和滞后)，通过有向无环图(DAG)学习营销渠道间的统计依赖和因果结构，并实现基于Hill方程的饱和曲线建模。

Result: 该包提供数据驱动的设计、多区域建模、鲁棒统计方法、全面的响应曲线分析和14+交互式可视化仪表板等关键创新功能。

Conclusion: DeepCausalMMM通过结合深度学习、因果推断和先进营销科学，解决了传统营销组合建模的局限性，能够更好地估计营销活动对业务结果的影响。
...
### [108] [Neural Triangular Transport Maps: A New Approach Towards Sampling in Lattice QCD](https://arxiv.org/abs/2510.13112)
*Andrey Bryutkin,Youssef Marzouk*

Main category: cs.LG

TL;DR: 提出了稀疏三角传输映射方法，利用周期性边界条件下晶格图的条件独立结构，通过单调整流神经网络实现高效的玻尔兹曼分布采样，解决了传统方法在大规模晶格中的内存和计算复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 晶格场理论是计算物理的重要测试平台，但采样其玻尔兹曼分布面临多模态和长程相关性的挑战。归一化流方法虽然前景广阔，但在大晶格应用中受到内存需求过高和模型表达能力不足的限制。

Method: 使用稀疏三角传输映射，利用晶格图在周期性边界条件下的条件独立结构，通过单调整流神经网络实现。该方法在精确稀疏性（尊重目标分布的边际条件独立性）和近似稀疏性（无填充的计算可处理性）之间取得平衡。

Result: 将每个三角映射组件限制在局部过去，实现了站点并行评估和与晶格大小N成线性关系的时间复杂度，同时保持了表达性和可逆结构。在二维φ^4模型中的实验验证了节点标记对稀疏性和性能的影响。

Conclusion: 该方法在计算效率和表达能力之间取得了良好平衡，为大规模晶格场理论的玻尔兹曼分布采样提供了有效解决方案，相比混合蒙特卡洛和RealNVP等现有方法具有优势。
...
### [109] [On the Reasoning Abilities of Masked Diffusion Language Models](https://arxiv.org/abs/2510.13117)
*Anej Svete,Ashish Sabharwal*

Main category: cs.LG

TL;DR: 该论文证明了在有限精度对数宽度设置下，掩码扩散模型（MDMs）与多项式填充循环变换器（PLTs）等价，且能解决所有CoT增强变换器能解决的问题。MDMs在某些问题上（如正则语言）比CoT变换器更高效，并行生成能显著加速推理。


<details>
  <summary>Details</summary>
Motivation: 探索掩码扩散模型的计算能力和并行性限制，与传统自回归语言模型相比，MDMs的并行生成虽然高效，但其推理能力和效率边界尚未充分研究。

Method: 通过将MDMs与链式思维（CoT）和填充循环变换器（PLTs）在有限精度对数宽度设置下建立理论联系，证明MDMs与多项式填充PLTs的等价性。

Result: 证明了MDMs能解决所有CoT增强变换器能解决的问题，并在某些问题类别（如正则语言）上比CoT变换器更高效，并行生成带来显著的推理加速。

Conclusion: 掩码扩散模型在理论上具有强大的推理能力，其并行特性在某些问题类型上能提供比传统自回归模型更高效的解决方案。
...
### [110] [Cluster-Based Client Selection for Dependent Multi-Task Federated Learning in Edge Computing](https://arxiv.org/abs/2510.13132)
*Jieping Luo,Qiyue Li,Zhizhang Liu,Hang Qi,Jiaying Yin,Jingjin Wu*

Main category: cs.LG

TL;DR: 提出了CoDa-FL框架，通过集群化客户端选择和依赖感知任务分配来减少联邦学习在移动边缘计算环境中的总完成时间


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习在移动边缘计算环境下，特别是在依赖多任务设置中的客户端选择问题，以减少完成各种学习任务所需的总时间

Method: 使用基于地球移动距离的客户端聚类来降低计算成本和提升通信效率，结合有向无环图的任务调度机制来管理任务依赖关系

Result: 数值实验验证CoDa-FL在异构MEC设置下实现了更快的收敛速度、更低的通信和计算成本以及更高的学习精度

Conclusion: CoDa-FL框架通过集群导向和依赖感知的方法，在联邦学习中有效减少了总完成时间，优于现有基准方法
...
### [111] [Convergence, design and training of continuous-time dropout as a random batch method](https://arxiv.org/abs/2510.13134)
*Antonio Álvarez-López,Martín Hernández*

Main category: cs.LG

TL;DR: 本文通过随机批次方法研究连续时间模型中的dropout正则化，构建了无偏估计器，建立了轨迹和分布层面的收敛性，分析了训练过程中的偏差，并验证了理论在分类和流匹配任务上的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究连续时间模型中dropout正则化的理论基础和实际效果，通过随机批次方法降低计算成本并保持正则化效果。

Method: 使用随机批次方法构建无偏估计器，在时间间隔h内对神经元进行批次采样，建立轨迹收敛性和分布稳定性分析，并进行伴随分析和梯度下降迭代偏差分析。

Result: 建立了轨迹层面的线性收敛率，分布层面的总变差误差为h^1/2阶，推导了成本-精度权衡关系，在单层神经ODE上验证了理论预测的收敛率和正则化效果。

Conclusion: 随机批次方法为连续时间dropout提供了理论基础，实现了计算效率与正则化效果的平衡，在分类和流匹配任务中表现出良好的运行时间和内存特性。
...
### [112] [Behavioral Embeddings of Programs: A Quasi-Dynamic Approach for Optimization Prediction](https://arxiv.org/abs/2510.13158)
...
<details>
  <summary>Details</summary>
Motivation: 现有程序表示方法存在两难：静态表示高效但无法洞察程序在复杂代码变换下的行为，动态表示深入但开销大且非确定性。需要一种能够结合两者优势的新方法。

Method: 引入程序行为谱表示，通过用多样化优化序列探测程序的IR并量化静态特征变化。使用产品量化将连续反应向量离散化为结构化子词，并用多任务Transformer模型PQ-BERT学习这些行为代码的深度上下文语法。

Result: 在两个代表性编译器优化任务（最佳传递预测和-Oz收益预测）上的综合实验表明，该方法优于最先进的静态基线方法。

Conclusion: 提出的准动态框架成功超越了传统程序表示方法的权衡，为编译器优化提供了更有效的程序表示。
...
### [113] [Universally Invariant Learning in Equivariant GNNs](https://arxiv.org/abs/2510.13169)
*Jiacheng Cen,Anyi Li,Ning Lin,Tingyang Xu,Yu Rong,Deli Zhao,Zihe Wang,Wenbing Huang*

Main category: cs.LG

TL;DR: 提出了一种高效实用的构建完全等变图神经网络的理论框架，通过规范形式和满秩可转向基集实现完全性，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有等变GNN通过更深架构、增强体阶或增加可转向特征维度来实现完全性，但计算成本高且缺乏多项式时间解决方案。

Method: 基于两个关键组件构建完全等变GNN：1）完整标量函数（几何图的规范形式）；2）满秩可转向基集。在EGNN和TFN模型基础上提出高效算法。

Result: 实证结果显示模型具有优越的完全性，仅需少量层数即可实现优秀性能，显著降低计算开销。

Conclusion: 该框架为构建完全等变GNN提供了理论支撑和实用方法，在保持强大实际效能的同时大幅提升计算效率。
...
### [114] [Information-Theoretic Criteria for Knowledge Distillation in Multimodal Learning](https://arxiv.org/abs/2510.13182)
...
<details>
  <summary>Details</summary>
Motivation: 跨模态知识蒸馏技术虽然应用广泛，但缺乏理论指导，导致效果不稳定。需要建立理论框架来解释和指导实践。

Method: 提出了跨模态互补性假设(CCH)，在联合高斯模型中进行理论验证，并在多种多模态数据集上进行实证检验。

Result: 理论验证和实证研究都支持CCH假设，为跨模态知识蒸馏提供了理论依据。

Conclusion: 建立了一个新的理论框架来理解跨模态知识蒸馏，并基于CCH准则提供了选择最优教师模态的实用指南。
...
### [115] [CleverCatch: A Knowledge-Guided Weak Supervision Model for Fraud Detection](https://arxiv.org/abs/2510.13205)
...
<details>
  <summary>Details</summary>
Motivation: 医疗欺诈检测面临标注数据稀缺、欺诈手段不断演变和医疗记录高维度等挑战。传统监督方法受限于极端标签稀缺，而纯无监督方法往往无法捕捉临床意义上的异常。

Method: 将结构化领域专业知识整合到神经网络架构中，在共享嵌入空间中对齐规则和数据样本。通过联合训练编码器学习软规则嵌入，使其能够泛化到复杂的真实世界数据集。

Result: 在大规模真实世界数据集上的实验表明，CleverCatch优于四种最先进的异常检测基线方法，在AUC和召回率上分别平均提升1.3%和3.4%。消融研究进一步验证了专家规则的补充作用。
...
### [116] [Performance Evaluation of Ising and QUBO Variable Encodings in Boltzmann Machine Learning](https://arxiv.org/abs/2510.13210)
*Yasushi Hasegawa,Masayuki Ohzeki*

Main category: cs.LG

TL;DR: 比较Ising和QUBO编码在玻尔兹曼机学习中的表现，发现QUBO编码导致Fisher信息矩阵条件数更差，使得SGD收敛更慢，而自然梯度下降对两种编码表现相似。


<details>
  <summary>Details</summary>
Motivation: 研究不同变量编码（Ising的{-1,+1} vs QUBO的{0,1}）如何影响玻尔兹曼机学习的信息几何和收敛动态。

Method: 在固定模型、采样器和步长的受控协议下，通过可视化经验矩和Fisher信息矩阵来分析两种编码的差异。

Result: QUBO编码产生更大的交叉项，导致Fisher信息矩阵有更多小特征值方向，降低了谱熵，使得SGD收敛更慢。自然梯度下降对两种编码收敛相似。
...
### [117] [Towards Understanding Valuable Preference Data for Large Language Model Alignment](https://arxiv.org/abs/2510.13212)
*Zizhuo Zhang,Qizhou Wang,Shanshan Ye,Jianing Zhu,Jiangchao Yao,Bo Han,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 本文提出了一种基于截断影响函数(TIF)的数据质量评估方法，发现偏好数据质量是模型相关的，并开发了更简单的评分函数来改进偏好数据选择。
...
Method: 提出截断影响函数(TIF)评估数据质量，开发两种计算更简单的评分函数(SF)，并将它们组合以抵消不同的误差源，形成有效的数据选择规则。

Result: 在多样化对齐基准和多种LLM家族上的实验表明，使用更少数据可以实现更好的对齐性能，证明了方法的通用性。
...
### [118] [Rethinking Graph Domain Adaptation: A Spectral Contrastive Perspective](https://arxiv.org/abs/2510.13254)
*Haoyu Zhang,Yuxuan Cheng,Wenqi Fan,Yulong Chen,Yifan Zhang*

Main category: cs.LG

TL;DR: 提出了FracNet，一种基于频率感知的对比图神经网络，通过谱分析将图分解为高低频分量，分别处理全局和局部模式，以解决图神经网络在领域适应中的结构分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 传统图神经网络在领域适应中表现不佳，主要原因是未能区分处理全局和局部模式，导致多层GNN后局部细节被破坏。通过谱分析发现低频分量编码域不变全局模式，高频分量捕获域特定局部细节。

Method: FracNet包含两个协同模块：将原始图分解为高低频分量，并进行频率感知的领域适应。结合对比学习框架改善领域适应的模糊边界问题。
...
### [119] [Hypernetworks for Perspectivist Adaptation](https://arxiv.org/abs/2510.13259)
*Daniil Ignatev,Denis Paperno,Massimo Poesio*

Main category: cs.LG

TL;DR: 应用超网络+适配器架构进行视角感知分类，在仇恨言论和毒性检测任务中达到专业模型性能，同时显著减少参数数量。
...
Method: 采用超网络+适配器的现有架构组合，应用于视角主义分类任务。

Result: 该解决方案能够在仇恨言论和毒性检测任务中与专业模型竞争，同时使用显著更少的参数。

Conclusion: 提出的解决方案是架构无关的，可以即插即用地应用于广泛的基模型。
...
### [120] [BlendFL: Blended Federated Learning for Handling Multimodal Data Heterogeneity](https://arxiv.org/abs/2510.13266)
*Alejandro Guerra-Manzanares,Omar El-Herraoui,Michail Maniatakos,Farah E. Shamout*

Main category: cs.LG

TL;DR: BlendFL是一个新颖的联邦学习框架，通过融合横向和纵向联邦学习的原理，解决了多模态数据异构性问题，支持客户端根据本地数据集灵活选择训练方式，并采用去中心化推理机制。


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中多模态数据异构性挑战，现有横向和纵向联邦学习框架在非理想设置下效果有限，无法处理客户端间模态和样本不完整分布的情况。

Method: 提出BlendFL框架，同步融合横向和纵向联邦学习原则，支持客户端根据可用数据集灵活选择训练方式；引入去中心化推理机制；提出BlendAvg自适应全局模型聚合策略。

Result: 在三个分类任务上使用大规模真实多模态医疗数据集和流行多模态基准进行评估，BlendFL在多模态和单模态分类上均表现优异，消融研究显示其收敛速度更快。
...
### [121] [To Steer or Not to Steer? Mechanistic Error Reduction with Abstention for Language Models](https://arxiv.org/abs/2510.13290)
*Anna Hedström,Salim I. Amoukou,Tom Bewley,Saumitra Mishra,Manuela Veloso*

Main category: cs.LG

TL;DR: MERA是一个通过选择性干预来减少语言模型错误的框架，它优化干预方向和校准干预时机与强度，避免过度或不足干预。
...
Method: MERA框架通过(i)优化干预方向，(ii)校准何时干预及干预强度，在无法自信修正时选择弃权，从而安全有效地减少错误。

Result: 在多个数据集和语言模型家族上的实验表明，MERA能安全、有效、非退化地纠正错误，且优于现有基线方法。
...
### [122] [Federated Conditional Conformal Prediction via Generative Models](https://arxiv.org/abs/2510.13297)
*Rui Xu,Sihong Xie*
...
<details>
  <summary>Details</summary>
Motivation: 标准保形预测假设数据独立同分布，但在联邦学习中客户端数据分布差异很大，现有方法只能保证边际覆盖，无法反映输入条件的不确定性
...
### [123] [Km-scale dynamical downscaling through conformalized latent diffusion models](https://arxiv.org/abs/2510.13301)
...
<details>
  <summary>Details</summary>
Motivation: 生成扩散模型在动态降尺度任务中虽然具有重建保真度和可扩展采样优势，但缺乏有限样本保证，导致网格点级不确定性估计校准不佳，影响在业务应用中的可靠性。

Method: 在降尺度流程中增强保形预测框架，对扩散模型的样本进行后处理得到条件分位数估计，然后采用保形化分位数回归方法构建局部自适应预测区间。

Result: 在意大利ERA5再分析数据上的实验表明，该方法相比扩散模型基线显著改善了网格点级不确定性估计的覆盖率和概率评分稳定性。

Conclusion: 保形化生成模型在构建高分辨率气象场的可信概率降尺度方面具有重要潜力。
...
### [124] [Isolation-based Spherical Ensemble Representations for Anomaly Detection](https://arxiv.org/abs/2510.13311)
*Yang Cao,Sikun Yang,Hao Tian,Kai He,Lianyong Qi,Ming Liu,Yujiu Yang*

Main category: cs.LG

TL;DR: 提出ISER方法，通过使用超球体半径作为局部密度特征的代理，扩展了现有的基于隔离的异常检测方法，保持了线性时间和常数空间复杂度。
...
Conclusion: ISER通过使用超球体半径作为密度代理，有效解决了现有基于隔离方法的局限性，提高了异常检测的准确性和效率。
...
### [125] [RockNet: Distributed Learning on Ultra-Low-Power Devices](https://arxiv.org/abs/2510.13320)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习在信息物理系统中的广泛应用，需要从云端训练转向设备端训练(TinyML)，但超低功耗微控制器的有限计算资源使训练变得困难。

Method: 设计分布式学习方法，利用多个设备进行分布式训练，结合专门的计算高效分类器和定制的无线多跳通信协议，最小化通信开销。

Result: 在20个超低功耗设备的测试平台上，RockNet成功从零开始学习时间序列分类任务，准确率比最新方法提升2倍，分布式架构使内存、延迟和能耗降低90%。
...
### [126] [When In Doubt, Abstain: The Impact of Abstention on Strategic Classification](https://arxiv.org/abs/2510.13327)
*Lina Alkarmi,Ziyuan Huang,Mingyan Liu*

Main category: cs.LG

TL;DR: 该论文研究在战略分类中引入弃权机制的影响，证明最优弃权策略能确保决策者的效用不低于非弃权设置，同时还能作为操纵威慑手段。


<details>
  <summary>Details</summary>
Motivation: 算法决策系统易受战略操纵，而分类器弃权已被证明能提高准确性。本文旨在探索在战略分类背景下引入弃权机制如何影响战略代理的响应以及决策者应如何最优利用这一机制。

Method: 使用Stackelberg博弈模型，决策者作为领导者首先宣布决策策略，战略代理作为跟随者随后操纵其特征以获得期望结果。专注于二元分类器，代理操纵可观察特征而非真实特征。

Result: 最优弃权策略确保决策者的效用在战略代理存在时不低于非弃权设置。弃权不仅能提高准确性，还能作为操纵威慑，使代理（尤其是资质较差的代理）在操纵成本足够高时更难通过操纵获得积极结果。

Conclusion: 弃权是减少算法决策系统中战略行为负面影响的宝贵工具，既能提高决策准确性，又能有效威慑操纵行为。
...
### [127] [Thompson Sampling via Fine-Tuning of LLMs](https://arxiv.org/abs/2510.13328)
*Nicolas Menet,Aleksandar Terzić,Andreas Krause,Abbas Rahimi*

Main category: cs.LG

TL;DR: 提出ToSFiT方法，通过微调大型语言模型实现Thompson采样，避免在大规模离散空间中优化获取函数的计算成本，在多个任务上验证了其有效性。
...
Method: 基于Thompson采样的ToSFiT方法，通过直接参数化候选解产生最大奖励的概率，利用提示条件化的大型语言模型的先验知识，并逐步适应后验分布。

Result: 在FAQ响应优化、热稳定蛋白质搜索和量子电路设计三个任务上验证，在线微调显著提高了样本效率，对计算效率影响可忽略。

Conclusion: 提出的变分Thompson采样方法具有与标准方法相同的理论保证，通过仔细适应最大后验概率的原则支撑了ToSFiT算法的有效性。
...
### [128] [Kernel Representation and Similarity Measure for Incomplete Data](https://arxiv.org/abs/2510.13352)
...
### [129] [Generalist++: A Meta-learning Framework for Mitigating Trade-off in Adversarial Training](https://arxiv.org/abs/2510.13361)
*Yisen Wang,Yichuan Mo,Hongjun Wang,Junyi Li,Zhouchen Lin*

Main category: cs.LG

TL;DR: 提出Generalist框架，通过将整体泛化目标分解为多个子任务，每个子任务由专门的基学习器处理，然后通过参数插值和周期性重分配来缓解对抗训练中的自然精度下降和跨攻击鲁棒性差的问题。
...
Method: 将整体泛化目标分解为多个子任务，每个子任务由专门的基学习器处理；在训练后期通过参数插值形成全局学习器，并周期性将全局参数重分配给基学习器以防止优化轨迹偏离共享目标。
...
### [130] [A New Perspective on Transformers in Online Reinforcement Learning for Continuous Control](https://arxiv.org/abs/2510.13367)
*Nikita Kachaev,Daniil Zelezetsky,Egor Cherepanov,Alexey K. Kovelev,Aleksandr I. Panov*

Main category: cs.LG

TL;DR: 本文研究表明transformer可以在在线无模型强化学习中成为连续控制的强基线，探讨了输入条件、组件共享和序列数据处理等关键设计问题。
...
Method: 研究关键设计问题：如何条件化输入、在actor和critic之间共享组件、以及如何切片序列数据进行训练，提出了稳定的架构和训练策略。

Result: 实验表明该方法在完全和部分可观测任务中都能达到有竞争力的性能，适用于向量和图像两种设置。
...
### [131] [Contrastive Learning-Based Dependency Modeling for Anomaly Detection in Cloud Services](https://arxiv.org/abs/2510.13368)
*Yue Xing,Yingnan Deng,Heyao Liu,Ming Wang,Yun Zi,Xiaoxuan Sun*

Main category: cs.LG

TL;DR: 提出了一种结合对比学习的依赖建模和异常检测方法，通过构建依赖图、提取时空特征，使用对比学习框架增强正常和异常模式的可分性，在云服务环境中实现稳定可靠的异常检测。
...
### [132] [Prediction Markets with Intermittent Contributions](https://arxiv.org/abs/2510.13385)
...
<details>
  <summary>Details</summary>
Motivation: 尽管数据可用性和准确预测需求增加，但利益相关者之间的协作常受数据所有权和竞争利益限制。现有合作博弈框架存在局限，需要更通用的解决方案。

Method: 设计预测市场机制，使用稳健回归模型学习最优预测组合并处理缺失提交，引入考虑样本内外表现的收益分配机制，满足多项经济特性。

Result: 通过模拟和真实数据案例研究，证明了所提市场设计的有效性和适应性。

Conclusion: 该预测市场框架能够有效促进利益相关者之间的协作预测，同时保护数据隐私和竞争利益。
...
### [133] [Going with the Flow: Approximating Banzhaf Values via Graph Neural Networks](https://arxiv.org/abs/2510.13391)
*Benjamin Kempinski,Tal Kachman*

Main category: cs.LG

TL;DR: 提出了一种基于图神经网络(GNN)的新方法，用于近似计算网络流博弈中的Banzhaf值，相比传统方法实现了数量级的速度提升，并展示了强大的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在网络流博弈中计算Banzhaf值对于量化多智能体系统中的智能体影响力至关重要，但精确计算在超过约20个智能体时由于指数复杂度而不可行。蒙特卡洛采样方法存在高样本复杂性和无法在不同网络配置间迁移知识的问题。

Method: 将问题构建为图级预测任务，使用图神经网络(GNN)直接从网络拓扑和控制结构中学习智能体影响力的通用模式。比较了三种最先进的GNN架构：图注意力网络(GAT)、带边特征的图同构网络(GINE)和EdgeConv。

Result: 训练后的GNN模型实现了高保真度的Banzhaf值近似，相比精确和基于采样的方法实现了数量级的速度提升。最重要的是，模型展示了强大的零样本泛化能力：在特定大小和拓扑结构的图上训练的模型能够准确预测具有不同结构特性的全新网络的Banzhaf值，无需重新训练。
...
### [134] [Assessing the robustness of heterogeneous treatment effects in survival analysis under informative censoring](https://arxiv.org/abs/2510.13397)
*Yuxin Wang,Dennis Frauen,Jonas Schweisthal,Maresa Schröder,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 提出一个假设宽松的框架，用于评估生存分析中条件平均治疗效应估计在面临删失偏差时的稳健性，通过部分识别推导CATE的边界，并开发具有双重稳健性和准oracle效率的新型元学习器。


<details>
  <summary>Details</summary>
Motivation: 临床研究中高达一半的患者因副作用或其他原因提前退出，当退出是信息性时（即依赖于生存时间），会引入删失偏差，导致治疗效果估计产生偏差。

Method: 使用部分识别方法推导条件平均治疗效应的边界，开发新型元学习器，利用任意机器学习模型估计边界，具有双重稳健性和准oracle效率的理论特性。
...
### [135] [SWIR-LightFusion: Multi-spectral Semantic Fusion of Synthetic SWIR with {Thermal} IR {(LWIR/MWIR)} and RGB](https://arxiv.org/abs/2510.13404)
*Muhammad Ishfaq Hussain,Ma Van Linh,Zubia Naz,Unse Fatima,Yeongmin Ko,Moongu Jeon*

Main category: cs.LG

TL;DR: 提出了一种从LWIR数据合成SWIR样图像的方法，并开发了融合RGB、LWIR和合成SWIR的多模态融合框架，在恶劣能见度条件下提升场景理解能力。
...
Method: 使用对比度增强技术从LWIR数据合成SWIR样图像，构建多模态融合框架，采用优化的编码器-解码器架构和软最大门控融合头。

Result: 在多个公开基准和私有数据集上验证，合成SWIR增强的融合框架提升了融合图像质量（对比度、边缘定义、结构保真度），同时保持实时性能。

Conclusion: 该方法在监视和自主系统应用中具有重要潜力，为恶劣能见度条件下的场景理解提供了有效解决方案。
...
### [136] [Optimizing Storage Overhead of User Behavior Log for ML-embedded Mobile Apps](https://arxiv.org/abs/2510.13405)
*Chen Gong,Yan Zhuang,Zhenzhe Zheng,Yiliu Chen,Sheng Wang,Fan Wu,Guihai Chen*

Main category: cs.LG

TL;DR: AdaLog是一个轻量级自适应系统，通过消除特征级冗余数据和优化存储结构，显著减少移动应用中用户行为日志的存储空间，同时保持模型推理精度和延迟。
...
### [137] [When Embedding Models Meet: Procrustes Bounds and Applications](https://arxiv.org/abs/2510.13406)
*Lucas Maystre,Alvaro Ortega Gonzalez,Charles Park,Rares Dolga,Tudor Berariu,Yu Zhao,Kamil Ciosek*

Main category: cs.LG

TL;DR: 该论文研究了不同嵌入模型之间的对齐问题，提出通过正交变换实现嵌入空间的互操作性，并提供了基于Procrustes后处理的简单对齐方法。


<details>
  <summary>Details</summary>
Motivation: 由于分别训练的相似数据嵌入模型产生的表示虽然编码了稳定信息，但无法直接互换，这在模型重训练、部分模型升级和多模态搜索等实际应用中带来了挑战。

Method: 通过证明如果成对点积近似保持，则存在等距变换可以紧密对齐两个嵌入集，并提供了对齐误差的紧界。据此提出了Procrustes后处理方法，使两个嵌入模型可互操作同时保持各自嵌入空间的几何结构。

Result: 在三个应用中验证了方法的有效性：跨重训练的兼容性维护、不同模型组合用于文本检索、改进混合模态搜索，并在混合模态搜索中达到了最先进的性能。

Conclusion: Procrustes后处理是一种简单有效的嵌入对齐方法，能够实现不同嵌入模型之间的互操作性，同时保持各自的几何特性，在多个实际应用中表现出色。
...
### [138] [Modeling Adoptive Cell Therapy in Bladder Cancer from Sparse Biological Data using PINNs](https://arxiv.org/abs/2510.13431)
*Kayode Olumoyin,Katarzyna Rejniak*

Main category: cs.LG

TL;DR: 提出了一种改进的物理信息神经网络框架，应用于肿瘤学领域，能够从稀疏的肿瘤体积数据中学习组合疗法在肿瘤微环境中的时变相互作用。


<details>
  <summary>Details</summary>
Motivation: 肿瘤学实验数据通常稀疏且仅包含少数时间点的肿瘤体积数据，需要一种能够利用先验动力学系统信息的方法来增强学习能力。

Method: 扩展物理信息神经网络，将观察到的生物约束作为正则化项嵌入，通过嵌入从动力学系统先验信息导出的归纳偏置来改进PINN算法。

Result: 该算法能够推导出ODE模型的解和部分模型参数的时变形式，在间歇性治疗组合疗法的ODE模型中表现出良好的收敛性。

Conclusion: 改进的PINN框架能够在少量训练样本下实现良好泛化，为肿瘤学中的组合疗法动态建模提供了有效工具。
...
### [139] [Hybrid Interval Type-2 Mamdani-TSK Fuzzy System for Regression Analysis](https://arxiv.org/abs/2510.13437)
*Ashish Bhatia,Renato Cordeiro de Amorim,Vito De Feo*

Main category: cs.LG

TL;DR: 提出了一种新颖的模糊回归方法，结合了Mamdani系统的可解释性和TSK模型的精确性，通过混合规则结构和双重主导类型来平衡准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统回归方法在处理现实世界数据的不确定性和模糊性方面存在困难，深度学习虽然能捕捉复杂非线性关系但缺乏可解释性且容易在小数据集上过拟合。模糊系统提供了处理不确定性的框架，但Mamdani和TSK系统在可解释性和准确性之间存在权衡。

Method: 提出混合模糊回归方法，结合Mamdani系统的可解释性和TSK模型的精确性，采用具有模糊和清晰组件的混合规则结构以及双重主导类型。
...
### [140] [Rectify and Align GPS Points to Parking Spots via Rank-1 Constraint](https://arxiv.org/abs/2510.13439)
...
Method: 利用停车位与道路平行的物理约束，提出无监督低秩方法，在统一框架中校正GPS误差并对其到停车位位置。

Result: 广泛的实验证明了该方法在解决实际问题方面的优越性。

Conclusion: 该非常规的校正和对齐方法简单且有效，适用于任何类型的GPS点误差。
...
### [141] [Neural Sum-of-Squares: Certifying the Nonnegativity of Polynomials with Transformers](https://arxiv.org/abs/2510.13444)
*Nico Pelleriti,Christoph Spiegel,Shiwei Liu,David Martínez-Rubio,Max Zimmer,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 本文提出了首个学习增强的算法来验证多项式非负性，通过Transformer模型预测近似最小的单项式基，大幅减少对应的半定规划问题规模，实现100倍以上的加速。


<details>
  <summary>Details</summary>
Motivation: 验证多项式非负性是一个NP难问题，传统的平方和(SOS)方法需要求解半定规划，其维度随单项式基规模呈二次增长，计算成本高昂。

Method: 训练Transformer模型预测给定多项式的近似最小单项式基，包含三个关键组件：生成超过1亿个SOS多项式的训练数据集、设计和训练Transformer架构、确保正确终止的系统回退机制。

Result: 在200多个基准数据集上验证，相比最先进求解器实现超过100倍的加速，并能解决竞争方法无法处理的实例。

Conclusion: 该方法为提升SOS规划的实际可扩展性提供了新的思路，通过机器学习与传统优化方法的结合显著提高了计算效率。
...
### [142] [$L_2$-Regularized Empirical Risk Minimization Guarantees Small Smooth Calibration Error](https://arxiv.org/abs/2510.13450)
...
Method: 建立了基于优化误差、正则化强度和Rademacher复杂度的平滑校准误差的有限样本泛化界，并在再生核希尔伯特空间中实例化该理论。

Result: 实验证实了理论保证，表明L2正则化ERM可以在不使用boosting或后处理重新校准的情况下提供良好校准的模型。
...
### [143] [DistilCLIP-EEG: Enhancing Epileptic Seizure Detection Through Multi-modal Learning and Knowledge Distillation](https://arxiv.org/abs/2510.13497)
*Zexin Wang,Lin Shi,Haoyu Wu,Junru Luo,Xiangzeng Kong,Jun Qi*

Main category: cs.LG

TL;DR: 提出了基于CLIP框架的多模态癫痫检测模型DistilCLIP-EEG，整合EEG信号和文本描述，通过知识蒸馏训练轻量级学生模型，在多个数据集上准确率超过97%。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法仅依赖单模态EEG信号，忽略了多模态信息的潜在优势，需要开发能够整合EEG和文本描述的综合癫痫检测方法。

Method: 使用Conformer架构的EEG编码器和可学习BERT作为文本编码器，在共享潜在空间进行跨模态表示学习，并通过知识蒸馏训练更紧凑的学生模型。

Result: 在TUSZ、AUBMC和CHB-MIT数据集上，教师和学生模型准确率均超过97%，F1分数始终高于0.94，学生模型参数和大小约为教师模型的58.1%。
...
### [144] [Towards Blackwell Optimality: Bellman Optimality Is All You Can Get](https://arxiv.org/abs/2510.13476)
*Victor Boone,Adrienne Tuynman*

Main category: cs.LG

TL;DR: 本文研究了马尔可夫决策过程中偏差最优性层次结构的策略识别问题，提出了具有渐近误差的学习算法，并确定了可在有限时间内停止识别的MDP类别。


<details>
  <summary>Details</summary>
Motivation: 平均增益最优性在MDP中过于渐进，需要结合即时损失度量来考虑偏差最优性层次结构，直至Blackwell最优性。

Method: 为每个最优性阶次构建学习算法，并开发可触发有限时间停止的停止规则。

Result: 识别了具有唯一Bellman最优策略的MDP类别，该类别与最优性阶次无关，且算法可在可能时有限时间停止。

Conclusion: 提出的学习算法具有渐近误差消失特性，停止规则在可能时确保有限时间触发，为偏差最优性策略识别提供了有效解决方案。
...
### [145] [Tahakom LLM guidelines and receipts: from pre-training data to an Arabic LLM](https://arxiv.org/abs/2510.13481)
...
TL;DR: 本文探讨了开发阿拉伯语大语言模型面临的独特挑战，包括数据整理、分词器设计和评估方法，并分享了相关数据和解决方案。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语大语言模型开发面临数据收集、分词器设计和评估框架等独特挑战，需要专门的方法论来解决这些问题。

Method: 采用系统化的方法进行阿拉伯语预训练数据集的收集和过滤，评估不同分词器设计对模型性能的影响，并提出针对现有阿拉伯语评估框架局限性的纠正方法。

Result: 开发了专门针对阿拉伯语的数据整理和分词器设计方法，提出了改进的评估框架，并公开分享了相关数据和方法。

Conclusion: 通过系统化的方法解决了阿拉伯语大语言模型开发的关键挑战，促进了阿拉伯语语言建模的透明性和协作发展。
...
### [146] [Offline and Online KL-Regularized RLHF under Differential Privacy](https://arxiv.org/abs/2510.13512)
*Yulian Wu,Rushil Thareja,Praneeth Vepakomma,Francesco Orabona*

Main category: cs.LG

TL;DR: 本文研究了在局部差分隐私(ϵ-LDP)下的人类反馈强化学习(RLHF)的离线和在线设置，提出了基于悲观主义和乐观主义的算法，分别实现了最优的次优性差距和对数遗憾界。


<details>
  <summary>Details</summary>
Motivation: 研究在保护人类偏好标签隐私的前提下，如何有效进行KL正则化的人类反馈强化学习，解决现有方法在隐私保护方面的不足。

Method: 离线设置采用基于悲观主义的算法，在线设置采用基于乐观主义的算法，并在ϵ-局部差分隐私模型下分析KL正则化RLHF问题。

Result: 离线设置获得了Õ(1/[(e^ϵ-1)^2 n])的次优性差距并证明其最优性；在线设置获得了O(d_F log(N_F·T)/(e^ϵ-1)^2)的对数遗憾界。

Conclusion: 首次在理论上研究了LDP下的KL正则化RLHF问题，提出的算法在离线和在线设置中均取得了理论保证，并为非隐私设置提供了首个分析。
...
### [147] [K-Merge: Online Continual Merging of Adapters for On-device Large Language Models](https://arxiv.org/abs/2510.13537)
...
<details>
  <summary>Details</summary>
Motivation: 移动设备部署LLM时使用LoRA适配器支持多样化任务，但存储容量有限。现有模型合并技术无法有效处理LoRA增量交付的场景，需要在线持续合并新LoRA同时保持已有任务性能。

Method: 设计了一种数据无关且计算高效的策略，当新LoRA可用时，在设备存储容量限制下选择和合并LoRA适配器，无需训练数据。

Result: 在真实世界任务上的广泛实验表明，该方法在遵守设备存储预算和计算限制的同时，相比替代策略具有优越性能。

Conclusion: 提出的方法有效解决了设备上在线持续合并LoRA的挑战，能够在有限存储下支持不断增长的任务需求。
...
### [148] [ProtoTopic: Prototypical Network for Few-Shot Medical Topic Modeling](https://arxiv.org/abs/2510.13542)
*Martin Licht,Sara Ketabi,Farzad Khalvati*

Main category: cs.LG

TL;DR: ProtoTopic是一种基于原型网络的主题模型，专门用于医疗论文摘要的主题生成，在数据有限的情况下表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统主题建模技术在医疗文本上表现不佳，特别是在某些医疗主题文档数量较少的情况下。

Method: 使用原型网络构建主题模型，通过计算输入数据点与原型表示之间的距离来进行预测，特别适合低数据或少样本学习场景。

Result: 与文献中两种主题建模基线方法相比，ProtoTopic在主题连贯性和多样性方面都有显著提升。

Conclusion: ProtoTopic能够在数据有限的情况下生成医学相关的主题，证明了原型网络在医疗文本主题建模中的有效性。
...
### [149] [Multi-Objective $\textit{min-max}$ Online Convex Optimization](https://arxiv.org/abs/2510.13560)
*Rahul Vaze,Sumiran Mishra*

Main category: cs.LG

TL;DR: 本文提出了多目标在线凸优化问题，考虑K个不同的损失函数序列，目标是设计在线算法以最小化min-max遗憾，即与知道所有序列的离线最优算法相比的最大损失差异。


<details>
  <summary>Details</summary>
Motivation: 扩展传统在线凸优化(OCO)框架，考虑多个损失函数序列同时存在的情况，需要平衡跟踪不同序列的权衡，这在现实应用中很常见。

Method: 提出结合Hedge算法和在线梯度下降(OGD)的简单算法，用于i.i.d.输入设置，其中所有损失函数从未知分布独立同分布生成。

Result: 通过简洁证明表明，该算法的期望min-max遗憾为O(√(T log K))，其中T是时间范围，K是损失函数序列数量。

Conclusion: 在多目标OCO框架下，提出的简单算法能够有效处理min-max遗憾问题，在i.i.d.设置下获得次线性遗憾界。
...
### [150] [Message Passing on the Edge: Towards Scalable and Expressive GNNs](https://arxiv.org/abs/2510.13615)
*Pablo Barceló,Fabian Jogl,Alexander Kozachinskiy,Matthias Lanzinger,Stefan Neumann,Cristóbal Rojas*

Main category: cs.LG

TL;DR: 提出了EB-1WL边基颜色细化测试和EB-GNN架构，在消息传递中显式使用三角形，比1-WL更具表达能力，同时保持接近线性的时间和内存复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有GNN架构在表达能力上存在局限，需要开发既具有更强表达能力又保持计算效率的新方法。

Method: 基于Chiba和Nishizeki的经典三角形计数算法，在消息传递过程中显式利用三角形结构，提出EB-1WL测试和EB-GNN架构。

Result: EB-1WL比1-WL表达能力显著更强，EB-GNN在保持接近线性复杂度的同时，性能优于简单MPNN，与专用GNN竞争且计算效率更高。

Conclusion: EB-GNN是一种高效通用的GNN架构，在表达能力和计算效率之间实现了良好平衡，为图学习任务提供了实用解决方案。
...
### [151] [DOLFIN: Balancing Stability and Plasticity in Federated Continual Learning](https://arxiv.org/abs/2510.13567)
...
Method: 提出DOLFIN方法，结合Vision Transformers和低秩适配器(LoRA)，使用DualGradient Projection Memory(DualGPM)防止遗忘，在联邦环境中高效稳定学习新任务。

Result: 在CIFAR-100、ImageNet-R、ImageNet-A和CUB-200数据集上，DOLFIN在两种Dirichlet异构设置下持续超越六个强基线方法，在最终平均准确率上表现更优，同时保持相同内存占用。
...
### [152] [Selective Adversarial Attacks on LLM Benchmarks](https://arxiv.org/abs/2510.13570)
*Ivan Dubrovsky,Anastasia Orlova,Illarion Iov,Nina Gubina,Irena Gureeva,Alexey Zaytsev*

Main category: cs.LG

TL;DR: 该论文研究选择性对抗攻击对LLM基准测试的影响，发现即使细微的文本扰动也能显著改变模型在MMLU基准上的相对排名，挑战了排行榜评估的公平性和可复现性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试容易受到语义等效对抗扰动的影响，但现有研究主要关注对所有模型产生同等影响的攻击，缺乏针对特定模型的选择性攻击研究。

Method: 使用TextAttack框架中的经典攻击方法，开发选择性评估协议，设计定制约束以提高攻击选择性，并提出基于代理LLM的选择性扰动生成流程。

Result: 实验证明选择性对抗攻击确实存在，能够实质性改变模型相对排名，影响评估的公平性和透明度。

Conclusion: 研究结果表明需要采用扰动感知的报告机制和鲁棒性诊断来改进LLM评估，即使是细微的编辑也可能改变比较判断。
...
### [153] [ArtNet: Hierarchical Clustering-Based Artificial Netlist Generator for ML and DTCO Application](https://arxiv.org/abs/2510.13582)
*Andrew B. Kahng. Seokhyeong Kang,Seonghyeon Park,Dooseok Yoon*

Main category: cs.LG

TL;DR: ArtNet是一个人工网表生成器，通过生成具有关键拓扑特征的人工数据集，提升机器学习模型的泛化能力，支持更广泛的设计空间探索，在PPA优化中取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 在先进节点中，PPA优化变得高度复杂和具有挑战性。机器学习和设计技术协同优化面临训练数据缺乏和设计流程周转时间长的限制。

Method: 提出ArtNet人工网表生成器，复制关键拓扑特征，生成更接近目标参数的逼真人工数据集。

Result: 在CNN基DRV预测中，ArtNet数据增强使F1分数比仅使用原始数据集提高0.16；在DTCO中，ArtNet生成的mini-brains实现PPA匹配度达97.94%。

Conclusion: ArtNet能够有效解决训练数据不足问题，支持更高效的PPA优化和设计空间探索。
...
### [154] [Time Series Foundation Models: Benchmarking Challenges and Requirements](https://arxiv.org/abs/2510.13654)
*Marcel Meyer,Sascha Kaltenpoth,Kevin Zalipski,Oliver Müller*

Main category: cs.LG

TL;DR: 本文分析了时间序列基础模型（TSFMs）评估中存在的多重挑战，包括基准数据集代表性不足、缺乏时空评估、信息泄露风险以及外部冲击导致的模式记忆问题。


<details>
  <summary>Details</summary>
Motivation: 随着时间序列基础模型的发展，其零样本预测能力需要可靠的评估方法。然而，现有评估存在数据完整性、基准代表性和信息泄露等问题，这些问题在LLM和传统时间序列评估中已经出现过。

Method: 通过调查现有TSFM评估方法，识别出数据分区混淆、时空评估缺失、数据集重叠和外部冲击记忆等关键问题。

Result: 研究发现现有评估存在普遍的数据分区混乱问题，导致性能估计膨胀和全局知识错误转移到局部时间序列。

Conclusion: 呼吁研究社区开发稳健的评估方法，包括使用真正的样本外未来数据进行评估，以确保TSFM评估的完整性。
...
### [155] [EEGChaT: A Transformer-Based Modular Channel Selector for SEEG Analysis](https://arxiv.org/abs/2510.13592)
*Chen Wang,Yansen Wang,Dongqi Han,Zilong Wang,Dongsheng Li*

Main category: cs.LG

TL;DR: 提出了EEGChaT，一种基于Transformer的通道选择模块，用于自动识别SEEG记录中最具任务相关性的通道，显著提升解码精度并增强可解释性。
...
Result: 在DuIN数据集上的评估显示，将EEGChaT与现有分类模型集成可一致提高解码准确率，实现高达17%的绝对增益，且通道权重与人工选择通道有显著重叠。

Conclusion: EEGChaT是高维SEEG分析中有效且可泛化的通道选择解决方案，既提升了性能又提供了神经信号相关性的见解。
...
### [156] [Axial Neural Networks for Dimension-Free Foundation Models](https://arxiv.org/abs/2510.13665)
*Hyunsu Kim,Jonggeon Park,Joan Bruna,Hongseok Yang,Juho Lee*

Main category: cs.LG

TL;DR: 提出了维度无关的轴向神经网络(XNN)，用于解决物理数据中不同维度系统的训练挑战，在PDE基础模型中实现了跨维度泛化。


<details>
  <summary>Details</summary>
Motivation: 基础模型在物理数据训练中面临维度变化的挑战，传统方法要么固定最大维度要么使用单独编码器，导致效率低下。

Method: 提出轴向神经网络(XNN)架构，受Deep Sets和图神经网络的参数共享结构启发，能够泛化到不同张量维度并保持计算效率。

Result: XNN在三种训练场景下表现与原模型相当，且在未见维度上展现出更优的泛化能力，证明了多维预训练的重要性。

Conclusion: XNN架构有效解决了物理基础模型中的维度变化问题，为多维预训练提供了重要价值。
...
### [157] [Physics-augmented Multi-task Gaussian Process for Modeling Spatiotemporal Dynamics](https://arxiv.org/abs/2510.13601)
*Xizhuo Zhang,Bing Yao*

Main category: cs.LG

TL;DR: 提出了一种物理增强的多任务高斯过程框架，用于高维时空动态系统建模，在心脏电动力学任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 复杂几何域上的高维时空数据建模面临空间结构不规则、时间动态快速以及多变量联合预测的挑战，需要有效整合物理约束和几何先验。

Method: 开发了几何感知的多任务高斯过程模型来捕捉时空结构和任务间依赖关系，并通过物理正则化方案将控制物理定律融入模型。

Result: 在3D心脏电动力学建模任务中，数值实验表明该方法通过有效整合领域特定物理约束和几何先验，显著提高了预测精度。

Conclusion: 物理增强的多任务高斯过程框架能够有效建模复杂时空动态系统，通过结合几何感知和物理约束实现了更准确和鲁棒的预测。
...
### [158] [Simplicial Embeddings Improve Sample Efficiency in Actor-Critic Agents](https://arxiv.org/abs/2510.13704)
*Johan Obando-Ceron,Walter Mayor,Samuel Lavoie,Scott Fujimoto,Aaron Courville,Pablo Samuel Castro*

Main category: cs.LG

TL;DR: 提出使用单纯形嵌入来提升强化学习算法的样本效率和最终性能，这种几何归纳偏置能产生稀疏离散特征，稳定评论家自举并增强策略梯度。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模环境并行化方法虽然加速了训练时间，但仍需要大量环境交互才能达到理想性能。结构良好的表示可以提升深度强化学习的泛化能力和样本效率。

Method: 引入单纯形嵌入层，将嵌入约束到单纯形结构中，产生稀疏离散特征。该方法应用于FastTD3、FastSAC和PPO算法。

Result: 在多种连续和离散控制环境中，单纯形嵌入一致提升了样本效率和最终性能，且不损失运行速度。

Conclusion: 单纯形嵌入是一种有效的几何归纳偏置，能显著提升强化学习算法的性能，同时保持计算效率。
...
TL;DR: 提出了一种基于任务算术和神经正切核的创新方法，用于快速从模型中移除客户端影响，解决现有知识移除方法需要多轮通信导致模型不可用的问题。


<details>
  <summary>Details</summary>
Motivation: 由于隐私法规和安全要求，需要强制移除客户端对模型的贡献。现有知识移除方法需要多轮通信，导致模型在移除过程中不可用，影响系统用户服务。

Method: 基于任务算术和神经正切核的创新解决方案，能够快速移除客户端对模型的影响。

Result: 该方法能够快速有效地移除客户端贡献，满足特定的效能和时间要求。

Conclusion: 提出的解决方案解决了现有知识移除方法的通信延迟问题，能够在保护隐私的同时保持模型可用性。
...
### [160] [The Art of Scaling Reinforcement Learning Compute for LLMs](https://arxiv.org/abs/2510.13786)
...
TL;DR: 该研究提出了首个大规模系统性的强化学习扩展分析框架，通过40万GPU小时的实验定义了可预测的RL扩展方法，并提出了最佳实践方案ScaleRL。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在训练大语言模型中变得至关重要，但缺乏与预训练相当的预测性扩展方法，无法系统评估算法改进对RL计算扩展的影响。

Method: 通过超过40万GPU小时的大规模实验，拟合RL训练的S型计算-性能曲线，消融分析各种常见设计选择对渐近性能和计算效率的影响。

Result: 发现：(1)不同配方的渐近性能不同；(2)损失聚合、归一化、课程学习等细节主要影响计算效率而非渐近性能；(3)稳定可扩展的配方遵循可预测的扩展轨迹。基于此提出了ScaleRL最佳实践方案。

Conclusion: 该工作为分析RL扩展提供了科学框架和实用配方，使RL训练更接近预训练的可预测性水平。
...
TL;DR: 该论文提出了一个系统框架，为经典非线性降维方法构建神经解码器架构，首次实现双向映射，并在此基础上实现扩散生成过程。


<details>
  <summary>Details</summary>
Motivation: 经典非线性降维技术（如t-SNE、Isomap、LLE）只能进行单向降维，无法将低维嵌入映射回原始高维空间，这限制了它们在生成应用中的使用。

Method: 构建神经解码器架构实现双向映射，并在学习到的流形空间上实现基于扩散的生成过程。

Result: 解码器能够成功重建数据，但质量不如端到端优化的自编码器；流形约束扩散生成的样本质量较差。

Conclusion: 为设计主要用于可视化和分析的NLDR方法添加生成能力存在固有挑战，经典NLDR嵌入的离散和稀疏特性不适合生成模型所需的连续插值。
...
### [162] [Provably Invincible Adversarial Attacks on Reinforcement Learning Systems: A Rate-Distortion Information-Theoretic Approach](https://arxiv.org/abs/2510.13792)
*Ziqing Lu,Lifeng Lai,Weiyu Xu*

Main category: cs.LG

TL;DR: 提出一种基于信息论的"不可战胜"对抗攻击方法，通过率失真理论随机改变智能体对转移核的观察，使其在训练中无法获取真实环境信息。


<details>
  <summary>Details</summary>
Motivation: 现有确定性对抗攻击可被智能体通过逆向攻击击败，需要研究更强大的攻击方法以提高RL系统的鲁棒性。

Method: 采用率失真信息论方法，随机改变智能体对转移核或其他属性的观察，限制其获取真实环境信息的能力。

Result: 推导了接收智能体奖励遗憾的信息论下界，展示了率失真攻击对最先进模型基和无模型算法的影响。

Conclusion: 这种信息论方法可扩展到其他类型的对抗攻击，如状态观测攻击，为RL系统安全性研究提供了新视角。
...
### [163] [Multivariate Time Series Forecasting with Gate-Based Quantum Reservoir Computing on NISQ Hardware](https://arxiv.org/abs/2510.13634)
*Wissal Hamhoum,Soumaya Cherkaoui,Jean-Frederic Laprade,Ola Ahmed,Shengrui Wang*

Main category: cs.LG

TL;DR: 提出了基于门的量子储层计算(MTS-QRC)用于多元时间序列预测，该方法针对NISQ硬件优化，在噪声环境下甚至能超越无噪声模拟器。


<details>
  <summary>Details</summary>
Motivation: 现有量子储层计算研究主要针对单变量信号，且忽略了近期硬件的实际约束。

Method: 使用注入和记忆量子比特配对，采用适合当前设备连接性和深度的Trotter化最近邻横向场Ising演化。

Result: 在Lorenz-63和ENSO数据集上分别获得0.0087和0.0036的MSE，性能与经典储层计算相当，在ENSO上甚至优于无噪声模拟器。

Conclusion: 支持基于门的QRC在NISQ硬件上进行多元时间序列预测的实用性，并激励系统研究硬件噪声何时以及如何有益于QRC输出。
...
### [164] [What is the objective of reasoning with reinforcement learning?](https://arxiv.org/abs/2510.13651)
*Damek Davis,Benjamin Recht*

Main category: cs.LG

TL;DR: 本文揭示了多种大语言模型强化学习算法实际上是基于正确答案概率的单调变换进行随机梯度上升


<details>
  <summary>Details</summary>
Motivation: 理解不同强化学习算法在二元奖励场景下的数学本质，揭示它们之间的内在联系

Method: 将拒绝采样算法和GRPO算法等流行RL算法建模为对正确答案概率的单调变换进行随机梯度上升

Result: 发现拒绝采样算法对应对数变换，GRPO算法对应平方根反正弦变换

Conclusion: 不同强化学习算法在数学上具有统一性，都是对正确答案概率的某种单调变换进行优化
...
### [165] [Rebalancing with Calibrated Sub-classes (RCS): An Enhanced Approach for Robust Imbalanced Classification](https://arxiv.org/abs/2510.13656)
*Priyobrata Mondal,Faizanuddin Ansari,Swagatam Das*

Main category: cs.LG

TL;DR: 提出了一种名为RCS的分布校准方法，通过使用来自多数类和中间类的高斯混合组件的加权参数来估计少数类的分布参数，有效解决类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡问题导致分类器偏向多数类，现有方法在使用多数类分布近似少数类统计时存在过度泛化问题。

Method: 使用编码器-解码器网络保持不平衡数据的结构，通过分布校准策略从编码器提取的特征向量生成合成样本，利用邻近区域数据点的分布来校准参数。

Result: 实验结果表明，该方法在图像、文本和表格数据集上相比多个基线和最先进技术实现了更优越的分类性能。

Conclusion: RCS方法通过分布校准有效缓解了类别不平衡问题，提高了分类器的鲁棒性和性能。
...
TL;DR: 本文比较了基于Adam和高斯-牛顿(GN)的两种对角预条件方法，分析其在预条件基选择和梯度噪声影响下的表现，发现在某些情况下Adam优于GN方法，在随机设置下Adam与GN^{-1/2}行为相似。


<details>
  <summary>Details</summary>
Motivation: 对角预条件器作为二阶优化器的计算可行近似，在深度学习训练中显示出加速潜力。Adam和GN是两种主要方法，但它们在预条件基选择和梯度噪声影响下的相对性能尚不清楚。

Method: 通过二次目标和逻辑回归在所有四个象限下分析这些优化器，比较不同基选择和随机梯度噪声的影响。

Result: 无论基选择如何，都存在Adam在完整批次设置中优于GN^{-1}和GN^{-1/2}的情况。在随机设置下，对于高斯数据假设下的线性回归，Adam与GN^{-1/2}行为相似。

Conclusion: 理论分析揭示了Adam和GN方法在不同设置下的相对优势，为选择合适的对角预条件方法提供了指导，这些发现在凸和非凸目标上的实证研究得到了支持。
...
TL;DR: 提出了InfoRM框架，基于信息瓶颈原理过滤偏好无关信息，解决奖励模型的泛化问题；引入IBL正则化惩罚奖励黑客响应；开发MOP指标量化奖励黑客严重程度。


<details>
  <summary>Details</summary>
Motivation: RLHF中的奖励黑客问题严重，主要障碍包括奖励模型的错误泛化（过度拟合偏好无关特征）和RL优化中缺乏合适的正则化约束。

Method: 1. InfoRM：基于信息瓶颈原理的奖励建模框架，过滤偏好无关信息；2. IBL：分布级正则化，惩罚IB潜在空间中的异常响应；3. MOP：基于马氏距离的统计指标，量化奖励黑客严重程度。

Result: 在多种LLM和数据集上的广泛实验证实了方法的有效性，InfoRM和IBL能有效缓解奖励黑客问题，MOP作为诊断工具可靠。

Conclusion: 提出的InfoRM框架、IBL正则化和MOP指标共同推进了RLHF技术的发展，为解决奖励黑客问题提供了系统性的解决方案。
...
### [168] [Don't Be Greedy, Just Relax! Pruning LLMs via Frank-Wolfe](https://arxiv.org/abs/2510.13713)
*Christophe Roux,Max Zimmer,Alexandre d'Aspremont,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 提出了一种基于Frank-Wolfe算法的LLM剪枝方法，通过凸松弛解决组合优化问题，显著降低每层剪枝误差，优于现有基线方法


<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法需要重新训练模型来恢复性能损失，而现有LLM剪枝方法使用贪婪启发式算法，忽略了权重交互，无法找到最优剪枝掩码

Method: 将组合约束进行凸松弛，使用Frank-Wolfe算法求解，然后对松弛解进行取整得到近似解

Result: 大幅减少每层剪枝误差，在GPT架构上优于强基线方法，且保持内存效率

Conclusion: 该方法通过理论保证获得原始组合问题的近似解，为LLM剪枝提供了更有效的解决方案
...
TL;DR: 本文评估深度学习模型在气候降尺度任务中的表现，发现现有模型在地理泛化和物理一致性方面存在不足，并提出使用功率谱密度损失函数来改善小尺度物理结构的重建。
...
Method: 对最先进的深度学习模型进行基准测试，引入物理启发的诊断方法来评估性能，特别关注地理泛化和物理一致性。提出使用功率谱密度损失函数来改善模型表现。

Result: 实验显示，即使在欧洲中心地区训练良好的模型（如CorrDiff），也难以泛化到其他地区（如伊比利亚、摩洛哥、斯堪的纳维亚）。模型还无法准确捕捉从预测速度场导出的二阶变量（如散度和涡度）。

Conclusion: 当前深度学习模型在气候降尺度任务中面临地理泛化和物理一致性的挑战。提出的功率谱密度损失函数能经验性地改善地理泛化能力，但问题仍未完全解决。
...
### [170] [Asymptotically optimal reinforcement learning in Block Markov Decision Processes](https://arxiv.org/abs/2510.13748)
*Thomas van Vuren,Fiona Sloothaak,Maarten G. Wolf,Jaron Sanders*

Main category: cs.LG

TL;DR: 本文研究了块马尔可夫决策过程（BMDPs）中的强化学习，提出了一种利用聚类技术学习潜在状态结构的两阶段算法，显著改善了在高维观测空间中的学习效率。


<details>
  <summary>Details</summary>
Motivation: 高维状态和动作空间使得强化学习在许多实际应用中不可行，但许多环境存在可利用的结构特性。BMDPs模型能够捕捉这种结构，其中观测空间很大但转移动态完全由潜在状态决定。

Method: 采用两阶段强化学习算法：第一阶段通过随机探索学习潜在结构，第二阶段切换到适应已发现结构的乐观引导策略。

Result: 算法在可聚类的BMDPs类上实现了O(√T+n)的遗憾界，显著优于先前最好的O(√T+n²)结果，特别是在观测空间维度n很大时。

Conclusion: 该算法在BMDPs类上达到了渐近最优性，证明了准确估计潜在状态确实能够有效加速学习过程。
...
### [171] [Progressive multi-fidelity learning for physical system predictions](https://arxiv.org/abs/2510.13762)
*Paolo Conti,Mengwu Guo,Attilio Frangi,Andrea Manzoni*

Main category: cs.LG

TL;DR: 提出一种渐进式多保真度代理模型，能够顺序整合不同类型的数据，通过双连接系统利用不同数据集之间的相关性，同时确保每个级别对前一级别进行加法修正而不改变它。
...
Conclusion: 该渐进式多保真度代理模型能有效利用多模态数据，防止新数据集成时的性能下降，并基于可用输入自适应调整预测。
...
TL;DR: 提出了TGPS，一种基于张量高斯过程的PDE求解器，通过一维GP建模各输入维度的因子函数，利用张量分解组合来近似完整解，显著降低计算复杂度并实现大规模配置点的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习PDE求解器存在效率问题：神经网络求解器依赖随机训练且需要大量训练轮次，而高斯过程/核方法求解器在处理大规模配置点时存在可扩展性问题。

Method: 使用张量分解将高维GP建模分解为一维GP的集合，采用部分冻结策略和牛顿法线性化非线性项，开发交替最小二乘法进行高效训练。

Result: 在多个基准PDE上的实验表明，该方法相比现有方法实现了更高的精度和效率。

Conclusion: TGPS方法通过张量分解和高效训练策略，成功解决了传统PDE求解器的效率和可扩展性问题，为高维PDE求解提供了有效方案。
...
### [173] [UrbanFusion: Stochastic Multimodal Fusion for Contrastive Learning of Robust Spatial Representations](https://arxiv.org/abs/2510.13774)
...
<details>
  <summary>Details</summary>
Motivation: 解决当前地理空间数据分析中任务特定模型的局限性，以及现有基础模型对多模态支持不足和缺乏融合能力的问题。

Method: 使用模态特定编码器处理不同输入类型，通过基于Transformer的融合模块学习统一表示，支持在预训练和推理中灵活使用任何可用模态子集。

Result: 在56个城市的41个任务评估中，UrbanFusion在位置编码方面优于先前基础模型，支持推理时的多模态输入，并能很好地泛化到训练未见区域。
...
### [174] [T3former: Temporal Graph Classification with Topological Machine Learning](https://arxiv.org/abs/2510.13789)
*Md. Joshem Uddin,Soham Changani,Baris Coskunuzer*

Main category: cs.LG

TL;DR: T3former是一个新颖的拓扑时序Transformer，通过滑动窗口拓扑和谱描述符作为主要token，使用专门的描述符注意力机制，在时序图分类任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 时序图分类在网络安全、脑连接分析等领域至关重要，但现有方法存在时间信息丢失、长程依赖处理困难、过平滑和过压缩等问题。

Method: 使用滑动窗口拓扑和谱描述符作为主要token，通过专门的描述符注意力机制进行集成，避免刚性离散化，实现跨模态融合。

Result: 在动态社交网络、脑功能连接数据集和交通网络等多个基准测试中达到最先进性能，并提供在时间和结构扰动下的稳定性理论保证。

Conclusion: 结合拓扑和谱洞察能够推动时序图学习的前沿发展，T3former展示了这种方法的强大潜力。
...
### [175] [From Literal to Liberal: A Meta-Prompting Framework for Eliciting Human-Aligned Exception Handling in Large Language Models](https://arxiv.org/abs/2510.12864)
*Imran Khan*

Main category: cs.AI

TL;DR: 提出了Rule-Intent Distinction (RID)框架，一种零样本元提示技术，通过结构化认知模式帮助LLM进行人类对齐的异常处理，显著提升决策质量。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在作为智能体推理引擎时存在的"规则刚性"问题，即过度遵循显式规则而忽视人类常识和意图，这是构建可信自主智能体的重要障碍。

Method: RID框架通过结构化认知模式，让模型能够解构任务、分类规则、权衡冲突结果并最终决策，是一种低计算成本的元提示技术。

Result: 在20个需要细微判断的场景基准测试中，RID框架达到95%的人类对齐分数，显著优于基线(80%)和思维链提示(75%)，并产生更高质量的意图驱动推理。

Conclusion: RID框架提供了一种实用、易用且有效的方法，引导LLM从字面指令遵循转向自由、目标导向的推理，为构建更可靠和实用的AI智能体铺平道路。
...
### [176] [DeepPlanner: Scaling Planning Capability for Deep Research Agents via Advantage Shaping](https://arxiv.org/abs/2510.12979)
*Wei Fan,Wenlin Yao,Zheng Li,Feng Yao,Xin Liu,Liang Qiu,Qingyu Yin,Yangqiu Song,Bing Yin*

Main category: cs.AI

TL;DR: DeepPlanner是一个端到端的强化学习框架，通过熵基优势函数和选择性样本加权来增强深度研究代理的规划能力，在较低训练成本下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖推理阶段的隐式规划，要么引入显式规划器但没有系统优化规划阶段。研究发现标准强化学习下规划标记的熵显著高于其他动作标记，表明存在未优化的不确定决策点。

Method: 提出DeepPlanner框架：1）使用基于熵的优势函数在标记级别分配更大更新给高熵标记；2）对规划密集型轨迹选择性加权样本级优势。

Result: 在七个深度研究基准测试上的广泛实验表明，DeepPlanner提高了规划质量，并在显著降低的训练预算下实现了最先进的结果。

Conclusion: DeepPlanner通过系统优化规划阶段，有效增强了语言模型的规划能力，为复杂任务的长视野规划提供了高效解决方案。
...
### [177] [SENTINEL: A Multi-Level Formal Framework for Safety Evaluation of LLM-based Embodied Agents](https://arxiv.org/abs/2510.12985)
...
TL;DR: Sentinel是首个用于在语义、规划和轨迹三个层次上形式化评估基于LLM的具身智能体物理安全性的框架，通过时序逻辑语义将安全需求形式化，并采用多级验证流程。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式规则或主观的LLM判断，无法精确评估具身智能体的物理安全性，需要一种形式化的验证框架来确保智能体在物理环境中的安全行为。

Method: 采用三级验证管道：语义层将自然语言安全需求形式化为时序逻辑公式并验证LLM理解；规划层验证LLM生成的高层行动计划；轨迹层将执行轨迹合并为计算树进行最终安全检查。

Result: 在VirtualHome和ALFRED环境中评估了多个基于LLM的具身智能体，实验表明Sentinel能够发现先前方法忽略的安全违规，并提供对失败模式的深入洞察。

Conclusion: 通过将物理安全基于时序逻辑并在多个层次应用验证方法，Sentinel为系统评估基于LLM的具身智能体在物理环境中的安全性提供了严格基础。
...
### [178] [From Narratives to Probabilistic Reasoning: Predicting and Interpreting Drivers' Hazardous Actions in Crashes Using Large Language Model](https://arxiv.org/abs/2510.13002)
*Boyou Chen,Gerui Xu,Zifei Wang,Huizhong Guo,Ananna Ahmed,Zhaonan Sun,Zhen Hu,Kaihan Zhang,Shan Bao*

Main category: cs.AI

TL;DR: 提出基于微调大语言模型的框架，自动从事故文本叙述中推断驾驶员危险行为，提高DHA分类的有效性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决大规模数据库中DHA数据可靠性问题，传统人工编码方法不一致且劳动密集，需要自动化解决方案。

Method: 使用MTCF五年双车事故数据，微调Llama 3.2 1B模型处理详细事故叙述，并与随机森林、XGBoost、CatBoost和神经网络等传统机器学习分类器进行基准比较。

Result: 微调LLM达到80%的总体准确率，优于所有基线模型，在数据不平衡场景中表现尤其突出。通过概率推理方法分析模型输出变化，发现分心驾驶和青少年驾驶员对特定DHA类型有显著影响。

Conclusion: 该框架为大规模自动化DHA检测提供了稳健且可解释的解决方案，为交通安全分析和干预开辟了新机会。
...
### [179] [Toward Reasoning-Centric Time-Series Analysis](https://arxiv.org/abs/2510.13029)
*Xinlei Wang,Mingtian Tan,Jing Qiu,Junhua Zhao,Jinjin Gu*

Main category: cs.AI

TL;DR: 本文主张将时间序列分析重新构想为基于大语言模型的推理任务，强调因果结构和可解释性，而不是传统的数值回归方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的时间序列分析需要超越表面趋势，揭示驱动变化的实际力量。现有LLM方法大多忽视其深层推理潜力，仅使用数值回归能力。

Method: 将时间序列分析重新定义为推理任务，利用LLM的多模态输入整合能力，优先考虑因果结构和可解释性。

Result: 这种方法使时间序列分析更接近人类对齐的理解，在复杂现实环境中提供透明和上下文感知的洞察。

Conclusion: 需要重新思考如何有效利用LLM进行时间序列分析，将其从数值回归转向强调因果推理和可解释性的方法。
...
### [180] [Repairing Reward Functions with Human Feedback to Mitigate Reward Hacking](https://arxiv.org/abs/2510.13036)
...
<details>
  <summary>Details</summary>
Motivation: 人工设计的奖励函数经常与人类真实目标不一致，导致策略错位；而从头学习奖励函数需要大量人类偏好数据，成本高昂。

Method: PBRR使用目标探索策略和新的偏好学习目标，在代理奖励函数基础上学习状态转移相关的加性修正项，只需修正少量关键转移即可恢复最优性能。

Result: 在表格域中证明PBRR的累积遗憾与现有偏好强化学习方法相当；在奖励黑客基准测试中，PBRR始终优于从头学习奖励函数的方法，需要更少的偏好数据即可学习高性能策略。

Conclusion: PBRR提供了一种高效的奖励函数修复方法，结合了人工设计奖励函数的先验知识和偏好学习的优势，显著减少了所需的人类偏好数据量。
...
### [181] [Emotional Cognitive Modeling Framework with Desire-Driven Objective Optimization for LLM-empowered Agent in Social Simulation](https://arxiv.org/abs/2510.13195)
...
Conclusion: 该情感认知框架有效解决了LLM智能体在情感认知方面的局限性，实现了与人类更好的情感对齐和决策行为模拟。
...
### [182] [Adaptive Reasoning Executor: A Collaborative Agent System for Efficient Reasoning](https://arxiv.org/abs/2510.13214)
...
<details>
  <summary>Details</summary>
Motivation: 深度推理虽然能提升复杂任务性能，但对所有问题都应用深度推理计算成本过高，需要更高效的解决方案。

Method: 使用小型LLM生成初始答案，大型LLM进行验证，如果答案正确则直接采用，否则大型LLM执行深度推理。

Result: 在简单问题上，大型LLM的计算成本降低50%以上，准确率损失可忽略不计；在复杂任务上保持稳健性能。

Conclusion: 该互补代理系统在显著降低计算成本的同时，保持了任务性能，为LLM应用提供了高效解决方案。
...
### [183] [Personalized Learning Path Planning with Goal-Driven Learner State Modeling](https://arxiv.org/abs/2510.13215)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在个性化学习路径规划中缺乏目标对齐机制，而大型语言模型虽然具有个性化潜力，但需要更好的规划机制。
...
### [184] [EvoTest: Evolutionary Test-Time Learning for Self-Improving Agentic Systems](https://arxiv.org/abs/2510.13220)
*Yufei He,Juncheng Liu,Yue Liu,Yibo Li,Tri Cao,Zhiyuan Hu,Xinxing Xu,Bryan Hooi*

Main category: cs.AI

TL;DR: 提出了Jericho测试时学习基准和EvoTest进化测试时学习框架，通过在连续游戏回合中进化智能体配置来提升性能，无需微调或梯度计算。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体无法在测试时动态学习复杂技能，在陌生环境中表现受限，这严重影响了其实际应用价值。

Method: EvoTest框架包含两个角色：执行游戏的Actor智能体和分析游戏记录并改进配置的Evolver智能体，通过重写提示、更新记忆、调整超参数和学习工具使用例程来进化整个智能体系统。

Result: 在J-TTL基准测试中，EvoTest持续提升性能，优于反思、记忆和在线微调等方法，是唯一能赢得两个游戏的方法。

Conclusion: 进化测试时学习框架能有效解决智能体在陌生环境中的动态学习问题，为AI智能体的实际应用提供了新途径。
...
### [185] [An Analytical Framework to Enhance Autonomous Vehicle Perception for Smart Cities](https://arxiv.org/abs/2510.13230)
*Jalal Khan,Manzoor Khan,Sherzod Turaev,Sumbal Malik,Hesham El-Sayed,Farman Ullah*

Main category: cs.AI

TL;DR: 提出了一种基于效用的分析模型，用于自动驾驶车辆感知系统理解驾驶环境。该模型包含自定义数据集获取、YOLOv8s目标检测模型和感知服务效用测量模块。实验验证了AdamW优化器的YOLOv8s模型在各类别检测性能上表现最佳。
...
Result: 实验显示三个最佳性能的YOLOv8s实例：SGD-based (mAP@0.5: 0.832)、Adam-based (0.810)和AdamW-based (0.822)。AdamW模型在类别级性能上优于SGD模型（如汽车：0.921 vs 0.915，摩托车手：0.899 vs 0.892）。

Conclusion: 所提出的感知模型能够有效评估学习模型的效用，为自动驾驶车辆确定合适的感知方案，验证了该函数在寻找正确感知方面的能力。
...
### [186] [SAJA: A State-Action Joint Attack Framework on Multi-Agent Deep Reinforcement Learning](https://arxiv.org/abs/2510.13262)
*Weiqi Guo,Guanjun Liu,Ziyuan Zhou*

Main category: cs.AI

TL;DR: 提出了状态-动作联合攻击(SAJA)框架，通过协同扰动状态和动作来攻击多智能体深度强化学习模型，相比单独攻击更有效且隐蔽。
...
Result: 在Multi-Agent Particle Environment中评估表明，SAJA优于单独的状态或动作攻击，更隐蔽，且现有防御方法无法抵御其攻击。

Conclusion: SAJA框架通过协同扰动状态和动作，有效提升了多智能体深度强化学习模型的攻击效果，现有防御方法难以应对这种联合攻击。
...
### [187] [Learnable Game-theoretic Policy Optimization for Data-centric Self-explanation Rationalization](https://arxiv.org/abs/2510.13393)
*Yunxiao Zhao,Zhiqiang Wang,Xingtong Yu,Xiaoli Li,Jiye Liang,Ru Li*

Main category: cs.AI

TL;DR: 本文提出PORAT方法，从博弈论视角解决合理化中的模式崩溃问题，通过策略干预引导模型达到更优均衡，在多个数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统合理化方法存在模式崩溃问题，即预测器能正确预测但生成器持续输出崩溃模式的合理化解释，且现有研究缺乏统一考虑。

Method: 从博弈论视角重新审视合作合理化，提出PORAT方法，通过逐步引入策略干预来解决合作博弈过程中的均衡问题。

Result: 在9个真实世界数据集和2个合成设置上验证，PORAT相比现有最优方法性能提升最高达8.1%。

Conclusion: PORAT通过博弈论策略干预有效解决了合理化中的模式崩溃问题，理论分析和实验验证了方法的可行性。
...
### [188] [Assessing LLM Reasoning Through Implicit Causal Chain Discovery in Climate Discourse](https://arxiv.org/abs/2510.13417)
*Liesbeth Allein,Nataly Pineda-Castañeda,Andrea Rocci,Marie-Francine Moens*

Main category: cs.AI

TL;DR: 该研究评估了大型语言模型在因果链发现任务中的机制性因果推理能力，发现LLMs主要通过关联模式匹配而非真正的因果推理来生成中间因果步骤。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型如何理解因果关系，以及它们能否发现连接因果对之间的中间因果步骤，特别是在气候变化等争议性话题的论证场景中。

Method: 使用诊断性评估框架，让9个LLMs为给定的因果对生成所有可能的中间因果步骤，这些因果对来自气候变化论证研究资源。

Result: LLMs生成的因果步骤数量和粒度各不相同，虽然它们对生成的因果链具有自我一致性和信心，但主要基于关联模式匹配而非真正的因果推理。人类评估确认了生成因果链的逻辑连贯性和完整性。

Conclusion: 该研究为未来在论证场景中推进隐式机制性因果推理工作奠定了坚实基础，包括基线因果链发现方法、诊断评估见解和带有因果链的基准数据集。
...
### [189] [Mobile Coverage Analysis using Crowdsourced Data](https://arxiv.org/abs/2510.13459)
...
<details>
  <summary>Details</summary>
Motivation: 移动网络运营商需要有效评估网络覆盖和精确识别服务弱区，以提升用户体验质量(QoE)。

Method: 在单个天线级别进行覆盖分析，然后聚合到站点级别，使用OC-SVM算法将决策超平面建模为有效覆盖轮廓。

Result: 该框架能准确映射移动网络覆盖，特别是在复杂城市环境中突出显示细粒度的信号缺失区域。

Conclusion: 该方法在移动网络覆盖映射和弱信号区域识别方面表现出高效性。
...
### [190] [Confidence as a Reward: Transforming LLMs into Reward Models](https://arxiv.org/abs/2510.13501)
*He Du,Bowen Li,Chengxing Xie,Chang Gao,Kai Chen,Dacheng Tao*

Main category: cs.AI

TL;DR: 提出Confidence-as-a-Reward (CRew)方法，利用模型对最终答案的token级置信度作为奖励信号，无需训练即可在数学推理任务中超越现有无训练奖励方法，甚至优于多数训练过的奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型需要大量标注数据和昂贵训练，而无训练方法如LLM-as-a-Judge虽有效但置信度作为奖励的概念尚未系统研究。

Method: 使用token级置信度作为奖励代理，特别适用于封闭式任务；提出CRew-DPO训练策略，结合置信度得分和正确性信号构建偏好数据。

Result: 在MATH500和RewardMATH基准测试中，CRew优于现有无训练奖励方法，甚至超越多数训练过的奖励模型；CRew得分与模型实际推理性能强相关；CRew能有效筛选高质量训练数据。
...
### [191] [A Methodology for Assessing the Risk of Metric Failure in LLMs Within the Financial Domain](https://arxiv.org/abs/2510.13524)
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在金融服务行业的应用面临性能评估障碍，传统机器学习指标无法很好泛化到GenAI工作负载，而广泛使用的基准测试也无法适应工业应用需求。

Method: 开发了一个风险评估框架，结合领域专家评估和机器学习指标，解决特定指标选择中的独特风险问题。

Result: 提供了一个系统化的评估方法，能够更好地应用领域专家评估和机器学习指标来衡量生成式AI模型性能。

Conclusion: 提出的风险评估框架有助于克服生成式AI在金融服务行业应用中的性能评估障碍，提高模型评估的准确性和实用性。
...
### [192] [Tandem Training for Language Models](https://arxiv.org/abs/2510.13551)
...
<details>
  <summary>Details</summary>
Motivation: 随着语言模型快速发展，其推理过程可能超出人类和较弱智能体的理解范围，影响可解释性和监督。需要确保强模型解决方案对弱协作方保持可理解性

Method: 引入串联训练强化学习范式，在训练过程中随机从冻结的弱模型采样token而非强模型，只有当强模型的行为和推理过程能被弱模型继续时，训练才能成功

Result: 在GSM8K数学推理任务中，串联训练可靠地教会模型放弃专业术语，适应较弱合作伙伴的语言，同时保持高任务准确率

Conclusion: 串联训练为构建可由较弱智能体审计的AI系统提供了一条有前景的路径，对人与AI协作和多智能体通信具有重要意义
...
### [193] [A Modal Logic for Temporal and Jurisdictional Classifier Models](https://arxiv.org/abs/2510.13691)
...
Method: 引入包含时间维度和法院层级系统的分类器模态逻辑，以形式化法律案例推理并解决先例冲突

Result: 开发了一种能够捕捉法律案例推理特征的形式逻辑框架
...
### [194] [Training LLM Agents to Empower Humans](https://arxiv.org/abs/2510.13709)
*Evan Ellis,Vivek Myers,Jens Tuyls,Sergey Levine,Anca Dragan,Benjamin Eysenbach*

Main category: cs.AI

TL;DR: 提出了一种基于最大化人类赋权（empowerment）的辅助语言模型调优方法Empower，仅需离线文本数据，无需额外人工反馈即可训练更好的AI助手。


<details>
  <summary>Details</summary>
Motivation: 当前辅助代理方法往往鼓励AI独立完成任务而非真正协助人类达成目标，且需要昂贵的人工反馈。需要一种更有效、成本更低的训练方法。

Method: 基于最大化人类赋权（影响环境的能力）来调优语言模型，仅使用离线文本数据进行自监督训练，无需额外人工反馈。

Result: 用户研究中参与者78%偏好Empower助手（p=0.015），接受率提高31%，建议减少38%。在编程环境中，模拟人类程序员的成功率比SFT基线平均提高192%。
...
### [195] [From Refusal to Recovery: A Control-Theoretic Approach to Generative AI Guardrails](https://arxiv.org/abs/2510.13727)
*Ravi Pandya,Madison Bland,Duy P. Nguyen,Changliu Liu,Jaime Fernández Fisac,Andrea Bajcsy*

Main category: cs.AI

TL;DR: 本文提出了一种基于控制理论的安全护栏方法，通过实时监控和主动修正AI系统的输出，预防下游危害，而不是简单地阻止危险行为。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全护栏主要依赖输出分类和人工指定标准，对新危险情况脆弱，且检测到不安全时通常只是拒绝行动，这并不总是安全的选择。

Method: 将智能AI安全建模为序列决策问题，在AI模型的潜在表示空间中应用安全关键控制理论，构建预测性护栏，实时监控并主动修正风险输出。

Result: 在模拟驾驶和电子商务场景中的实验表明，控制理论护栏能可靠地引导LLM代理避免灾难性后果（从碰撞到破产），同时保持任务性能。

Conclusion: 控制理论护栏为当前标记-阻止式护栏提供了原则性的动态替代方案，能够更有效地确保智能AI系统的安全性。
...
### [196] [Hard2Verify: A Step-Level Verification Benchmark for Open-Ended Frontier Math](https://arxiv.org/abs/2510.13744)
*Shrey Pandit,Austin Xu,Xuan-Phi Nguyen,Yifei Ming,Caiming Xiong,Shafiq Joty*

Main category: cs.AI

TL;DR: Hard2Verify是一个人工标注的数学证明步骤验证基准，用于评估前沿LLM在挑战性数学问题中的步骤级验证能力。研究发现开源验证器普遍落后于闭源模型，并分析了影响验证性能的关键因素。


<details>
  <summary>Details</summary>
Motivation: 在LLM推理系统需要为每个步骤提供充分支持的挑战性开放环境中，需要强大的验证器来捕捉步骤级错误。现有的验证器评估不够严格，无法满足前沿数学问题验证的需求。

Method: 构建了Hard2Verify基准，包含500多小时人工标注的步骤级验证数据。评估了29个生成式评论器和过程奖励模型，分析验证器在识别前沿LLM生成的数学证明中第一个错误的能力。

Result: 除了少数表现突出者外，开源验证器普遍落后于闭源模型。研究还揭示了验证器计算规模扩展的影响、自验证能力以及验证与生成的动态关系。

Conclusion: Hard2Verify为评估步骤级验证器提供了严格基准，揭示了当前验证技术的局限性，并为改进LLM推理系统的验证能力提供了重要见解。
